...
### Fixed
- `LC044` now follows nested property- and field-receiver chains back to an entity materialized by `AsNoTracking()`, so silently lost graph mutations such as `user.Address.City = value` report before same-context `SaveChanges()` just like direct entity-property writes, while source-visible `[NotMapped]` members remain excluded. Reattach, explicit tracking-state, and earlier-save proof is evaluated independently per member path and must be guaranteed on the mutation-to-save path: updating the mutated nested entity or one of its owners suppresses that mutation, but neither a sibling update, an optional reattach/save, a branch or caught-exception transfer that skips reattachment, nor an earlier safely reattached mutation can hide another untracked write, including inside `foreach` loops. Complementary `if`/`else` branches and normal/catch paths that each unconditionally reattach are recognised collectively; branch targets and post-try operations that still flow through the reattach, non-matching or constant-false catches, exclusive mutation/throw branches, and handlers that return or jump past saving remain quiet.
