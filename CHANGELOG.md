...
### Fixed
- LC047 now reports `ExecuteDelete` / `ExecuteDeleteAsync` when Fluent `OnDelete(ClientCascade|ClientSetNull)` lives on an applied `IEntityTypeConfiguration<T>`: `ApplyConfiguration(new TConfig())`, a single-assignment configuration local, a derived configuration type, expression-bodied `Configure`, same-type configuration helpers including constructed generics, and `ApplyConfigurationsFromAssembly` of the current compilation (`typeof(T).Assembly`, `Assembly.GetExecutingAssembly()`, or a single-assignment local of either). FromAssembly stays quiet for a predicate argument and for abstract, open-generic, or non-public-constructor types, matching EF's constructible-type filter. `Configure` uses the interface implementation actually invoked, so a derived override that replaces `ClientCascade` with database `Cascade` stays quiet. Unapplied configurations, external assemblies, database `Cascade`, dependent deletes, query-filter-only configuration, lookalike `ApplyConfiguration` APIs, and a different context than the one that applied the configuration stay quiet. The cascade fixer remains withheld. LC012 is gated on the same evidence so `RemoveRange` is not rewritten into a hard DELETE.
