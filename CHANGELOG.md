...
### Fixed
- `LC036` now reports `DbContext` captures in `Parallel.For(...)` and `Parallel.Invoke(...)` callbacks, matching the existing `Parallel.ForEach(...)` threading guard while keeping callbacks that create their own context quiet.
