...
### Added
- Real-world corpus validation engine. All 46 analyzers now run against pinned real-world EF Core applications (eShopOnWeb and CleanArchitecture, exact commit hashes in `tools/CorpusValidator/corpus-manifest.json`) with three release gates: every corpus diagnostic must be triaged as a true positive or false positive (stale triage entries fail, so accepted false positives are impossible to ship by accident), analyzer exceptions on real code fail the run, and each analyzer must stay inside a per-rule execution-time budget (`max(baseline × 1.35, 2s)` plus a hard cap) measured on the real compilations. Runs weekly in CI (`corpus.yml`) and on demand. First corpus run: 5 diagnostics, all genuine issues in the corpus codebases, and 0 false positives.
