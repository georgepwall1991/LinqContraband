...
### Fixed
- LC047 now reports `ExecuteDelete` / `ExecuteDeleteAsync` when a `SaveChangesInterceptor` / `ISaveChangesInterceptor` with a proven Deleted-state conversion is registered on `AddDbContext<TContext>`, `AddDbContextPool<TContext>`, or `AddDbContextFactory<TContext>` through an inline options lambda that calls `DbContextOptionsBuilder.AddInterceptors`. Derived interceptor types, base-typed locals assigned from `new TInterceptor()`, and constructed generic contexts are included. Unregistered interceptors, a different generic context, non-generic `AddDbContext(Type, ...)`, lookalike `AddDbContext` / `AddInterceptors` APIs, method-group or helper-only options configuration, and framework `DbContext` stay quiet. LC012 is gated on the same evidence so `RemoveRange` is not rewritten into a hard DELETE.
