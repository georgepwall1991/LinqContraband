...
public sealed class AnalyzerModularizationTests
{
    ...
    public void LC044_RootScanEntries_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(entriesPath),
            "LC044 root-scan entry DTOs should live in a focused partial file."
        );
    ...
    public void LC044_RootScanOperationRecording_LivesInDedicatedPartial()
    {
        ...
        var rootScanPath = Path.Combine(analyzerDir, "AsNoTrackingThenModifyRootScan.cs");
        var operationRecordingPath = Path.Combine(
            analyzerDir,
            "AsNoTrackingThenModifyRootScanOperationRecording.cs"
        );

        Assert.True(
            File.Exists(operationRecordingPath),
            "LC044 root-scan operation recording should live in a focused partial file."
        );
    ...
    public void LC044_OptionalControlFlow_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(optionalFlowPath),
            "LC044 optional branch/loop reachability rules should live in a focused partial file."
        );

        var reachabilitySource = File.ReadAllText(reachabilityPath);
        Assert.DoesNotContain(
            "private static bool IsNestedUnderOptionalControlFlow",
            reachabilitySource
        );
        Assert.DoesNotContain(
    ...
    public void LC044_QuerySourceResolution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(querySourcePath),
            "LC044 AsNoTracking query/materialization source resolution should live in a focused partial file."
        );
    ...
    public void LC008_AsyncContextResolution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(asyncContextPath),
            "LC008 async-context boundary checks should live in a focused partial file."
        );
    ...
    public void LC004_MethodSummaryConsumption_LivesInDedicatedPartial()
    {
        ...
        var consumptionPath = Path.Combine(
            analyzerDir,
            "IQueryableLeakMethodSummaryConsumption.cs"
        );

        Assert.True(
            File.Exists(consumptionPath),
            "LC004 method-summary local consumption checks should live in a focused partial file."
        );
    ...
    public void LC004_InvocationInputMapping_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(invocationInputsPath),
            "LC004 invocation input mapping should live in a focused partial file."
        );

        var sourceResolutionSource = File.ReadAllText(sourceResolutionPath);
        Assert.DoesNotContain(
            "private IEnumerable<InvocationInput> EnumerateInvocationInputs",
    ...
    public void LC004_ExecutableRootTraversal_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(executableRootsPath),
            "LC004 executable-root traversal should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private IEnumerable<IOperation> EnumerateOperations",
            symbolHelpersSource
        );
    ...
    public void LC004_HazardousLookupTables_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(lookupTablesPath),
            "LC004 hazardous method/type lookup tables should live in a focused partial file."
        );

        var compilationStateSource = File.ReadAllText(compilationStatePath);
        Assert.DoesNotContain(
            "private static readonly ImmutableHashSet<string> HazardousEnumerableMethods",
            compilationStateSource
        );
        Assert.DoesNotContain(
            "private static readonly ImmutableHashSet<string> MaterializingCollectionTypes",
            compilationStateSource
        );
    ...
    public void LC001_StaticQueryableOrderingRewrite_LivesInDedicatedPartial()
    {
        ...
        var staticRewritePath = Path.Combine(
            analyzerDir,
            "LocalMethodFixerStaticQueryableRewrite.cs"
        );
        var orderingRewritePath = Path.Combine(
            analyzerDir,
            "LocalMethodFixerStaticQueryableOrdering.cs"
        );

        Assert.True(
            File.Exists(orderingRewritePath),
            "LC001 static Queryable ordering-source rewrite should live in a focused partial file."
        );
    ...
    public void LC044_DiagnosticReporting_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(reportingPath),
            "LC044 diagnostic reporting should live in a focused partial file."
        );
    ...
    public void LC017_FixerPropertyAccessSafety_LivesInDedicatedPartial()
    {
        ...
        var propertyAccessPath = Path.Combine(
            analyzerDir,
            "WholeEntityProjectionFixerPropertyAccess.cs"
        );
        var trackedExpressionsPath = Path.Combine(
            analyzerDir,
            "WholeEntityProjectionFixerTrackedEntityExpressions.cs"
        );

        Assert.True(
            File.Exists(propertyAccessPath),
            "LC017 fixer property-access safety checks should live in a focused partial file."
        );
        Assert.True(
            File.Exists(trackedExpressionsPath),
            "LC017 fixer tracked-entity expression recognition should live in a focused partial file."
        );
    ...
    public void LC017_UsageReferenceClassification_LivesInDedicatedPartial()
    {
        ...
        var usageReferencesPath = Path.Combine(
            analyzerDir,
            "WholeEntityProjectionUsageReferences.cs"
        );

        Assert.True(
            File.Exists(usageReferencesPath),
            "LC017 usage reference classification should live in a focused partial file."
        );
    ...
    public void LC017_SyntaxPropertyCollection_LivesInDedicatedPartial()
    {
        ...
        var syntaxAnalysisPath = Path.Combine(
            analyzerDir,
            "WholeEntityProjectionSyntaxAnalysis.cs"
        );
        var propertyCollectionPath = Path.Combine(
            analyzerDir,
            "WholeEntityProjectionSyntaxPropertyCollection.cs"
        );

        Assert.True(
            File.Exists(propertyCollectionPath),
            "LC017 syntax-based property collection should live in a focused partial file."
        );

        var syntaxAnalysisSource = File.ReadAllText(syntaxAnalysisPath);
        Assert.DoesNotContain(
            "private static void CollectSyntaxBasedPropertyAccesses",
            syntaxAnalysisSource
        );
    ...
    public void LC025_FixerSourceResolution_LivesInDedicatedPartial()
    {
        ...
        var sourceResolutionPath = Path.Combine(
            analyzerDir,
            "AsNoTrackingWithUpdateFixerSourceResolution.cs"
        );
        var noTrackingSourcePath = Path.Combine(
            analyzerDir,
            "AsNoTrackingWithUpdateFixerNoTrackingSource.cs"
        );

        Assert.True(
            File.Exists(sourceResolutionPath),
            "LC025 fixer AsNoTracking source resolution should live in a focused partial file."
        );
        Assert.True(
            File.Exists(noTrackingSourcePath),
            "LC025 fixer recursive no-tracking source tracing should live in a focused partial file."
        );
    ...
    public void LC025_FixerTrackingDirectiveAnalysis_LivesInDedicatedPartial()
    {
        ...
        var trackingDirectivePath = Path.Combine(
            analyzerDir,
            "AsNoTrackingWithUpdateFixerTrackingDirectiveAnalysis.cs"
        );

        Assert.True(
            File.Exists(trackingDirectivePath),
            "LC025 fixer tracking-directive chain analysis should live in a focused partial file."
        );

        var sourceResolutionSource = File.ReadAllText(sourceResolutionPath);
        Assert.DoesNotContain("private static bool HasAsNoTrackingInChain", sourceResolutionSource);
        Assert.DoesNotContain(
            "private static bool IsEfCoreNoTrackingDirective",
            sourceResolutionSource
        );
        Assert.DoesNotContain("private static bool IsEfCoreAsTracking", sourceResolutionSource);
        Assert.DoesNotContain(
            "private static InvocationExpressionSyntax? FindAsNoTrackingInvocation",
            sourceResolutionSource
        );
    ...
    {
        var extensionsDir = Path.Combine(_repoRoot, "src", "LinqContraband", "Extensions");
        ...
        Assert.True(
            File.Exists(lambdaPath),
            "IncludePathParser lambda navigation parsing should live in a focused partial file."
        );
        Assert.True(
            File.Exists(typeAnalysisPath),
            "IncludePathParser type/collection analysis should live in a focused partial file."
        );
    ...
    public void LC045_UsageScan_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(usageScanPath),
            "LC045 executable-root usage scanning should live in a focused partial file."
        );
    ...
    public void LC045_OriginAwareFlowResponsibilities_LiveInDedicatedPartials()
    {
        ...
        Assert.True(
            File.Exists(flowAnalysisPath),
            "LC045 origin-aware control-flow analysis should live in a focused partial file."
        );
        Assert.True(
            File.Exists(flowBindingsPath),
            "LC045 origin and alias binding discovery should live in a focused partial file."
        );
        Assert.True(
            File.Exists(flowContextPath),
            "LC045 origin-flow context construction should live in a focused partial file."
        );
        Assert.True(
            File.Exists(flowEventsPath),
            "LC045 origin-flow event collection should live in a focused partial file."
        );
        Assert.True(
            File.Exists(flowStatePath),
            "LC045 origin-flow state and event models should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "static readonly ConditionalWeakTable<IOperation",
            flowAnalysisSource
        );
    ...
    public void LC045_DiagnosticReporting_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(reportingPath),
            "LC045 missing-path diagnostic reporting should live in a focused partial file."
        );
    ...
    public void LC011_AssemblyLocalScope_LivesInDedicatedPartial()
    {
        ...
        var localResolutionPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyLocalResolution.cs"
        );
        var localScopePath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyLocalScope.cs"
        );

        Assert.True(
            File.Exists(localScopePath),
            "LC011 local Assembly scope/shadowing checks should live in a focused partial file."
        );
    ...
    public void LC011_KeyPropertyRules_LiveInDedicatedPartial()
    {
        ...
        var keyPropertyRulesPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyKeyPropertyRules.cs"
        );

        Assert.True(
            File.Exists(keyPropertyRulesPath),
            "LC011 convention/key-property validation should live in a focused partial file."
        );
    ...
    public void LC015_LocalValueCache_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(localValueCachePath),
            "LC015 local assignment resolution cache should live in a focused partial file."
        );
        Assert.True(
            File.Exists(localReferenceKeyPath),
            "LC015 local-reference cycle keys should live in a focused partial file."
        );
    ...
    public void LC015_QuerySourceResolution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(querySourcePath),
            "LC015 EF query source resolution should live in a focused partial file."
        );
    ...
    public void LC015_DownstreamLocalAnalysis_LivesInDedicatedPartial()
    {
        ...
        var localAnalysisPath = Path.Combine(
            analyzerDir,
            "MissingOrderByDownstreamLocalAnalysis.cs"
        );

        Assert.True(
            File.Exists(localAnalysisPath),
            "LC015 downstream sorting through assigned locals should live in a focused partial file."
        );
    ...
    public void LC015_DownstreamValueFlow_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(valueFlowPath),
            "LC015 downstream local/value-flow tracing should live in a focused partial file."
        );
    ...
    public void LC015_FixerKeyShapeChecks_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(keyShapePath),
            "LC015 fixer composite/keyless key-shape checks should live in a focused partial file."
        );
    ...
    public void LC023_PrimaryKeyCache_LivesInDedicatedPartial()
    {
        ...
        var keyAnalysisPath = Path.Combine(
            analyzerDir,
            "FindInsteadOfFirstOrDefaultKeyAnalysis.cs"
        );
        var primaryKeyCachePath = Path.Combine(
            analyzerDir,
            "FindInsteadOfFirstOrDefaultPrimaryKeyCache.cs"
        );

        Assert.True(
            File.Exists(primaryKeyCachePath),
            "LC023 primary-key cache state and scanning should live in a focused partial file."
        );
    ...
    public void LC027_ConfigurationSyntaxProcessing_LivesInDedicatedPartial()
    {
        ...
        var configurationAnalysisPath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyConfigurationAnalysis.cs"
        );
        var syntaxProcessingPath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyConfigurationSyntaxProcessing.cs"
        );

        Assert.True(
            File.Exists(syntaxProcessingPath),
            "LC027 per-syntax relationship configuration processing should live in a focused partial file."
        );
        ...
        Assert.Contains(
            "private static string GetNavigationConfigurationKey",
            syntaxProcessingSource
        );
    }
    ...
    public void LC045_QueryRootResolution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(queryRootPath),
            "LC045 DbSet query-root proof should live in a focused partial file."
        );
    ...
    public void LC033_FixerCollectionInitializerRewrite_LivesInDedicatedPartial()
    {
        ...
        var rewritePath = Path.Combine(
            analyzerDir,
            "UseFrozenSetForStaticMembershipCachesFixerRewrite.cs"
        );
        var collectionRewritePath = Path.Combine(
            analyzerDir,
            "UseFrozenSetForStaticMembershipCachesFixerCollectionRewrite.cs"
        );

        Assert.True(
            File.Exists(collectionRewritePath),
            "LC033 collection-initializer rewrite rules should live in a focused partial file."
        );
    ...
    public void LC001_StaticQueryableArgumentSelection_LivesInDedicatedPartial()
    {
        ...
        var argumentsPath = Path.Combine(
            analyzerDir,
            "LocalMethodFixerStaticQueryableArguments.cs"
        );

        Assert.True(
            File.Exists(argumentsPath),
            "LC001 static Queryable source-argument selection should live in a focused partial file."
        );
    ...
    public void LC041_PropertyConsumptionAnalysis_LivesInDedicatedPartial()
    {
        ...
        var propertyUsagePath = Path.Combine(
            analyzerDir,
            "SingleEntityScalarProjectionPropertyUsage.cs"
        );

        Assert.True(
            File.Exists(propertyUsagePath),
            "LC041 local property-consumption rules should live in a focused partial file."
        );
    ...
    public void LC027_RelationshipLocalShadowing_LivesInDedicatedPartial()
    {
        ...
        var localScopePath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyRelationshipLocalScope.cs"
        );
        var shadowingPath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyRelationshipLocalShadowing.cs"
        );

        Assert.True(
            File.Exists(shadowingPath),
            "LC027 nested local shadowing checks should live in a focused partial file."
        );
    ...
    public void LC002_ProviderSafeExpressionRules_LiveInDedicatedPartial()
    {
        ...
        var continuationSafetyPath = Path.Combine(
            analyzerDir,
            "PrematureMaterializationContinuationSafety.cs"
        );
        var providerSafeExpressionsPath = Path.Combine(
            analyzerDir,
            "PrematureMaterializationProviderSafeExpressions.cs"
        );

        Assert.True(
            File.Exists(providerSafeExpressionsPath),
            "LC002 provider-safe expression recursion should live in a focused partial file."
        );

        var continuationSafetySource = File.ReadAllText(continuationSafetyPath);
        Assert.DoesNotContain(
            "private static bool IsProviderSafeExpression",
            continuationSafetySource
        );
        Assert.DoesNotContain(
            "private static bool IsProviderSafeObjectCreation",
            continuationSafetySource
        );
        Assert.DoesNotContain(
            "private static bool IsProviderSafeInvocation",
            continuationSafetySource
        );
    ...
    public void LC002_RedundantDiagnosticPolicy_LivesInDedicatedPartial()
    {
        ...
        var originAnalysisPath = Path.Combine(
            analyzerDir,
            "PrematureMaterializationOriginAnalysis.cs"
        );
        var redundantDiagnosticPath = Path.Combine(
            analyzerDir,
            "PrematureMaterializationRedundantDiagnostic.cs"
        );

        Assert.True(
            File.Exists(redundantDiagnosticPath),
            "LC002 redundant-materialization diagnostic policy should live in a focused partial file."
        );

        var originAnalysisSource = File.ReadAllText(originAnalysisPath);
        Assert.DoesNotContain(
            "private static bool TryCreateRedundantDiagnostic",
            originAnalysisSource
        );

        var redundantDiagnosticSource = File.ReadAllText(redundantDiagnosticPath);
        Assert.Contains(
            "private static bool TryCreateRedundantDiagnostic",
            redundantDiagnosticSource
        );
    ...
    public void LC002_DiagnosticDescriptors_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            ...
            "LC002 diagnostic descriptors and diagnostic-property metadata should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "public static readonly DiagnosticDescriptor RedundantRule",
            analyzerSource
        );
        Assert.DoesNotContain(
    ...
    public void LC005_LocalInitializerFlow_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(localFlowPath),
            "LC005 local initializer and write tracking should live in a focused partial file."
        );
    ...
    public void LC006_IncludeChainResultModel_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(chainResultPath),
            "LC006 include-chain result model should live in a focused partial file."
        );
    ...
    public void LC024_GroupAccessTranslation_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(groupAccessPath),
            "LC024 group-chain translatability rules should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static IInvocationOperation FindOutermostGroupChainInvocation",
            analyzerSource
        );
    ...
    public void LC024_GroupingTypeAnalysis_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(groupingTypesPath),
            "LC024 grouping IQueryable/IGrouping type checks should live in a focused partial file."
        );

        var analyzerSource = File.ReadAllText(analyzerPath);
        Assert.DoesNotContain("private static bool IsGroupingQueryable", analyzerSource);
        Assert.DoesNotContain(
            "private static ITypeSymbol? GetQueryableElementType",
            analyzerSource
        );
    ...
    public void LC037_StringBuilderAliasIdentity_LivesInDedicatedPartial()
    {
        ...
        var aliasPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderAliases.cs"
        );
        var identityPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderIdentity.cs"
        );

        Assert.True(
            File.Exists(aliasPath),
            "LC037 StringBuilder alias matching should live in a focused partial file."
        );
        Assert.True(
            File.Exists(identityPath),
            "LC037 StringBuilder alias identity resolution should live in a focused partial file."
        );
    ...
    public void LC037_StringBuilderAliasGuaranteedWrites_LiveInDedicatedPartial()
    {
        ...
        var guaranteedWritesPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderAliasWrites.cs"
        );

        Assert.True(
            File.Exists(guaranteedWritesPath),
            "LC037 StringBuilder alias guaranteed-write resolution should live in a focused partial file."
        );
    ...
    public void LC037_StringBuilderAppendFlow_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(flowPath),
            "LC037 StringBuilder append/reset flow should live in a focused partial file."
        );

        var detectionSource = File.ReadAllText(detectionPath);
        Assert.DoesNotContain(
            "private static bool IsStringBuilderAppendArgumentNonConstant",
            detectionSource
        );
    ...
    public void LC037_StringBuilderLocalWriteFlow_LivesInDedicatedPartial()
    {
        ...
        var flowPath = Path.Combine(analyzerDir, "RawSqlStringConstructionStringBuilderFlow.cs");
        var localWritesPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderLocalWrites.cs"
        );
        var latestWritesPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderLatestLocalWrite.cs"
        );

        Assert.True(
            File.Exists(localWritesPath),
            "LC037 StringBuilder local-write flow should live in a focused partial file."
        );
        Assert.True(
            File.Exists(latestWritesPath),
            "LC037 StringBuilder latest local-write flow should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static IOperation GetCompoundAssignmentRightValue",
            flowSource
        );
    ...
    public void LC037_StringBuilderLoopFlow_LivesInDedicatedPartial()
    {
        ...
        var flowPath = Path.Combine(analyzerDir, "RawSqlStringConstructionStringBuilderFlow.cs");
        var loopFlowPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderLoopFlow.cs"
        );

        Assert.True(
            File.Exists(loopFlowPath),
            "LC037 StringBuilder loop-carried local write flow should live in a focused partial file."
        );
    ...
    public void LC037_LoopCarriedReachability_LivesInDedicatedPartial()
    {
        ...
        var loopReachabilityPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionLoopReachability.cs"
        );

        Assert.True(
            File.Exists(loopReachabilityPath),
            "LC037 loop-carried write reachability should live in a focused partial file."
        );

        var reachabilitySource = File.ReadAllText(reachabilityPath);
        Assert.DoesNotContain(
            "private static bool IsLoopCarriedWriteForReference",
            reachabilitySource
        );
        Assert.DoesNotContain("private static bool IsLoopSyntax", reachabilitySource);
        Assert.DoesNotContain(
            "private static bool CanWriteReachLaterLoopIteration",
            reachabilitySource
        );
    ...
    public void LC037_StringBuilderAppendScan_LivesInDedicatedPartial()
    {
        ...
        var flowPath = Path.Combine(analyzerDir, "RawSqlStringConstructionStringBuilderFlow.cs");
        var appendScanPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderAppendScan.cs"
        );

        Assert.True(
            File.Exists(appendScanPath),
            "LC037 StringBuilder append scan should live in a focused partial file."
        );

        var flowSource = File.ReadAllText(flowPath);
        Assert.DoesNotContain(
            "private static bool ContainsSuspiciousStringBuilderAppend",
            flowSource
        );
        Assert.DoesNotContain(
    ...
    public void LC037_StringBuilderResetDetection_LivesInDedicatedPartial()
    {
        ...
        var resetDetectionPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderResetDetection.cs"
        );

        Assert.True(
            File.Exists(resetDetectionPath),
            "LC037 StringBuilder reset detection should live in a focused partial file."
        );
    ...
    public void LC011_AssemblyConfigurationResolution_LivesInDedicatedPartial()
    {
        ...
        var configurationScanPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyConfigurationScan.cs"
        );
        var assemblyResolutionPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyResolution.cs"
        );
        var assemblyLocalsPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyLocalResolution.cs"
        );
        var assemblyMembersPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyMemberResolution.cs"
        );

        Assert.True(
            File.Exists(assemblyResolutionPath),
            "LC011 ApplyConfigurationsFromAssembly resolution should live in a focused partial file."
        );
        Assert.True(
            File.Exists(assemblyLocalsPath),
            "LC011 ApplyConfigurationsFromAssembly local resolution should live in a focused partial file."
        );
        Assert.True(
            File.Exists(assemblyMembersPath),
            "LC011 ApplyConfigurationsFromAssembly member resolution should live in a focused partial file."
        );

        var configurationScanSource = File.ReadAllText(configurationScanPath);
        Assert.DoesNotContain(
            "private static bool IsCurrentAssemblyExpression",
            configurationScanSource
        );
        Assert.DoesNotContain(
    ...
    public void LC011_AssemblyLocalAssignmentScan_LivesInDedicatedPartial()
    {
        ...
        var assignmentScanPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyLocalAssignments.cs"
        );

        Assert.True(
            File.Exists(assignmentScanPath),
            "LC011 ApplyConfigurationsFromAssembly local assignment scanning should live in a focused partial file."
        );
    ...
    public void LC011_AppliedConfigurationResolution_LivesInDedicatedPartial()
    {
        ...
        var appliedConfigurationPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAppliedConfigurationResolution.cs"
        );

        Assert.True(
            File.Exists(appliedConfigurationPath),
            "LC011 ApplyConfiguration argument/member/local resolution should live in a focused partial file."
        );
    ...
    public void LC011_EntityTypeConfigurationScan_LivesInDedicatedPartial()
    {
        ...
        var entityTypeConfigurationPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyEntityTypeConfigurationScan.cs"
        );

        Assert.True(
            File.Exists(entityTypeConfigurationPath),
            "LC011 IEntityTypeConfiguration<T> scanning should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool TryGetConfiguredEntityType",
            configurationScanSource
        );
    ...
    public void LC011_CompilationModel_LivesInDedicatedPartial()
    {
        ...
        var compilationModelPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyCompilationModel.cs"
        );

        Assert.True(
            File.Exists(compilationModelPath),
            "LC011 compilation model and type index should live in a focused partial file."
        );
    ...
    public void LC011_PrimaryKeyAttributeRules_LiveInDedicatedPartial()
    {
        ...
        var keyRulesPath = Path.Combine(analyzerDir, "EntityMissingPrimaryKeyKeyRules.cs");
        var primaryKeyAttributePath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyPrimaryKeyAttributeRules.cs"
        );

        Assert.True(
            File.Exists(primaryKeyAttributePath),
            "LC011 [PrimaryKey] attribute parsing should live in a focused partial file."
        );

        var keyRulesSource = File.ReadAllText(keyRulesPath);
        Assert.DoesNotContain("private bool HasValidPrimaryKeyAttribute", keyRulesSource);
        Assert.DoesNotContain(
            "private static List<string> GetPrimaryKeyPropertyNames",
            keyRulesSource
        );
    ...
    public void LC011_AssemblyUsingVisibility_LivesInDedicatedPartial()
    {
        ...
        var usingVisibilityPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyUsingVisibility.cs"
        );

        Assert.True(
            File.Exists(usingVisibilityPath),
            "LC011 System.Reflection using and Assembly alias visibility should live in a focused partial file."
        );
    ...
    public void LC017_FixerAccessedProperties_LivesInDedicatedPartial()
    {
        ...
        var contextAnalysisPath = Path.Combine(
            analyzerDir,
            "WholeEntityProjectionFixerContextAnalysis.cs"
        );
        var accessedPropertiesPath = Path.Combine(
            analyzerDir,
            "WholeEntityProjectionFixerAccessedProperties.cs"
        );

        Assert.True(
            File.Exists(accessedPropertiesPath),
            "LC017 fixer accessed-property scanning should live in a focused partial file."
        );
    ...
    public void LC011_AssemblyConfigurationTests_LiveInDedicatedPartial()
    {
        ...
        var assemblyTestsPath = Path.Combine(
            testDir,
            "EntityMissingPrimaryKeyAssemblyConfigurationTests.cs"
        );

        Assert.True(
            File.Exists(assemblyTestsPath),
            "LC011 ApplyConfigurationsFromAssembly edge cases should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "TestCrime_ExternalApplyConfigurationsFromAssembly_ShouldNotApplyLocalConfig",
            edgeCasesSource
        );
        Assert.DoesNotContain(
            "TestInnocent_MemberExecutingAssemblyApplyConfigurationsFromAssembly_ShouldNotTrigger",
            edgeCasesSource
        );
    ...
    public void LC011_BuilderResolution_LivesInDedicatedPartial()
    {
        ...
        var builderResolutionPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyBuilderResolution.cs"
        );
        var localBuilderResolutionPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyLocalBuilderResolution.cs"
        );

        Assert.True(
            File.Exists(builderResolutionPath),
            "LC011 builder-expression and owned-type resolution should live in a focused partial file."
        );
        Assert.True(
            File.Exists(localBuilderResolutionPath),
            "LC011 local builder backtracking should live in a focused partial file."
        );
    ...
    public void LC011_OwnedBuilderNavigationResolution_LivesInDedicatedPartial()
    {
        ...
        var ownedNavigationPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyOwnedBuilderNavigationResolution.cs"
        );

        Assert.True(
            File.Exists(ownedNavigationPath),
            "LC011 owned-entity builder navigation resolution should live in a focused partial file."
        );
    ...
    public void LC007_QueryProvenanceAnalysis_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(queryProvenancePath),
            "LC007 query provenance analysis should live in a focused partial file."
        );
    ...
    public void LC007_QueryProvenanceClassification_LivesInDedicatedPartial()
    {
        ...
        var classificationPath = Path.Combine(
            analyzerDir,
            "NPlusOneLooperQueryProvenanceClassification.cs"
        );

        Assert.True(
            File.Exists(classificationPath),
            "LC007 query provenance source classification should live in a focused partial file."
        );
        ...
        Assert.Contains("private enum QueryProvenanceKind", queryProvenanceSource);
        Assert.DoesNotContain(
            "private static bool IsDbContextSetInvocation",
            queryProvenanceSource
        );
        Assert.DoesNotContain(
            "private static bool IsClientBoundaryInvocation",
            queryProvenanceSource
        );
        Assert.DoesNotContain(
            "private static bool IsNavigationQueryInvocation",
            queryProvenanceSource
        );
        Assert.DoesNotContain("private static bool IsAsQueryableInvocation", queryProvenanceSource);
        Assert.DoesNotContain(
            "private static bool IsChangeTrackingNamespace",
            queryProvenanceSource
        );
    ...
    public void LC007_ExecutionMethodClassification_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(executionMethodsPath),
            "LC007 immediate execution and set-based method classification should live in a focused partial file."
        );

        var analysisSource = File.ReadAllText(analysisPath);
        Assert.DoesNotContain(
            "private static readonly HashSet<string> ImmediateQueryExecutionMethods",
            analysisSource
        );
        Assert.DoesNotContain(
            "private static readonly HashSet<string> SetBasedExecutorMethods",
            analysisSource
        );
    ...
    public void LC007_DatabaseExecutionMatching_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(databaseExecutionPath),
            "LC007 database execution matching should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "public static bool HasStronglyTypedNavigationAccessor",
            analysisSource
        );
    ...
    public void AnalyzerPerformance_LC023Sources_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(lc023SourcesPath),
            "LC023 performance source builders should live in a focused partial file."
        );
    ...
    public void LC002_FixerInlineMaterializationHelpers_LiveInDedicatedPartial()
    {
        ...
        var inlineMaterializationPath = Path.Combine(
            analyzerDir,
            "PrematureMaterializationFixerInlineMaterialization.cs"
        );

        Assert.True(
            File.Exists(inlineMaterializationPath),
            "LC002 fixer inline materializer shape/safety helpers should live in a focused partial file."
        );
        ...
        Assert.Contains(
            "private static bool IsInlineMaterializerReceiver",
        ...
        Assert.Contains(
            "private static bool IsMaterializingConstructor",
            inlineMaterializationSource
        );
    }
    ...
    public void LC010_RetryGuardAnalysis_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(retryGuardPath),
            "LC010 catch-guarded retry suppression should live in a focused partial file."
        );

        var analyzerSource = File.ReadAllText(analyzerPath);
        Assert.Contains("public sealed partial class SaveChangesInLoopAnalyzer", analyzerSource);
        Assert.DoesNotContain(
            "private static bool IsSaveInsideCatchGuardedRetryAttempt",
            analyzerSource
        );
        Assert.DoesNotContain(
    ...
    public void LC010_DelegateExecutionAnalysis_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(analyzerDir, "SaveChangesInLoopAnalyzer.cs");
        var delegateExecutionPath = Path.Combine(
            analyzerDir,
            "SaveChangesInLoopDelegateExecution.cs"
        );

        Assert.True(
            File.Exists(delegateExecutionPath),
            "LC010 delegate-loop execution proof should live in a focused partial file."
        );
        ...
        Assert.Contains(
            "private static bool IsInsideDelegateCalledFromLoop",
            delegateExecutionSource
        );
        Assert.Contains(
    ...
    public void LC010_FixerMoveSafety_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(moveSafetyPath),
            "LC010 fixer move-safety predicates should live in a focused partial file."
        );
    ...
    public void LC032_FixerExtensionNamespaceResolution_LivesInDedicatedPartial()
    {
        ...
        var namespaceResolutionPath = Path.Combine(
            analyzerDir,
            "ExecuteUpdateForBulkUpdatesFixerNamespaceResolution.cs"
        );

        Assert.True(
            File.Exists(namespaceResolutionPath),
            "LC032 fixer ExecuteUpdate extension namespace resolution should live in a focused partial file."
        );
        ...
        Assert.Contains(
            "private static string? ResolveExecuteUpdateNamespace",
            namespaceResolutionSource
        );
    ...
    public void LC016_FixerVariableNameGeneration_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(variableNamesPath),
            "LC016 fixer variable-name collision handling should live in a focused partial file."
        );
    ...
    public void LC023_KeyConfigurationAnalysis_LivesInDedicatedPartial()
    {
        ...
        var keyConfigurationPath = Path.Combine(
            analyzerDir,
            "FindInsteadOfFirstOrDefaultKeyConfiguration.cs"
        );

        Assert.True(
            File.Exists(keyConfigurationPath),
            "LC023 Fluent API key-configuration parsing should live in a focused partial file."
        );

        var keyAnalysisSource = File.ReadAllText(keyAnalysisPath);
        Assert.DoesNotContain(
            "private static bool TryGetEntityTypeBuilderEntity",
            keyAnalysisSource
        );
        Assert.DoesNotContain(
            "private static ConfiguredPrimaryKey AnalyzeKeyArgument",
            keyAnalysisSource
        );
    ...
    public void LC023_PredicateAnalysis_LivesInDedicatedPartial()
    {
        ...
        var predicateAnalysisPath = Path.Combine(
            analyzerDir,
            "FindInsteadOfFirstOrDefaultPredicateAnalysis.cs"
        );

        Assert.True(
            File.Exists(predicateAnalysisPath),
            "LC023 primary-key predicate analysis should live in a focused partial file."
        );
    ...
    public void LC023_FixerKeyValueAnalysis_LivesInDedicatedPartial()
    {
        ...
        var keyValueAnalysisPath = Path.Combine(
            analyzerDir,
            "FindInsteadOfFirstOrDefaultFixerKeyValueAnalysis.cs"
        );

        Assert.True(
            File.Exists(keyValueAnalysisPath),
            "LC023 fixer key-value analysis should live in a focused partial file."
        );
    ...
    public void LC045_UsageTargetHelpers_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(usageTargetsPath),
            "LC045 usage target and result-local helpers should live in a focused partial file."
        );
        Assert.True(
            File.Exists(indexedAccessPath),
            "LC045 indexed collection access classification should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static ILocalSymbol? FindVariableAssignment",
            usageAnalysisSource
        );
    ...
    public void LC045_NavigationAccessCollection_LivesInDedicatedPartial()
    {
        ...
        var usageAnalysisPath = Path.Combine(analyzerDir, "MissingIncludeUsageAnalysis.cs");
        var accessCollectionPath = Path.Combine(
            analyzerDir,
            "MissingIncludeNavigationAccessCollection.cs"
        );

        Assert.True(
            File.Exists(accessCollectionPath),
            "LC045 navigation access collection and satisfaction helpers should live in a focused partial file."
        );
    ...
    public void LC045_QueryOperatorClassification_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(queryOperatorsPath),
            "LC045 query operator/materializer classification should live in a focused partial file."
        );
        Assert.True(
            File.Exists(querySemanticsPath),
            "LC045 exact query symbol/source classification should live in a focused partial file."
        );
        ...
        Assert.Contains(
            "private static bool IsExactShapePreservingQueryStep",
            querySemanticsSource
        );
    }
    ...
    public void LC025_NoTrackingSourceResolution_LivesInDedicatedPartial()
    {
        ...
        var sourceResolutionPath = Path.Combine(
            analyzerDir,
            "AsNoTrackingWithUpdateSourceResolution.cs"
        );

        Assert.True(
            File.Exists(sourceResolutionPath),
            "LC025 local source resolution should live in a focused partial file."
        );
    ...
    public void LC025_EntryStateParsing_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(analyzerDir, "AsNoTrackingWithUpdateAnalyzer.cs");
        var entryStatePath = Path.Combine(
            analyzerDir,
            "AsNoTrackingWithUpdateEntryStateParsing.cs"
        );

        Assert.True(
            File.Exists(entryStatePath),
            "LC025 Entry(entity).State parsing should live in a focused partial file."
        );
    ...
    public void LC014_QuerySourceResolution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(querySourcePath),
            "LC014 EF query-source resolution should live in a focused partial file."
        );
    ...
    public void LC014_ReceiverDependency_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(analyzerDir, "AvoidStringCaseConversionAnalyzer.cs");
        var receiverDependencyPath = Path.Combine(
            analyzerDir,
            "AvoidStringCaseConversionReceiverDependency.cs"
        );

        Assert.True(
            File.Exists(receiverDependencyPath),
            "LC014 receiver dependency analysis should live in a focused partial file."
        );
    ...
    public void LC030_DependencyInjectionRegistration_LivesInDedicatedPartial()
    {
        ...
        var registrationPath = Path.Combine(
            analyzerDir,
            "DbContextInSingletonRegistrationAnalysis.cs"
        );

        Assert.True(
            File.Exists(registrationPath),
            "LC030 dependency-injection registration analysis should live in a focused partial file."
        );
    ...
    public void LC030_RegistrationLifetimeArgumentRules_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(lifetimePath),
            "LC030 AddDbContext singleton-lifetime argument parsing should live in a focused partial file."
        );

        var registrationSource = File.ReadAllText(registrationPath);
        Assert.Contains("private static void AnalyzeAddDbContext", registrationSource);
        Assert.DoesNotContain(
            "private static bool HasSingletonContextLifetimeArgument",
            registrationSource
        );
    ...
    public void LC030_LongLivedEvidence_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            ...
            "LC030 long-lived type evidence should live in a focused partial file."
        );

        var analyzerSource = File.ReadAllText(analyzerPath);
        Assert.DoesNotContain("private static void AddIntrinsicLongLivedEvidence", analyzerSource);
        Assert.DoesNotContain(
            "private static bool HasConventionalMiddlewareSignature",
            analyzerSource
        );
    ...
    public void LC030_LongLivedOptions_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            ...
            "LC030 long-lived options and configured type matching should live in a focused partial file."
        );

        var evidenceSource = File.ReadAllText(evidencePath);
        Assert.DoesNotContain("private static Lc030Options GetOptions", evidenceSource);
        Assert.DoesNotContain(
            "private static bool TryGetConfiguredLongLivedReason",
            evidenceSource
        );
    ...
    public void LC030_FreshComputedPropertyAnalysis_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(analyzerDir, "DbContextInSingletonAnalyzer.cs");
        var freshPropertyPath = Path.Combine(
            analyzerDir,
            "DbContextInSingletonFreshComputedProperty.cs"
        );

        Assert.True(
            File.Exists(freshPropertyPath),
            "LC030 fresh computed DbContext property analysis should live in a focused partial file."
        );
    ...
    public void LC030_CandidateCollectionAndReporting_LiveInDedicatedPartials()
    {
        ...
        var analyzerPath = Path.Combine(analyzerDir, "DbContextInSingletonAnalyzer.cs");
        var candidateCollectionPath = Path.Combine(
            analyzerDir,
            "DbContextInSingletonCandidateCollection.cs"
        );
        var candidateReportingPath = Path.Combine(
            analyzerDir,
            "DbContextInSingletonCandidateReporting.cs"
        );

        Assert.True(
            File.Exists(candidateCollectionPath),
            "LC030 candidate field/property/constructor collection should live in a focused partial file."
        );
        Assert.True(
            File.Exists(candidateReportingPath),
            "LC030 candidate collection and final diagnostic reporting should live in a focused partial file."
        );
    ...
    public void LC033_FixerSyntaxHelpers_LiveInDedicatedPartial()
    {
        ...
        var syntaxPath = Path.Combine(
            fixerDir,
            "UseFrozenSetForStaticMembershipCachesFixerSyntax.cs"
        );

        Assert.True(
            File.Exists(syntaxPath),
            "LC033 FrozenSet fixer syntax construction helpers should live in a focused partial file."
        );

        var rewriteSource = File.ReadAllText(rewritePath);
        Assert.DoesNotContain(
            "private static ExpressionSyntax CreateToFrozenSetInvocation",
            rewriteSource
        );
        Assert.DoesNotContain("private static TypeSyntax CreateTypeSyntax", rewriteSource);
        Assert.DoesNotContain(
            "private static ExpressionSyntax ParenthesizeIfNeeded",
            rewriteSource
        );
    ...
    public void LC035_LocalConditionalFlow_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(
            analyzerDir,
            "MissingWhereBeforeExecuteDeleteUpdateAnalyzer.cs"
        );
        var localFlowPath = Path.Combine(
            analyzerDir,
            "MissingWhereBeforeExecuteDeleteUpdateLocalFlow.cs"
        );

        Assert.True(
            File.Exists(localFlowPath),
            "LC035 local initializer and conditional assignment flow should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool IsControlFlowConditionalAssignment",
            analyzerSource
        );
    ...
    public void LC027_RelationshipBuilderLocalResolution_LivesInDedicatedPartial()
    {
        ...
        var relationshipLocalsPath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyRelationshipLocals.cs"
        );

        Assert.True(
            File.Exists(relationshipLocalsPath),
            "LC027 relationship-builder local resolution should live in a focused partial file."
        );
    ...
    public void LC027_RelationshipBuilderLocalScope_LivesInDedicatedPartial()
    {
        ...
        var relationshipLocalScopePath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyRelationshipLocalScope.cs"
        );
        var relationshipLocalShadowingPath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyRelationshipLocalShadowing.cs"
        );

        Assert.True(
            File.Exists(relationshipLocalScopePath),
            "LC027 relationship-builder local assignment/write-target logic should live in a focused partial file."
        );
        Assert.True(
            File.Exists(relationshipLocalShadowingPath),
            "LC027 relationship-builder local shadowing logic should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static SyntaxNode? FindDesignationScope",
            relationshipLocalsSource
        );
        ...
        Assert.Contains(
            "private static bool IsShadowedByNestedLocal",
            relationshipLocalShadowingSource
        );
        Assert.Contains(
            "private static SyntaxNode? FindDesignationScope",
            relationshipLocalShadowingSource
    ...
    public void LC027_ConfigurationNameExtraction_LivesInDedicatedPartial()
    {
        ...
        var nameExtractionPath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyConfigurationNameExtraction.cs"
        );

        Assert.True(
            File.Exists(nameExtractionPath),
            "LC027 configuration navigation/entity/owned-type extraction should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static string? ExtractNavigationNameFromChain",
            configurationSource
        );
        Assert.DoesNotContain(
    ...
    public void LC032_SaveChangesRewriteMode_LivesInDedicatedPartial()
    {
        ...
        var saveChangesModePath = Path.Combine(
            fixerDir,
            "ExecuteUpdateForBulkUpdatesFixerSaveChangesMode.cs"
        );
        var asyncSupportPath = Path.Combine(
            fixerDir,
            "ExecuteUpdateForBulkUpdatesFixerAsyncSupport.cs"
        );

        Assert.True(
            File.Exists(saveChangesModePath),
            "LC032 SaveChanges/async rewrite-mode logic should live in a focused partial file."
        );
        Assert.True(
            File.Exists(asyncSupportPath),
            "LC032 ExecuteUpdateAsync capability detection should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool HasExecuteUpdateAsyncTokenOverload",
            fixerSource
        );
    ...
    public void LC032_FixerRewriteApplication_LivesInDedicatedPartial()
    {
        ...
        var fixerPath = Path.Combine(fixerDir, "ExecuteUpdateForBulkUpdatesFixer.cs");
        var rewriteApplicationPath = Path.Combine(
            fixerDir,
            "ExecuteUpdateForBulkUpdatesFixerRewriteApplication.cs"
        );

        Assert.True(
            File.Exists(rewriteApplicationPath),
            "LC032 ExecuteUpdate statement generation and document rewrite should live in a focused partial file."
        );
    ...
    public void LC032_SetterExtraction_LivesInDedicatedPartial()
    {
        ...
        var fixerPath = Path.Combine(fixerDir, "ExecuteUpdateForBulkUpdatesFixer.cs");
        var setterExtractionPath = Path.Combine(
            fixerDir,
            "ExecuteUpdateForBulkUpdatesFixerSetters.cs"
        );

        Assert.True(
            File.Exists(setterExtractionPath),
            "LC032 setter extraction and duplicate-target safety should live in a focused partial file."
        );
    ...
    public void LC032_ReceiverNormalization_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(receiverPath),
            "LC032 receiver materializer stripping and unsupported-step detection should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static ExpressionSyntax StripCollectionMaterializer",
            fixerSource
        );
        Assert.DoesNotContain(
            "private static bool HasUnsupportedExecuteUpdateReceiverStep",
            fixerSource
        );
    ...
    public void LC012_RewriteSafetyAnalysis_LivesInDedicatedPartial()
    {
        ...
        var querySourcePath = Path.Combine(
            fixerDir,
            "OptimizeRemoveRangeFixerQuerySourceContext.cs"
        );

        Assert.True(
            File.Exists(rewriteSafetyPath),
            "LC012 rewrite-safety analysis should live in a focused partial file."
        );
        Assert.True(
            File.Exists(querySourcePath),
            "LC012 query-source context resolution should live in a focused partial file."
        );
    ...
    public void LC012_RewriteModeSelection_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(rewriteModePath),
            "LC012 async/sync ExecuteDelete rewrite mode selection should live in a focused partial file."
        );

        var rewriteSafetySource = File.ReadAllText(rewriteSafetyPath);
        Assert.DoesNotContain(
            "private static RewriteMode DetermineRewriteMode",
            rewriteSafetySource
        );
        Assert.DoesNotContain("private static bool IsAsyncContext", rewriteSafetySource);
        Assert.DoesNotContain(
            "private static bool HasExecuteDeleteAsyncSupport",
            rewriteSafetySource
        );
    ...
    public void LC012_SaveChangesSafetyAnalysis_LivesInDedicatedPartial()
    {
        ...
        var rewriteSafetyPath = Path.Combine(fixerDir, "OptimizeRemoveRangeFixerRewriteSafety.cs");
        var saveChangesSafetyPath = Path.Combine(
            fixerDir,
            "OptimizeRemoveRangeFixerSaveChangesSafety.cs"
        );

        Assert.True(
            File.Exists(saveChangesSafetyPath),
            "LC012 SaveChanges detection safety should live in a focused partial file."
        );
    ...
    public void LC012_AnalyzerSaveChangesSafety_LivesInDedicatedPartial()
    {
        ...
        var saveChangesSafetyPath = Path.Combine(
            analyzerDir,
            "OptimizeRemoveRangeAnalyzerSaveChangesSafety.cs"
        );

        Assert.True(
            File.Exists(saveChangesSafetyPath),
            "LC012 analyzer SaveChanges detection and context-aliasing safety should live in a focused partial file."
        );
    ...
    public void LC012_AnalyzerBranchExclusion_LivesInDedicatedPartial()
    {
        ...
        var branchExclusionPath = Path.Combine(
            analyzerDir,
            "OptimizeRemoveRangeAnalyzerBranchExclusion.cs"
        );

        Assert.True(
            File.Exists(branchExclusionPath),
            "LC012 analyzer if/switch mutual-exclusion checks should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static SyntaxNode? GetContainingIfBranch",
            saveChangesSafetySource
        );
        Assert.DoesNotContain(
            "private static SwitchSectionSyntax? GetContainingSwitchSection",
            saveChangesSafetySource
        );
    ...
    public void LC013_AssignedOriginResolution_LivesInDedicatedPartial()
    {
        ...
        var assignedOriginPath = Path.Combine(
            analyzerDir,
            "DisposedContextQueryAssignedOriginResolution.cs"
        );

        Assert.True(
            File.Exists(assignedOriginPath),
            "LC013 assigned local origin resolution should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static IEnumerable<IOperation> EnumerateOperations",
            originSource
        );
    ...
    public void LC016_ExpressionBodyFixer_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(expressionBodyPath),
            "LC016 expression-bodied member rewrite logic should live in a focused partial file."
        );
    ...
    public void LC016_ExpressionBodyStatements_LiveInDedicatedPartial()
    {
        ...
        var expressionBodyPath = Path.Combine(fixerDir, "AvoidDateTimeNowFixerExpressionBody.cs");
        var statementPath = Path.Combine(
            fixerDir,
            "AvoidDateTimeNowFixerExpressionBodyStatements.cs"
        );

        Assert.True(
            File.Exists(statementPath),
            "LC016 expression-bodied return/expression-statement selection should live in a focused partial file."
        );
    ...
    {
        var catalogDir = Path.Combine(_repoRoot, "src", "LinqContraband", "Catalog");
        ...
        Assert.True(
            File.Exists(lc016ToLc030Path),
            "LC016-LC030 catalog entries should live in a focused partial file."
        );
    ...
    public void RuleCatalog_LC031ToLC045Entries_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(lc031ToLc045Path),
            "LC031-LC045 catalog entries should live in a focused partial file."
        );
    ...
    public void LC044_ReachabilityAnalysis_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(analyzerDir, "AsNoTrackingThenModifyAnalyzer.cs");
        var reachabilityPath = Path.Combine(analyzerDir, "AsNoTrackingThenModifyReachability.cs");
        var terminatorsPath = Path.Combine(
            analyzerDir,
            "AsNoTrackingThenModifyReachabilityTerminators.cs"
        );

        Assert.True(
            File.Exists(reachabilityPath),
            "LC044 reachability/control-flow analysis should live in a focused partial file."
        );
        Assert.True(
            File.Exists(terminatorsPath),
            "LC044 reachability terminator and branch-blocking rules should live in a focused partial file."
        );
    ...
    public void LC044_TrackingStateAnalysis_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(trackingStatePath),
            "LC044 reattach/save-state analysis should live in a focused partial file."
        );
    ...
    public void LC044_RootScanTrackingStateParsing_LivesInDedicatedPartial()
    {
        ...
        var rootScanPath = Path.Combine(analyzerDir, "AsNoTrackingThenModifyRootScan.cs");
        var parsingPath = Path.Combine(
            analyzerDir,
            "AsNoTrackingThenModifyRootScanTrackingState.cs"
        );

        Assert.True(
            File.Exists(parsingPath),
            "LC044 root-scan reattach/detach parsing should live in a focused partial file."
        );
    ...
    public void LC044_RootScanBuckets_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(bucketsPath),
            "LC044 root-scan bucket/index helpers should live in a focused partial file."
        );
    ...
    public void LC045_ConditionalAccessAnalysis_LivesInDedicatedPartial()
    {
        ...
        var conditionalAccessPath = Path.Combine(
            analyzerDir,
            "MissingIncludeConditionalAccessAnalysis.cs"
        );

        Assert.True(
            File.Exists(conditionalAccessPath),
            "LC045 conditional-access path analysis should live in a focused partial file."
        );

        var usageSource = File.ReadAllText(usagePath);
        Assert.DoesNotContain(
            "private static IOperation? FindConditionalAccessEntryProperty",
            usageSource
        );
        Assert.DoesNotContain(
            "private static IOperation? ResolveConditionalAccessReceiver",
            usageSource
        );
    ...
    public void LC045_NavigationPathResolution_LivesInDedicatedPartial()
    {
        ...
        var usagePath = Path.Combine(analyzerDir, "MissingIncludeUsageAnalysis.cs");
        var pathResolutionPath = Path.Combine(
            analyzerDir,
            "MissingIncludeNavigationPathAnalysis.cs"
        );

        Assert.True(
            File.Exists(pathResolutionPath),
            "LC045 navigation-path resolution should live in a focused partial file."
        );
    ...
    public void LC037_LocalWriteReachability_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(reachabilityPath),
            "LC037 local-write reachability analysis should live in a focused partial file."
        );
    ...
    public void LC037_ExceptionFlowReachability_LivesInDedicatedPartial()
    {
        ...
        var reachabilityPath = Path.Combine(analyzerDir, "RawSqlStringConstructionReachability.cs");
        var exceptionFlowPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionExceptionFlow.cs"
        );

        Assert.True(
            File.Exists(exceptionFlowPath),
            "LC037 throw/catch reachability analysis should live in a focused partial file."
        );

        var reachabilitySource = File.ReadAllText(reachabilityPath);
        Assert.DoesNotContain(
            "private static bool ThrowCanContinueThroughCatch",
            reachabilitySource
        );
    ...
    public void LC037_ExceptionTypeResolution_LivesInDedicatedPartial()
    {
        ...
        var exceptionTypesPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionExceptionTypes.cs"
        );

        Assert.True(
            File.Exists(exceptionTypesPath),
            "LC037 exception type-name and hierarchy matching should live in a focused partial file."
        );

        var exceptionFlowSource = File.ReadAllText(exceptionFlowPath);
        Assert.DoesNotContain("private static string? GetSimpleTypeName", exceptionFlowSource);
        Assert.DoesNotContain(
            "private static string? GetResolvedSimpleTypeName",
            exceptionFlowSource
        );
    ...
    public void LC037_TerminationFlowReachability_LivesInDedicatedPartial()
    {
        ...
        var reachabilityPath = Path.Combine(analyzerDir, "RawSqlStringConstructionReachability.cs");
        var terminationFlowPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionTerminationFlow.cs"
        );

        Assert.True(
            File.Exists(terminationFlowPath),
            "LC037 branch, jump, and terminator reachability should live in a focused partial file."
        );
    ...
    public void LC037_StringBuilderFlowTests_LiveInDedicatedPartial()
    {
        ...
        var stringBuilderTestsPath = Path.Combine(
            testDir,
            "RawSqlStringConstructionStringBuilderTests.cs"
        );

        Assert.True(
            File.Exists(stringBuilderTestsPath),
            "LC037 StringBuilder flow tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "ExecuteSqlRaw_WithStringBuilderAliasAndLaterAppend_ShouldTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "ExecuteSqlRaw_WithStringBuilderInitializerAppendsClearedBeforeCall_ShouldNotTrigger",
            generalTestsSource
        );
    ...
    public void LC037_StringBuilderReachabilityTests_LiveInDedicatedPartial()
    {
        ...
        var generalTestsPath = Path.Combine(testDir, "RawSqlStringConstructionTests.cs");
        var reachabilityTestsPath = Path.Combine(
            testDir,
            "RawSqlStringConstructionStringBuilderReachabilityTests.cs"
        );

        Assert.True(
            File.Exists(reachabilityTestsPath),
            "LC037 StringBuilder reachability tests should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.DoesNotContain(
            "ExecuteSqlRaw_WithStringBuilderAppendBeforeCaughtThrow_ShouldTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "ExecuteSqlRaw_WithStringBuilderConstructorTaintClearedInFluentChain_ShouldNotTrigger",
            generalTestsSource
        );
    ...
    public void LC037_LocalWriteFlowTests_LiveInDedicatedPartial()
    {
        ...
        var generalTestsPath = Path.Combine(testDir, "RawSqlStringConstructionTests.cs");
        var localWriteTestsPath = Path.Combine(
            testDir,
            "RawSqlStringConstructionLocalWriteTests.cs"
        );

        Assert.True(
            File.Exists(localWriteTestsPath),
            "LC037 local-write flow tests should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.DoesNotContain(
            "ExecuteSqlRaw_WithConstructedInitialValueOverwrittenByConstantBeforeCall_ShouldNotTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "ExecuteSqlRaw_WithConditionalConstructedSqlOverwrittenByConstantBeforeCall_ShouldNotTrigger",
            generalTestsSource
        );
    ...
    public void LC018_ProviderVariantTests_LiveInDedicatedPartial()
    {
        ...
        var providerTestsPath = Path.Combine(
            testDir,
            "AvoidFromSqlRawWithInterpolationProviderVariantTests.cs"
        );

        Assert.True(
            File.Exists(providerTestsPath),
            "LC018 provider-variant and SqlQueryRaw coverage should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "FromSqlRaw_OnDbSet_WithUnsafeInterpolation_ShouldTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "SqlQueryRaw_WithInterpolatedString_ShouldTriggerLC018",
            generalTestsSource
        );
    ...
    public void LC032_FixerSafetyTests_LiveInDedicatedPartial()
    {
        ...
        var safetyTestsPath = Path.Combine(
            testDir,
            "ExecuteUpdateForBulkUpdatesFixerSafetyTests.cs"
        );

        Assert.True(
            File.Exists(safetyTestsPath),
            "LC032 fixer safety and decline cases should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "Fixer_DuplicateProperty_LaterReadsEarlierWrite_DoesNotRegister",
            fixerTestsSource
        );
        Assert.DoesNotContain(
            "Fixer_SyncLocalFunctionInsideAsyncMethod_UsesSyncExecuteUpdate",
            fixerTestsSource
        );
    ...
    public void LC032_FixerQuerySourceTests_LiveInDedicatedPartial()
    {
        ...
        var fixerTestsPath = Path.Combine(testDir, "ExecuteUpdateForBulkUpdatesFixerTests.cs");
        var querySourceTestsPath = Path.Combine(
            testDir,
            "ExecuteUpdateForBulkUpdatesFixerQuerySourceTests.cs"
        );

        Assert.True(
            File.Exists(querySourceTestsPath),
            "LC032 query-source and materializer fixer coverage should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "Fixer_AwaitedInlineToListAsync_StripsMaterializer",
            fixerTestsSource
        );
    ...
    public void LC035_ConditionalFlowTests_LiveInDedicatedPartial()
    {
        ...
        var generalTestsPath = Path.Combine(
            testDir,
            "MissingWhereBeforeExecuteDeleteUpdateTests.cs"
        );
        var conditionalFlowTestsPath = Path.Combine(
            testDir,
            "MissingWhereBeforeExecuteDeleteUpdateConditionalFlowTests.cs"
        );

        Assert.True(
            File.Exists(conditionalFlowTestsPath),
            "LC035 conditional-flow and reassignment tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "ExecuteDelete_UnconditionalFilterThenOptionalNarrowing_ShouldNotTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "ExecuteUpdate_CatchPathReassignsToUnfilteredQuery_ShouldTrigger",
            generalTestsSource
        );
    ...
    public void LC001_StaticQueryableFixerTests_LiveInDedicatedPartial()
    {
        ...
        var staticQueryableTestsPath = Path.Combine(
            testDir,
            "LocalMethodFixerStaticQueryableTests.cs"
        );

        Assert.True(
            File.Exists(staticQueryableTestsPath),
            "LC001 static Queryable fixer coverage should live in a focused partial test file."
        );

        var fixerTestsSource = File.ReadAllText(fixerTestsPath);
        Assert.Contains("public partial class LocalMethodFixerTests", fixerTestsSource);
        Assert.DoesNotContain(
            "FixCrime_StaticQueryableWhere_SwitchesToEnumerable",
            fixerTestsSource
        );
        Assert.DoesNotContain(
            "FixCrime_StaticQueryableThenBy_RewritesExtensionOrderedSourceChain",
            fixerTestsSource
        );
    ...
    public void LC001_StaticQueryableRewrite_LivesInDedicatedPartial()
    {
        ...
        var staticQueryableRewritePath = Path.Combine(
            fixerDir,
            "LocalMethodFixerStaticQueryableRewrite.cs"
        );

        Assert.True(
            File.Exists(staticQueryableRewritePath),
            "LC001 static Queryable rewrite logic should live in a focused partial file."
        );
    ...
    public void LC001_StaticQueryableQualifiers_LiveInDedicatedPartial()
    {
        ...
        var staticQueryableQualifiersPath = Path.Combine(
            fixerDir,
            "LocalMethodFixerStaticQueryableQualifiers.cs"
        );

        Assert.True(
            File.Exists(staticQueryableQualifiersPath),
            "LC001 static Queryable qualifier and method classification should live in a focused partial file."
        );

        var staticQueryableRewriteSource = File.ReadAllText(staticQueryableRewritePath);
        Assert.DoesNotContain(
            "private static ExpressionSyntax CreateEnumerableQualifier",
            staticQueryableRewriteSource
        );
        Assert.DoesNotContain(
            "private static bool IsSystemLinqQueryableType",
        ...
        Assert.Contains(
            "private static ExpressionSyntax CreateEnumerableQualifier",
            staticQueryableQualifiersSource
        );
        Assert.Contains(
            "private static bool IsSystemLinqQueryableType",
            staticQueryableQualifiersSource
    ...
    public void LC001_QueryDiscovery_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(queryDiscoveryPath),
            "LC001 query invocation discovery and rewrite eligibility should live in a focused partial file."
        );
    ...
    public void LC001_AnalyzerTrustPolicy_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(trustPolicyPath),
            "LC001 trusted method and translation-marker policy should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static IEnumerable<IMethodSymbol> EnumerateMethodVariants",
            analyzerSource
        );
    ...
    public void LC036_CapturedContextAnalysis_LivesInDedicatedPartial()
    {
        ...
        var captureAnalysisPath = Path.Combine(
            analyzerDir,
            "DbContextCapturedAcrossThreadsCaptureAnalysis.cs"
        );

        Assert.True(
            File.Exists(captureAnalysisPath),
            "LC036 captured DbContext syntax/symbol analysis should live in a focused partial file."
        );
    ...
    public void LC007_LocalWriteTracking_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(localWritePath),
            "LC007 local-write tracking should live in a focused partial file."
        );
    ...
    public void LC045_ConditionalAccessEdgeCaseTests_LiveInDedicatedPartial()
    {
        ...
        var conditionalAccessTestsPath = Path.Combine(
            testDir,
            "MissingIncludeConditionalAccessEdgeCasesTests.cs"
        );

        Assert.True(
            File.Exists(conditionalAccessTestsPath),
            "LC045 conditional-access edge cases should live in a focused partial test file."
        );

        var edgeCasesSource = File.ReadAllText(edgeCasesPath);
        Assert.Contains("public partial class MissingIncludeEdgeCasesTests", edgeCasesSource);
        Assert.DoesNotContain(
            "TestInnocent_ConditionalCollectionMutatorCall_NoDiagnostic",
            edgeCasesSource
        );
        Assert.DoesNotContain(
            "TestCrime_InheritedNavigationParenthesizedConditionalAccessReportsFullNestedPath",
            edgeCasesSource
        );
    ...
    public void LC045_ConditionalAccessCrimeTests_LiveInDedicatedPartial()
    {
        ...
        var conditionalAccessTestsPath = Path.Combine(
            testDir,
            "MissingIncludeConditionalAccessTests.cs"
        );

        Assert.True(
            File.Exists(conditionalAccessTestsPath),
            "LC045 conditional-access crime cases should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.Contains("public partial class MissingIncludeTests", generalTestsSource);
        Assert.DoesNotContain(
            "TestCrime_ConditionalAccessNav_TriggersDiagnostic",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "TestCrime_LocalFromConditionalIndexer_TriggersDiagnostic",
            generalTestsSource
        );
    ...
    public void LC045_CoveredNavigationTests_LiveInDedicatedPartial()
    {
        ...
        var edgeCasesPath = Path.Combine(testDir, "MissingIncludeEdgeCasesTests.cs");
        var coveredNavigationTestsPath = Path.Combine(
            testDir,
            "MissingIncludeCoveredNavigationTests.cs"
        );

        Assert.True(
            File.Exists(coveredNavigationTestsPath),
            "LC045 include-covered navigation tests should live in a focused partial test file."
        );

        var edgeCasesSource = File.ReadAllText(edgeCasesPath);
        Assert.Contains("public partial class MissingIncludeEdgeCasesTests", edgeCasesSource);
        Assert.DoesNotContain(
            "TestInnocent_LambdaIncludeCoversAccess_NoDiagnostic",
            edgeCasesSource
        );
        ...
        Assert.Contains(
            "TestInnocent_CastMidPathInclude_NoDiagnostic",
            coveredNavigationTestsSource
        );
    }
    ...
    public void LC045_FixerSyntaxTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(syntaxTestsPath),
            "LC045 fixer syntax and fix-all edge cases should live in a focused partial test file."
        );

        var fixerTestsSource = File.ReadAllText(fixerTestsPath);
        Assert.Contains("public partial class MissingIncludeFixerTests", fixerTestsSource);
        Assert.DoesNotContain(
            "FixCrime_StaticCallSyntax_WrapsTheArgumentNotTheTypeName",
            fixerTestsSource
        );
    ...
    public void LC025_ReassignmentTests_LiveInDedicatedPartial()
    {
        ...
        var reassignmentTestsPath = Path.Combine(
            testDir,
            "AsNoTrackingWithUpdateReassignmentTests.cs"
        );

        Assert.True(
            File.Exists(reassignmentTestsPath),
            "LC025 reassignment and ambiguous-origin tests should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.Contains("public partial class AsNoTrackingWithUpdateTests", generalTestsSource);
        Assert.DoesNotContain(
            "ConditionallyReassignedToNoTracking_AmbiguousOrigin_ShouldNotTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "ReassignedInsideSameBranchAsUpdate_ShouldStillTrigger",
            generalTestsSource
        );
    ...
    public void LC025_ProjectionTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(projectionTestsPath),
            "LC025 projection and query-alias tests should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.Contains("public partial class AsNoTrackingWithUpdateTests", generalTestsSource);
        Assert.DoesNotContain(
            "MaterializedFromNoTrackingQueryAlias_ShouldTriggerLC025",
            generalTestsSource
        );
    ...
    public void LC024_QuerySyntaxTests_LiveInDedicatedPartial()
    {
        ...
        var querySyntaxTestsPath = Path.Combine(
            testDir,
            "GroupByNonTranslatableQuerySyntaxTests.cs"
        );

        Assert.True(
            File.Exists(querySyntaxTestsPath),
            "LC024 query-syntax GroupBy coverage should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.Contains("public partial class GroupByNonTranslatableTests", generalTestsSource);
        Assert.DoesNotContain(
            "QuerySyntaxGroupBy_Select_ToList_ShouldTriggerLC024",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "EnumerableQuerySyntaxGroupBy_Select_ClientProjection_ShouldNotTrigger",
            generalTestsSource
        );
    ...
    public void LC012_FixerContextSafetyTests_LiveInDedicatedPartial()
    {
        ...
        var contextSafetyTestsPath = Path.Combine(
            testDir,
            "OptimizeRemoveRangeFixerContextSafetyTests.cs"
        );

        Assert.True(
            File.Exists(contextSafetyTestsPath),
            "LC012 fixer SaveChanges/context safety tests should live in a focused partial test file."
        );

        var fixerTestsSource = File.ReadAllText(fixerTestsPath);
        Assert.Contains("public partial class OptimizeRemoveRangeFixerTests", fixerTestsSource);
        Assert.DoesNotContain(
            "Fixer_ShouldNotRegister_WhenRemoveRangeIsFollowedBySaveChanges",
            fixerTestsSource
        );
        Assert.DoesNotContain(
            "Fixer_ShouldNotRegister_WhenQueryCombinesLaterSaveContextSource",
            fixerTestsSource
        );
    ...
    public void LC012_AnalyzerSaveChangesTests_LiveInDedicatedPartial()
    {
        ...
        var saveChangesTestsPath = Path.Combine(
            testDir,
            "OptimizeRemoveRangeAnalyzerSaveChangesTests.cs"
        );

        Assert.True(
            File.Exists(saveChangesTestsPath),
            "LC012 analyzer SaveChanges/control-flow suppression tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "RemoveRange_FollowedBySaveChanges_ShouldNotTrigger",
            analyzerTestsSource
        );
        Assert.DoesNotContain(
            "RemoveRange_InTryWithSaveChangesInCatch_ShouldNotTrigger",
            analyzerTestsSource
        );
    ...
    public void LC014_ColumnDerivedArgumentTests_LiveInDedicatedPartial()
    {
        ...
        var argumentTestsPath = Path.Combine(
            testDir,
            "AvoidStringCaseConversionColumnDerivedArgumentTests.cs"
        );

        Assert.True(
            File.Exists(argumentTestsPath),
            "LC014 column-derived method argument tests should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.Contains("public partial class AvoidStringCaseConversionTests", generalTestsSource);
        Assert.DoesNotContain(
            "ToLower_OnMethodResultWithParameterArgument_ShouldTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "ConstantReceiver_ColumnAsStringArgument_ShouldTrigger",
            generalTestsSource
        );
    ...
    public void LC015_PositionOperatorTests_LiveInDedicatedPartial()
    {
        ...
        var positionOperatorTestsPath = Path.Combine(
            testDir,
            "MissingOrderByPositionOperatorTests.cs"
        );

        Assert.True(
            File.Exists(positionOperatorTestsPath),
            "LC015 ElementAt/Last operator coverage should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "TakeLast_IsNotFlagged_BecauseEfCannotTranslateItAtAll",
            generalTestsSource
        );
    ...
    public void LC023_QueryFilterTests_LiveInDedicatedPartial()
    {
        ...
        var queryFilterTestsPath = Path.Combine(
            testDir,
            "FindInsteadOfFirstOrDefaultQueryFilterTests.cs"
        );

        Assert.True(
            File.Exists(queryFilterTestsPath),
            "LC023 query-filter gate tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "FirstOrDefault_WithId_WhenEntityHasQueryFilter_ShouldNotTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "FirstOrDefault_WithUnrelatedGenericHasQueryFilter_ShouldStillTrigger",
            generalTestsSource
        );
    ...
    public void LC023_KeyConfigurationTests_LiveInDedicatedPartial()
    {
        ...
        var generalTestsPath = Path.Combine(testDir, "FindInsteadOfFirstOrDefaultTests.cs");
        var keyConfigurationTestsPath = Path.Combine(
            testDir,
            "FindInsteadOfFirstOrDefaultKeyConfigurationTests.cs"
        );

        Assert.True(
            File.Exists(keyConfigurationTestsPath),
            "LC023 key-configuration tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "FirstOrDefault_WithConventionId_WhenFluentApiConfiguresDifferentKey_ShouldNotTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "FirstOrDefault_WithPartialCompositeFluentApiKey_ShouldNotTrigger",
            generalTestsSource
        );
    ...
    public void LC023_QueryFilterCache_LivesInDedicatedPartial()
    {
        ...
        var queryFilterCachePath = Path.Combine(
            analyzerDir,
            "FindInsteadOfFirstOrDefaultQueryFilterCache.cs"
        );

        Assert.True(
            File.Exists(primaryKeyCachePath),
            "LC023 primary-key cache state should live in a focused partial file."
        );
        Assert.True(
            File.Exists(queryFilterCachePath),
            "LC023 query-filter cache registration and lookup should live in a focused partial file."
        );
    ...
    public void LC023_FixerContext_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            ...
            "LC023 fixer context derivation should live in a focused partial file."
        );
    ...
    public void LC027_RelationshipBuilderLocalTests_LiveInDedicatedPartial()
    {
        ...
        var relationshipBuilderTestsPath = Path.Combine(
            testDir,
            "MissingExplicitForeignKeyRelationshipBuilderLocalTests.cs"
        );

        Assert.True(
            File.Exists(relationshipBuilderTestsPath),
            "LC027 relationship-builder local and shadowing tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "Navigation_WithRelationshipBuilderLocalShadowForeignKey_ShouldNotTrigger",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "Navigation_WithMemberAssignmentSharingRelationshipBuilderLocalName_ShouldNotTrigger",
            generalTestsSource
        );
        ...
        Assert.Contains(
            "public partial class MissingExplicitForeignKeyEdgeCasesTests",
            relationshipBuilderTestsSource
        );
        Assert.Contains(
            "Navigation_WithRelationshipBuilderLocalShadowForeignKey_ShouldNotTrigger",
            relationshipBuilderTestsSource
    ...
    public void LC041_NullConditionalTests_LiveInDedicatedPartial()
    {
        ...
        var nullConditionalTestsPath = Path.Combine(
            testDir,
            "SingleEntityScalarProjectionNullConditionalTests.cs"
        );

        Assert.True(
            File.Exists(nullConditionalTestsPath),
            "LC041 null-conditional projection tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "FirstOrDefault_WithNullConditionalSinglePropertyUsage_Triggers",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "First_WithNullConditionalSinglePropertyMethodUsage_ShouldNotOfferFix",
            generalTestsSource
        );
    ...
    public void LC041_FixerTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(fixerTestsPath),
            "LC041 fixer and fix-all coverage should live in a focused partial test file."
        );
    ...
    public void LC041_FixerContext_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(contextPath),
            "LC041 fixer context derivation and predicate safety should live in a focused partial file."
        );
    ...
    public void LC041_PrimaryKeyLookupAnalysis_LivesInDedicatedPartial()
    {
        ...
        var queryAnalysisPath = Path.Combine(
            analyzerDir,
            "SingleEntityScalarProjectionQueryAnalysis.cs"
        );
        var primaryKeyLookupPath = Path.Combine(
            analyzerDir,
            "SingleEntityScalarProjectionPrimaryKeyLookup.cs"
        );

        Assert.True(
            File.Exists(primaryKeyLookupPath),
            "LC041 primary-key lookup exemption analysis should live in a focused partial file."
        );
    ...
    public void LC033_InitializerClassification_LivesInDedicatedPartial()
    {
        ...
        var analysisPath = Path.Combine(
            analyzerDir,
            "UseFrozenSetForStaticMembershipCachesAnalysis.cs"
        );
        var classificationPath = Path.Combine(
            analyzerDir,
            "UseFrozenSetForStaticMembershipCachesInitializerClassification.cs"
        );
        var toHashSetPath = Path.Combine(
            analyzerDir,
            "UseFrozenSetForStaticMembershipCachesToHashSetClassification.cs"
        );

        Assert.True(
            File.Exists(classificationPath),
            "LC033 initializer classification should live in a focused partial file."
        );
        Assert.True(
            File.Exists(toHashSetPath),
            "LC033 ToHashSet classification should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool IsSupportedCollectionInitializer",
            analysisSource
        );
    ...
    public void LC018_ConstantInterpolationTests_LiveInDedicatedPartial()
    {
        ...
        var generalTestsPath = Path.Combine(testDir, "AvoidFromSqlRawWithInterpolationTests.cs");
        var constantInterpolationTestsPath = Path.Combine(
            testDir,
            "AvoidFromSqlRawWithInterpolationConstantInterpolationTests.cs"
        );

        Assert.True(
            File.Exists(constantInterpolationTestsPath),
            "LC018 constant-interpolation boundary tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "FromSqlRaw_WithConstantOnlyInterpolation_ShouldNotTriggerLC018",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "FromSqlRaw_WithStaticReadonlyFieldInterpolation_ShouldTriggerLC018",
            generalTestsSource
        );
        ...
        Assert.Contains(
            "public partial class AvoidFromSqlRawWithInterpolationTests",
            constantInterpolationTestsSource
        );
        Assert.Contains(
            "FromSqlRaw_WithConstantOnlyInterpolation_ShouldNotTriggerLC018",
            constantInterpolationTestsSource
    ...
    public void LC018_FixerTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(fixerTestsPath),
            "LC018 fixer and fix-all coverage should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "Fixer_ShouldReplaceFromSqlRawWithFromSqlInterpolated",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "FixAll_RewritesAllFromSqlRawWithInterpolatedStringInstances",
            generalTestsSource
        );
    ...
    public void LC018_FixerSqlSafety_LivesInDedicatedPartial()
    {
        ...
        var sqlSafetyPath = Path.Combine(
            fixerDir,
            "AvoidFromSqlRawWithInterpolationFixerSqlSafety.cs"
        );

        Assert.True(
            File.Exists(sqlSafetyPath),
            "LC018 fixer SQL interpolation safety checks should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool HasInterpolationInsideSqlStringLiteral",
            fixerSource
        );
        Assert.DoesNotContain(
            "private static bool HasInterpolationOutsideLikelySqlValuePosition",
            fixerSource
        );
    ...
    public void LC034_FixerTests_LiveInDedicatedPartial()
    {
        ...
        var fixerTestsPath = Path.Combine(
            testDir,
            "AvoidExecuteSqlRawWithInterpolationFixerTests.cs"
        );

        Assert.True(
            File.Exists(fixerTestsPath),
            "LC034 fixer coverage should live in a focused partial test file."
        );
    ...
    public void LC011_FluentConfigurationTests_LiveInDedicatedPartial()
    {
        ...
        var edgeCasesPath = Path.Combine(testDir, "EntityMissingPrimaryKeyEdgeCasesTests.cs");
        var fluentConfigurationTestsPath = Path.Combine(
            testDir,
            "EntityMissingPrimaryKeyFluentConfigurationTests.cs"
        );

        Assert.True(
            File.Exists(fluentConfigurationTestsPath),
            "LC011 fluent configuration edge cases should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "TestCrime_UnappliedEntityTypeConfiguration_ShouldTrigger",
            edgeCasesSource
        );
        Assert.DoesNotContain(
            "TestInnocent_ScopedBuilderVariableReuse_ShouldNotTrigger",
            edgeCasesSource
        );
        ...
        Assert.Contains(
            "public partial class EntityMissingPrimaryKeyEdgeCasesTests",
            fluentConfigurationTestsSource
        );
        Assert.Contains(
            "TestCrime_UnappliedEntityTypeConfiguration_ShouldTrigger",
            fluentConfigurationTestsSource
    ...
    public void LC011_KeyConventionTests_LiveInDedicatedPartial()
    {
        ...
        var conventionTestsPath = Path.Combine(
            testDir,
            "EntityMissingPrimaryKeyKeyConventionTests.cs"
        );

        Assert.True(
            File.Exists(conventionTestsPath),
            "LC011 key convention and key-shape tests should live in a focused partial test file."
        );
    ...
    public void LC030_RegistrationTests_LiveInDedicatedPartial()
    {
        ...
        var registrationTestsPath = Path.Combine(
            testDir,
            "DbContextInSingletonRegistrationTests.cs"
        );

        Assert.True(
            File.Exists(registrationTestsPath),
            "LC030 dependency-injection registration tests should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.Contains("public partial class DbContextInSingletonTests", generalTestsSource);
        Assert.DoesNotContain(
            "AddSingletonRegisteredService_WithStoredDbContext_ShouldTriggerLC030",
            generalTestsSource
        );
    ...
    public void LC030_RegistrationTypeResolution_LivesInDedicatedPartial()
    {
        ...
        var typeResolutionPath = Path.Combine(
            analyzerDir,
            "DbContextInSingletonRegistrationTypeResolution.cs"
        );

        Assert.True(
            File.Exists(typeResolutionPath),
            "LC030 registration type resolution should live in a focused partial file."
        );
    ...
    public void LC017_PropertyAccessEdgeCaseTests_LiveInDedicatedPartial()
    {
        ...
        var propertyAccessTestsPath = Path.Combine(
            testDir,
            "WholeEntityProjectionPropertyAccessEdgeCasesTests.cs"
        );

        Assert.True(
            File.Exists(propertyAccessTestsPath),
            "LC017 property-access edge cases should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "TestCrime_NullConditionalPropertyAccess_TriggersDiagnostic",
            edgeCasesSource
        );
    ...
    public void LC017_FixerSafetyTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(safetyTestsPath),
            "LC017 fixer safety/no-fix cases should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "ConditionalInterfaceCastAccessedProperty_HasNoFix",
            fixerTestsSource
        );
    ...
    public void RuleCatalog_LC001ToLC015Entries_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(firstRangePath),
            "RuleCatalog LC001-LC015 entries should live in a focused partial file."
        );
    ...
    public void LC039_TransactionTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(transactionTestsPath),
            "LC039 transaction-boundary tests should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "RepeatedSavesInsideAwaitUsingDeclarationTransaction_DoesNotTrigger",
            generalTestsSource
        );
    ...
    public void LC039_TransactionBoundaryClassification_LivesInDedicatedPartial()
    {
        ...
        var transactionBoundaryPath = Path.Combine(
            analyzerDir,
            "NestedSaveChangesTransactionBoundaryClassification.cs"
        );

        Assert.True(
            File.Exists(transactionBoundaryPath),
            "LC039 EF transaction-boundary classification should live in a focused partial file."
        );

        var analyzerSource = File.ReadAllText(analyzerPath);
        Assert.DoesNotContain(
            "private static bool IsTransactionBoundaryInvocation",
            analyzerSource
        );
        Assert.DoesNotContain(
            "private static bool IsEfCoreTransactionBoundaryType",
            analyzerSource
        );
        Assert.DoesNotContain(
            "private static bool IsEfCoreTransactionBoundaryNamedType",
            analyzerSource
        );
    ...
    public void LC039_BranchExclusivity_LivesInDedicatedPartial()
    {
        ...
        var branchExclusivityPath = Path.Combine(
            analyzerDir,
            "NestedSaveChangesBranchExclusivity.cs"
        );

        Assert.True(
            File.Exists(branchExclusivityPath),
            "LC039 mutually-exclusive branch detection should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static SwitchExpressionArmSyntax? GetContainingSwitchExpressionArm",
            boundarySource
        );
        ...
        Assert.Contains(
            "private static bool AreMutuallyExclusiveBranches",
            branchExclusivitySource
        );
        Assert.Contains(
            "private static SyntaxNode? GetContainingTryBranch",
            branchExclusivitySource
        );
        Assert.Contains(
            "private static StatementSyntax? GetContainingBranch",
    ...
    public void LC039_DiagnosticReporting_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(reportingPath),
            "LC039 compilation-end diagnostic grouping/reporting should live in a focused partial file."
        );
    ...
    public void LC020_QueryableParameterTracing_LivesInDedicatedPartial()
    {
        ...
        var queryableParametersPath = Path.Combine(
            analyzerDir,
            "StringContainsWithComparisonQueryableParameters.cs"
        );

        Assert.True(
            File.Exists(queryableParametersPath),
            "LC020 queryable lambda parameter tracing should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static ImmutableArray<IParameterSymbol> GetQueryableExpressionLambdaParameters",
            analyzerSource
        );
        Assert.DoesNotContain(
    ...
    public void LC024_ProjectionAccessScanning_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(analyzerDir, "GroupByNonTranslatableAnalyzer.cs");
        var projectionAccessPath = Path.Combine(
            analyzerDir,
            "GroupByNonTranslatableProjectionAccess.cs"
        );

        Assert.True(
            File.Exists(projectionAccessPath),
            "LC024 projection access scanning/reporting should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static System.Collections.Generic.IEnumerable<IOperation> GetAllOperations",
            analyzerSource
        );
    ...
    public void LC014_QueryableLambdaScope_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(analyzerDir, "AvoidStringCaseConversionAnalyzer.cs");
        var queryableLambdaScopePath = Path.Combine(
            analyzerDir,
            "AvoidStringCaseConversionQueryableLambdaScope.cs"
        );

        Assert.True(
            File.Exists(queryableLambdaScopePath),
            "LC014 queryable lambda scope and target method classification should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private ImmutableArray<IParameterSymbol> GetEnclosingQueryableLambdaParameters",
            analyzerSource
        );
        ...
        Assert.Contains(
            "private static ImmutableArray<IParameterSymbol> GetEnclosingQueryableLambdaParameters",
            queryableLambdaScopeSource
        );
    ...
    public void LC045_EntityLocalCollection_LivesInDedicatedPartial()
    {
        ...
        var usageAnalysisPath = Path.Combine(analyzerDir, "MissingIncludeUsageAnalysis.cs");
        var entityLocalCollectionPath = Path.Combine(
            analyzerDir,
            "MissingIncludeEntityLocalCollection.cs"
        );

        Assert.True(
            File.Exists(entityLocalCollectionPath),
            "LC045 materialized-result entity local collection should live in a focused partial file."
        );
    ...
    public void LC025_AnalyzerNoTrackingSourceTracing_LivesInDedicatedPartial()
    {
        ...
        var noTrackingSourcePath = Path.Combine(
            analyzerDir,
            "AsNoTrackingWithUpdateNoTrackingSource.cs"
        );

        Assert.True(
            File.Exists(noTrackingSourcePath),
            "LC025 analyzer no-tracking source evaluation should live in a focused partial file."
        );
    ...
    public void LC040_DiagnosticReporting_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(reportingPath),
            "LC040 compilation-end diagnostic grouping/reporting should live in a focused partial file."
        );
    ...
    public void LC043_BufferedLocalPattern_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(patternPath),
            "LC043 buffered-local syntax matching should live in a focused partial file."
        );
    ...
    public void LC043_FixerLoopDiscovery_LivesInDedicatedPartial()
    {
        ...
        var loopDiscoveryPath = Path.Combine(
            analyzerDir,
            "AsyncEnumerableBufferingFixerLoopDiscovery.cs"
        );

        Assert.True(
            File.Exists(loopDiscoveryPath),
            "LC043 fixer loop/declaration discovery should live in a focused partial file."
        );
    ...
    public void LC044_TrackedStateEdgeCases_LiveInDedicatedPartial()
    {
        ...
        var trackedStateTestsPath = Path.Combine(
            testDir,
            "AsNoTrackingThenModifyTrackedStateTests.cs"
        );

        Assert.True(
            File.Exists(trackedStateTestsPath),
            "LC044 attach/update/state-reset edge cases should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "AsNoTracking_Mutate_ContextUpdate_ThenSave_DoesNotTrigger",
            edgeCasesSource
        );
        Assert.DoesNotContain(
            "AsNoTracking_ContextAttach_ThenChangeTrackerClear_ThenMutate_ThenSave_StillTriggers",
            edgeCasesSource
        );
    ...
    public void LC009_SetGenericSourceTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(setSourceTestsPath),
            "LC009 DbContext.Set<T> query-source coverage should live in a focused partial test file."
        );

        var generalTestsSource = File.ReadAllText(generalTestsPath);
        Assert.Contains("public partial class MissingAsNoTrackingTests", generalTestsSource);
        Assert.DoesNotContain(
            "TestCrime_SetGenericSource_ReadOnly_TriggersDiagnostic",
            generalTestsSource
        );
        Assert.DoesNotContain(
            "TestInnocent_SetGenericSource_WithSelect_NoDiagnostic",
            generalTestsSource
        );
    ...
    public void LC009_ResultLocalResolution_LivesInDedicatedPartial()
    {
        ...
        var mutationDetectionPath = Path.Combine(
            analyzerDir,
            "MissingAsNoTrackingMutationDetection.cs"
        );
        var resultLocalPath = Path.Combine(analyzerDir, "MissingAsNoTrackingResultLocal.cs");

        Assert.True(
            File.Exists(resultLocalPath),
            "LC009 materialized-result local and wrapper resolution should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static ILocalSymbol? FindResultLocal",
            mutationDetectionSource
        );
        Assert.DoesNotContain(
            "private static IOperation? WalkUpThroughWrappers",
            mutationDetectionSource
        );
    ...
    public void LC007_IgnoredSourceTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(ignoredSourceTestsPath),
            "LC007 ignored and ambiguous query-source coverage should live in a focused partial test file."
        );
    ...
    public void LC045_EscapedResultEdgeCaseTests_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(escapedResultTestsPath),
            "LC045 escaped or ambiguous result edge cases should live in a focused partial test file."
        );
        ...
        Assert.DoesNotContain(
            "TestInnocent_IndexedEntityPassedAsArgument_NoDiagnostic",
            edgeCasesSource
        );
        ...
        Assert.Contains(
            "TestInnocent_ResultPassedAsArgument_NoDiagnostic",
            escapedResultTestsSource
        );
        Assert.Contains(
            "TestInnocent_IndexedEntityPassedAsArgument_NoDiagnostic",
    ...
    public void LC045_OriginAwareFlowTests_LiveInDedicatedPartial()
    {
        ...
        var uncertainReadTestsPath = Path.Combine(
            testDir,
            "MissingIncludeOriginFlowUncertainReadTests.cs"
        );

        Assert.True(
            File.Exists(originFlowTestsPath),
            "LC045 origin-aware control-flow coverage should live in a focused partial test file."
        );
        Assert.True(
            File.Exists(uncertainReadTestsPath),
            "LC045 uncertain-read control-flow guardrails should live in a focused partial test file."
        );
        ...
        Assert.Contains(
            "TestCrime_OriginFlow_ReadBeforeHelperEscape_StillReports",
            originFlowTestsSource
        );
        Assert.Contains(
            "TestCrime_OriginFlow_OneBranchNavigationAssignment_StillReports",
            originFlowTestsSource
        );
        Assert.Contains(
            "TestCrime_OriginFlow_WriteOnDifferentEntity_DoesNotSatisfyRead",
            originFlowTestsSource
        );
        Assert.Contains(
            "TestCrime_OriginFlow_LoopReadBeforeWrite_StillReports",
            originFlowTestsSource
        );
        Assert.DoesNotContain(
            "TestInnocent_OriginFlow_HelperEscapeBeforeRead_NoDiagnostic",
            originFlowTestsSource
        );
        ...
        Assert.Contains(
            "TestInnocent_OriginFlow_BothBranchesAssignNavigationBeforeRead_NoDiagnostic",
            uncertainReadTestsSource
        );
    }
    ...
    public void LC031_QuerySourceResolution_LivesInDedicatedPartial()
    {
        ...
        var querySourcePath = Path.Combine(
            analyzerDir,
            "UnboundedQueryMaterializationQuerySource.cs"
        );

        Assert.True(
            File.Exists(querySourcePath),
            "LC031 DbSet/query-source chain resolution should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static QuerySourceResolution ResolveQuerySource",
            analyzerSource
        );
    ...
    public void LC003_ExistenceComparisonRecognition_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            ...
            "LC003 Count comparison recognition should live in a focused partial file."
        );
    ...
    public void LC025_AnalyzerTrackingDirectiveAnalysis_LivesInDedicatedPartial()
    {
        ...
        var trackingDirectivePath = Path.Combine(
            analyzerDir,
            "AsNoTrackingWithUpdateTrackingDirectiveAnalysis.cs"
        );

        Assert.True(
            File.Exists(trackingDirectivePath),
            "LC025 analyzer tracking-directive chain analysis should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool IsProjectionToConstructedObject",
            analyzerSource
        );
    ...
    public void LC032_FixerAsyncSupportDetection_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(asyncSupportPath),
            "LC032 fixer ExecuteUpdateAsync capability detection should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool HasExecuteUpdateAsyncSupport",
            saveChangesModeSource
        );
        Assert.DoesNotContain(
            "private static bool HasExecuteUpdateAsyncMethod",
            saveChangesModeSource
        );
        Assert.DoesNotContain(
            "private static bool IsExecuteUpdateAsyncLikeMethod",
            saveChangesModeSource
        );
        Assert.DoesNotContain(
            "private static bool IsEntityFrameworkCoreNamespace",
            saveChangesModeSource
        );
    ...
    public void LC032_LocalWriteDetection_LivesInDedicatedPartial()
    {
        ...
        var statementAnalysisPath = Path.Combine(
            analyzerDir,
            "ExecuteUpdateForBulkUpdatesStatementAnalysis.cs"
        );
        var localWriteDetectionPath = Path.Combine(
            analyzerDir,
            "ExecuteUpdateForBulkUpdatesLocalWriteDetection.cs"
        );

        Assert.True(
            File.Exists(localWriteDetectionPath),
            "LC032 local write detection should live in a focused analyzer partial file."
        );
    ...
    public void LC011_AssemblyUsingScopeTraversal_LivesInDedicatedPartial()
    {
        ...
        var visibilityPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyUsingVisibility.cs"
        );
        var usingScopePath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyUsingScope.cs"
        );

        Assert.True(
            File.Exists(usingScopePath),
            "LC011 assembly using-scope traversal should live in a focused partial file."
        );
    ...
    public void LC011_AssemblyMemberResolution_LivesInDedicatedPartial()
    {
        ...
        var memberResolutionPath = Path.Combine(
            analyzerDir,
            "EntityMissingPrimaryKeyAssemblyMemberResolution.cs"
        );

        Assert.True(
            File.Exists(memberResolutionPath),
            "LC011 assembly member initializer resolution should live in a focused partial file."
        );
    ...
    public void LC016_FixerExpressionBodyClockAccessDiscovery_LivesInDedicatedPartial()
    {
        ...
        var expressionBodyPath = Path.Combine(fixerDir, "AvoidDateTimeNowFixerExpressionBody.cs");
        var clockAccessPath = Path.Combine(
            fixerDir,
            "AvoidDateTimeNowFixerExpressionBodyClockAccesses.cs"
        );

        Assert.True(
            File.Exists(clockAccessPath),
            "LC016 expression-bodied clock access discovery should live in a focused partial file."
        );
    ...
    public void LC007_FixerIncludeRewrite_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(includeRewritePath),
            "LC007 fixer Include rewrite construction should live in a focused partial file."
        );
    ...
    public void LC010_FreshContextExemption_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(freshContextPath),
            "LC010 fresh DbContext-in-loop exemption should live in a focused partial file."
        );
    ...
    public void LC033_ToHashSetInvocationClassification_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(toHashSetPath),
            "LC033 ToHashSet invocation classification should live in a focused partial file."
        );
    ...
    public void LC022_GroupingQueryableExemption_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(groupingPath),
            "LC022 grouping-queryable exemption should live in a focused partial file."
        );
    ...
    public void LC003_FixerZeroConstantDetection_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(zeroConstantPath),
            "LC003 fixer zero-constant detection should live in a focused partial file."
        );
    ...
    public void LC027_FixerPrimaryKeyDiscovery_LivesInDedicatedPartial()
    {
        ...
        var primaryKeyPath = Path.Combine(
            fixerDir,
            "MissingExplicitForeignKeyFixerPrimaryKeyDiscovery.cs"
        );

        Assert.True(
            File.Exists(primaryKeyPath),
            "LC027 fixer primary-key discovery should live in a focused partial file."
        );

        var fixerSource = File.ReadAllText(fixerPath);
        Assert.Contains("public sealed partial class MissingExplicitForeignKeyFixer", fixerSource);
        Assert.DoesNotContain(
            "private static IPropertySymbol? TryFindConventionPrimaryKey",
            fixerSource
        );
        Assert.DoesNotContain(
    ...
    public void LC016_QueryableScopeDetection_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(queryableScopePath),
            "LC016 queryable-lambda scope detection should live in a focused partial file."
        );

        var analyzerSource = File.ReadAllText(analyzerPath);
        Assert.Contains("public sealed partial class AvoidDateTimeNowAnalyzer", analyzerSource);
        Assert.DoesNotContain(
            "private static IAnonymousFunctionOperation? FindQueryableLambda",
            analyzerSource
        );
        Assert.DoesNotContain("private static bool IsTargetQueryableInvocation", analyzerSource);
        Assert.DoesNotContain(
            "private static IAnonymousFunctionOperation? FindEnclosingLambda",
            analyzerSource
        );
        Assert.DoesNotContain(
            "private static IEnumerable<IOperation> EnumerateOperations",
    ...
    public void LC027_TypeIndexConstruction_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(typeIndexPath),
            "LC027 type-index construction should live in a focused partial file."
        );
    ...
    public void LC001_QueryableScopeDetection_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(queryableScopePath),
            "LC001 queryable-scope detection should live in a focused partial file."
        );

        var analyzerSource = File.ReadAllText(analyzerPath);
        Assert.DoesNotContain(
            "private static bool IsTranslationCriticalQueryableInvocation",
            analyzerSource
        );
        Assert.DoesNotContain(
            "private static IArgumentOperation? GetInputSequenceArgument",
            analyzerSource
        );
        Assert.DoesNotContain(
    ...
    public void LC029_IdentitySelectorClassification_LivesInDedicatedPartial()
    {
        ...
        var identitySelectorPath = Path.Combine(
            analyzerDir,
            "RedundantIdentitySelectIdentitySelector.cs"
        );

        Assert.True(
            File.Exists(identitySelectorPath),
            "LC029 identity-selector classification should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static IAnonymousFunctionOperation? TryGetLambda",
            analyzerSource
        );
    ...
    public void LC012_FixerQuerySourceContextResolution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(querySourcePath),
            "LC012 fixer query-source context resolution should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool IsSingleSourceTransparentQueryMethod",
            rewriteSafetySource
        );
    ...
    public void LC036_LocalFunctionCallbackCapture_LivesInDedicatedPartial()
    {
        ...
        var localFunctionPath = Path.Combine(
            analyzerDir,
            "DbContextCapturedAcrossThreadsLocalFunctionCapture.cs"
        );

        Assert.True(
            File.Exists(localFunctionPath),
            "LC036 local-function callback capture should live in a focused partial file."
        );
    ...
    public void LC035_WhereChainTraversal_LivesInDedicatedPartial()
    {
        ...
        var whereChainPath = Path.Combine(
            analyzerDir,
            "MissingWhereBeforeExecuteDeleteUpdateWhereChain.cs"
        );

        Assert.True(
            File.Exists(whereChainPath),
            "LC035 Where-chain traversal should live in a focused partial file."
        );
    ...
    public void LC002_FixerRedundantMaterializationRewrite_LivesInDedicatedPartial()
    {
        ...
        var redundantRewritePath = Path.Combine(
            fixerDir,
            "PrematureMaterializationFixerRedundantMaterialization.cs"
        );

        Assert.True(
            File.Exists(redundantRewritePath),
            "LC002 redundant-materialization fixer rewrite should live in a focused partial file."
        );
    ...
    public void LC037_JumpSkippingFlow_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(jumpFlowPath),
            "LC037 jump/reference skipping flow should live in a focused partial file."
        );

        var terminationFlowSource = File.ReadAllText(terminationFlowPath);
        Assert.DoesNotContain(
            "private static bool HasPotentiallySkippingAncestor",
            terminationFlowSource
        );
    ...
    public void LocalAssignmentCache_RootScan_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(rootScanPath),
            "LocalAssignmentCache root scanning should live in a focused partial file."
        );
    ...
    public void LC002_ProviderSafeStringMethods_LiveInDedicatedPartial()
    {
        ...
        var providerSafePath = Path.Combine(
            analyzerDir,
            "PrematureMaterializationProviderSafeExpressions.cs"
        );
        var stringMethodsPath = Path.Combine(
            analyzerDir,
            "PrematureMaterializationProviderSafeStringMethods.cs"
        );

        Assert.True(
            File.Exists(stringMethodsPath),
            "LC002 provider-safe string method rules should live in a focused partial file."
        );

        var providerSafeSource = File.ReadAllText(providerSafePath);
        Assert.DoesNotContain(
            "private static bool IsAllowedProviderSafeStringMethod",
            providerSafeSource
        );
        Assert.DoesNotContain(
            "private static bool HasStringComparisonParameter",
            providerSafeSource
        );
    ...
    public void LC016_FixerStaticLambdaSafety_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(staticLambdaPath),
            "LC016 static-lambda fixer safety should live in a focused partial file."
        );
    ...
    public void LC044_ReachabilityBlockNavigation_LivesInDedicatedPartial()
    {
        ...
        var reachabilityPath = Path.Combine(analyzerDir, "AsNoTrackingThenModifyReachability.cs");
        var blockNavigationPath = Path.Combine(
            analyzerDir,
            "AsNoTrackingThenModifyReachabilityBlocks.cs"
        );

        Assert.True(
            File.Exists(blockNavigationPath),
            "LC044 reachability block navigation should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static IBlockOperation? FindEnclosingBlock",
            reachabilitySource
        );
    ...
    public void LC030_LongLivedTypeRecognition_LivesInDedicatedPartial()
    {
        ...
        var evidencePath = Path.Combine(analyzerDir, "DbContextInSingletonLongLivedEvidence.cs");
        var typeRecognitionPath = Path.Combine(
            analyzerDir,
            "DbContextInSingletonLongLivedTypeRecognition.cs"
        );

        Assert.True(
            File.Exists(typeRecognitionPath),
            "LC030 long-lived/scoped type recognition helpers should live in a focused partial file."
        );
    ...
    public void LC012_FixerSaveChangesBranchExclusion_LivesInDedicatedPartial()
    {
        ...
        var branchExclusionPath = Path.Combine(
            analyzerDir,
            "OptimizeRemoveRangeFixerSaveChangesBranchExclusion.cs"
        );

        Assert.True(
            File.Exists(branchExclusionPath),
            "LC012 fixer save-changes branch exclusion should live in a focused partial file."
        );
    ...
    public void LC037_StringBuilderIdentityReferenceTraversal_LivesInDedicatedPartial()
    {
        ...
        var referenceTraversalPath = Path.Combine(
            analyzerDir,
            "RawSqlStringConstructionStringBuilderIdentityReferences.cs"
        );

        Assert.True(
            File.Exists(referenceTraversalPath),
            "LC037 StringBuilder identity reference traversal should live in a focused partial file."
        );
    ...
    public void LC013_AssignedValueCollection_LivesInDedicatedPartial()
    {
        ...
        var originResolutionPath = Path.Combine(
            analyzerDir,
            "DisposedContextQueryAssignedOriginResolution.cs"
        );
        ...
        Assert.True(
            File.Exists(assignedValuesPath),
            "LC013 assigned-value collection should live in a focused partial file."
        );
    ...
    public void LC041_FixerInvocationRewrite_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(rewritePath),
            "LC041 fixer invocation rewrite should live in a focused partial file."
        );
    ...
    public void LC002_MaterializerClassification_LivesInDedicatedPartial()
    {
        ...
        var materializersPath = Path.Combine(
            analyzerDir,
            "PrematureMaterializationMaterializerRules.cs"
        );

        Assert.True(
            File.Exists(materializersPath),
            "LC002 materializer classification rules should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool IsDirectCollectionMaterializer",
            methodRulesSource
        );
    ...
    public void LC044_TrackingStateDetachInvalidation_LivesInDedicatedPartial()
    {
        ...
        var trackingStatePath = Path.Combine(analyzerDir, "AsNoTrackingThenModifyTrackingState.cs");
        var detachInvalidationPath = Path.Combine(
            analyzerDir,
            "AsNoTrackingThenModifyTrackingDetachInvalidation.cs"
        );

        Assert.True(
            File.Exists(detachInvalidationPath),
            "LC044 detach/tracker-clear invalidation should live in a focused partial file."
        );
    ...
    public void LC025_SourceResolutionConditionalOrigins_LiveInDedicatedPartial()
    {
        ...
        var conditionalOriginsPath = Path.Combine(
            analyzerDir,
            "AsNoTrackingWithUpdateSourceConditionalOrigins.cs"
        );

        Assert.True(
            File.Exists(conditionalOriginsPath),
            "LC025 conditional origin state should live in a focused partial file."
        );

        var sourceResolutionSource = File.ReadAllText(sourceResolutionPath);
        Assert.DoesNotContain("private readonly struct LocalOrigin", sourceResolutionSource);
        Assert.DoesNotContain(
            "private static bool IsConditionalRelativeTo",
            sourceResolutionSource
        );
    ...
    public void LC034_SqlSafetyDetection_LivesInDedicatedPartial()
    {
        ...
        var analyzerPath = Path.Combine(
            analyzerDir,
            "AvoidExecuteSqlRawWithInterpolationAnalyzer.cs"
        );
        var sqlSafetyPath = Path.Combine(
            analyzerDir,
            "AvoidExecuteSqlRawWithInterpolationSqlSafety.cs"
        );

        Assert.True(
            File.Exists(sqlSafetyPath),
            "LC034 SQL safety classification should live in a focused partial file."
        );
    ...
    public void LC005_FixerRewriteValidation_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            ...
            "LC005 fixer rewrite validation should live in a focused partial file."
        );
    ...
    public void LC008_FixerAwaitContextValidation_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(awaitContextPath),
            "LC008 fixer await-context validation should live in a focused partial file."
        );
    ...
    public void LC018_AnalyzerSqlSafetyDetection_LivesInDedicatedPartial()
    {
        ...
        var sqlSafetyPath = Path.Combine(
            analyzerDir,
            "AvoidFromSqlRawWithInterpolationAnalyzerSqlSafety.cs"
        );

        Assert.True(
            File.Exists(sqlSafetyPath),
            "LC018 analyzer SQL safety classification should live in a focused partial file."
        );
    ...
    public void LC042_QueryMethodLookupTables_LiveInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(methodSetsPath),
            "LC042 query method lookup tables should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static readonly ImmutableHashSet<string> QuerySteps",
            analyzerSource
        );
    ...
    public void LC028_ThenIncludeChainAnalysis_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(chainAnalysisPath),
            "LC028 ThenInclude chain analysis should live in a focused partial file."
        );
    ...
    public void SymbolAnalysis_PrimaryKeyDiscovery_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(primaryKeyPath),
            "Primary-key symbol discovery should live in a focused extension partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool IsDataAnnotationsKeyAttribute",
            symbolExtensionsSource
        );
    ...
    public void LC019_ConditionalIncludePathAnalysis_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(pathAnalysisPath),
            "LC019 conditional include path traversal should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "private static bool HasConditionalIncludeInvocationSource",
            analyzerSource
        );
    ...
    public void LC026_FixerExplicitTokenArgument_LivesInDedicatedPartial()
    {
        ...
        var explicitArgumentPath = Path.Combine(
            fixerDir,
            "MissingCancellationTokenFixerExplicitArgument.cs"
        );

        Assert.True(
            File.Exists(explicitArgumentPath),
            "LC026 fixer explicit-token argument lookup should live in a focused partial file."
        );
    ...
    public void LC032_ScalarTypeClassification_LivesInDedicatedPartial()
    {
        ...
        var assignmentAnalysisPath = Path.Combine(
            analyzerDir,
            "ExecuteUpdateForBulkUpdatesAssignmentAnalysis.cs"
        );
        var scalarTypesPath = Path.Combine(
            analyzerDir,
            "ExecuteUpdateForBulkUpdatesScalarTypes.cs"
        );

        Assert.True(
            File.Exists(scalarTypesPath),
            "LC032 scalar type classification should live in a focused partial file."
        );
    ...
    public void LC032_QueryStepLookupTables_LiveInDedicatedPartial()
    {
        ...
        var queryAnalysisPath = Path.Combine(
            analyzerDir,
            "ExecuteUpdateForBulkUpdatesQueryAnalysis.cs"
        );
        var queryStepsPath = Path.Combine(analyzerDir, "ExecuteUpdateForBulkUpdatesQuerySteps.cs");

        Assert.True(
            File.Exists(queryStepsPath),
            "LC032 allowed query/materializer step lookup tables should live in a focused partial file."
        );

        var queryAnalysisSource = File.ReadAllText(queryAnalysisPath);
        Assert.DoesNotContain(
            "private static readonly ImmutableHashSet<string> AllowedQuerySteps",
            queryAnalysisSource
        );
        Assert.DoesNotContain(
            "private static readonly ImmutableHashSet<string> MaterializerSteps",
            queryAnalysisSource
        );
    ...
    public void LC045_FixerQuerySourceResolution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(querySourcePath),
            "LC045 fixer query-source resolution should live in a focused partial file."
        );
        ...
        Assert.DoesNotContain(
            "semanticModel?.GetTypeInfo(querySource, cancellationToken)",
            fixerSource
        );
    ...
    public void LC006_ReceiverChainTraversal_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(receiverChainPath),
            "LC006 receiver-chain traversal should live in a focused partial file."
        );

        var chainAnalysisSource = File.ReadAllText(chainAnalysisPath);
        Assert.DoesNotContain(
            "private static ImmutableArray<IInvocationOperation> CollectReceiverChainInvocations",
            chainAnalysisSource
        );
        Assert.DoesNotContain(
            "private static bool HasRelevantQueryOperatorAncestor",
            chainAnalysisSource
        );
        Assert.DoesNotContain(
            "private static bool InvocationUsesReceiverChain",
            chainAnalysisSource
        );
    ...
    public void LC011_DbSetMemberExtraction_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(dbSetMembersPath),
            "LC011 DbSet member extraction should live in a focused partial file."
        );
    ...
    public void LC011_FixerEntityTypeResolution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(entityTypePath),
            "LC011 fixer entity-type and Id-member checks should live in a focused partial file."
        );
    ...
    public void LC034_FixerSqlArgumentSafety_LivesInDedicatedPartial()
    {
        ...
        var sqlArgumentPath = Path.Combine(
            fixerDir,
            "AvoidExecuteSqlRawWithInterpolationFixerSqlArgument.cs"
        );

        Assert.True(
            File.Exists(sqlArgumentPath),
            "LC034 fixer SQL argument and literal-safety helpers should live in a focused partial file."
        );
    ...
    public void LC006_FixerSyntaxChainSearch_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(syntaxChainPath),
            "LC006 fixer syntax-chain search helpers should live in a focused partial file."
        );

        var fixerSource = File.ReadAllText(fixerPath);
        Assert.Contains("public sealed partial class CartesianExplosionFixer", fixerSource);
        Assert.DoesNotContain(
            "private static InvocationExpressionSyntax? FindEffectiveAsSingleQueryInvocation",
            fixerSource
        );
        Assert.DoesNotContain(
            "private static InvocationExpressionSyntax? FindFirstIncludeInvocation",
            fixerSource
        );
    ...
    public void LC027_CollectionTypeClassification_LivesInDedicatedPartial()
    {
        ...
        var entityAnalysisPath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyEntityAnalysis.cs"
        );
        var collectionTypesPath = Path.Combine(
            analyzerDir,
            "MissingExplicitForeignKeyCollectionTypes.cs"
        );

        Assert.True(
            File.Exists(collectionTypesPath),
            "LC027 collection-type classification should live in a focused partial file."
        );
    ...
    public void LC009_MaterializerMethodClassification_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            File.Exists(materializersPath),
            "LC009 materializer method classification should live in a focused partial file."
        );
    ...
    {
        var verifierDir = Path.Combine(_repoRoot, "tools", "SampleDiagnosticsVerifier");
        ...
        Assert.True(
            ...
            "Sample diagnostics SARIF parsing should live in a focused Program partial file."
        );

        var programSource = File.ReadAllText(programPath);
        Assert.DoesNotContain(
            "static IReadOnlyList<SampleDiagnostic> ParseDiagnostics",
            programSource
        );
        ...
        Assert.Contains(
            "private static IReadOnlyList<SampleDiagnostic> ParseDiagnostics",
            sarifSource
        );
    ...
    public void SampleDiagnosticsVerifier_ManifestParsing_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            ...
            "Sample diagnostics manifest parsing should live in a focused Program partial file."
        );

        var programSource = File.ReadAllText(programPath);
        Assert.DoesNotContain(
            "static SampleExpectationGroup[] LoadExpectationGroups",
            programSource
        );
        ...
        Assert.Contains(
            "private static SampleExpectationGroup[] LoadExpectationGroups",
            manifestSource
        );
    ...
    public void SampleDiagnosticsVerifier_DotnetProcessExecution_LivesInDedicatedPartial()
    {
        ...
        Assert.True(
            ...
            "Sample diagnostics dotnet process execution should live in a focused Program partial file."
        );
