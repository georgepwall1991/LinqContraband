...
- EF Core's own analyzers (EF1002 since EF Core 8, EF1003 since EF Core 10) also look only at the SQL written directly in the call, so they do not overlap LC037: a SQL string built in a local, with `string.Format(...)`, `string.Concat(...)` or a `StringBuilder` is reported by LC037 alone. LC018 and LC034 defer to EF's analyzers on the direct shapes; see their pages.
...
- SQL text built inside a user-defined helper or method is out of scope: taint is tracked intraprocedurally through locals, aliases, and framework string APIs, but not across method boundaries in either direction. A constant returned from a helper stays quiet; a tainted value laundered through one is a known false negative.
