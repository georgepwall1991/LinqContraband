...
- Navigation writes are not reads: `o.Customer = c` (including compound, `??=`, and deconstruction assignments) and `o.Items.Add(x)` are recognized relationship-fix-up patterns and stay quiet. A navigation write satisfies a later read only for the same entity origin and only when every path reaching that read performs the write; a one-branch write or a write to a different extracted entity does not suppress the diagnostic. The write is credited to the origin it was made on, not to the collection, so a manual fix-up loop that writes the navigation on every element does **not** satisfy a read taken later through a different origin — a second `foreach`, an indexer, or an aggregate callback still reports. That is a known false positive for the manual relationship fix-up pattern; suppress it with `#pragma warning disable LC045` until collection-level write tracking lands.
