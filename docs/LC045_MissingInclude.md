...
- Exact top-level `AutoInclude()` chains in the nearest real `OnModelCreating` override are recognised, including constructed generic contexts. Exact unconditional `ApplyConfiguration(new TConfiguration())` calls are also recognised when the source-visible `IEntityTypeConfiguration<T>.Configure` implementation contains direct builder chains. `ApplyConfigurationsFromAssembly` over the current project's assembly is recognised the same way, without a predicate. Configuration instances supplied through locals, fields, factories, assembly scanning with a predicate or over another assembly, builder aliases, helper-delegated settings, inferred base-method calls, deferred calls, later unproven model mutations, and transitive auto-includes on a different entity remain unproven and can still report; keep an explicit `Include`, project, or use a reviewed suppression for those shapes.
...
- **A private method in the same file handed the *collection*, whose only use of it is iterating it, is treated the same way.** `Print(orders)` where `private void Print(List<Order> orders) { foreach ... }` is the closure form one call out. Handing the collection on, indexing it, or re-materializing it leaves the argument an escape.
- **A private method in the same file, handed the entity and only reading navigations on it, is treated the same way.** `private void Render(Order o) => ...o.Customer...;` is the shape real code uses for this, and a private method cannot be overridden, so the body found is the body that runs. Same file keeps it free: the semantic model already covers that tree. Anything callable from outside the type stays an escape.
- **A local function handed the entity, whose only use of it is reading navigations, is not an escape either.** `void Show(Order o) => ...o.Customer...;` called from a loop binds its parameter to the argument's origin, so the read is proven where the caller's own read would be. Any other use of the parameter — passing it on, assigning it, calling through it, and above all `db.Entry(o).Reference(...).Load()` — leaves the argument an escape, because a callee that loads before reading is correct code. Each call site is judged on its own: the parameter binds to that call's argument, so a helper shared between a loop over an included query and a loop over a bare one reports only for the bare one.
- **A local function that only reads the collection is not an escape.** `void Print() { foreach (var o in orders) ... }` followed by `Print();` is the same loop written one level down, so its reads are proven at the call — where the collection's state is known — and reported on the read itself. This requires exactly one call site, and the callee's only use of the collection must be iterating it: a callee that also hands the collection out, one invoked twice, one invoked after an escape, and a delegate variable all stay escapes. A callee that takes the *entity* rather than closing over the collection is analysed per call site instead: its parameter binds to that call's argument, so reads through it are proven exactly like the closure form, while passing the entity onward or explicitly loading the navigation stays an escape.
- Current scope is otherwise intra-procedural and local-based (methods and constructors). Out of scope (quiet, not flagged): `await foreach` over anything other than a proven EF stream, arbitrary callbacks and delegate/method-group consumers, the default-value element extractor overloads (their default can be an entity the query never produced), custom extraction lookalikes, provider-specific temporal APIs, `Find*`, and `IQueryable` parameters / repository-returned queries as roots.
