...
- **A local function handed the entity, whose only use of it is reading navigations, is not an escape either.** `void Show(Order o) => ...o.Customer...;` called from a loop binds its parameter to the argument's origin, so the read is proven where the caller's own read would be. Any other use of the parameter — passing it on, assigning it, calling through it, and above all `db.Entry(o).Reference(...).Load()` — leaves the argument an escape, because a callee that loads before reading is correct code. This also requires exactly one call site, since two callers may hand it different entities.
