...
- Interpolations outside likely SQL value positions, such as `FROM {tableName}`, `SELECT {columnName}`, `WHERE {columnName} = 1`, or `EXEC {procedureName}`, are not auto-fixed. `FromSqlInterpolated(...)` would parameterize the hole as a value, not a table, column, schema, procedure, or ordering fragment.
