...
description: A practical guide to LinqContraband's EF Core analyzer rules for query performance, loading, tracking, async execution, raw SQL safety, and CI policy.
...
LinqContraband provides {{ site.data.rules | size }} EF Core analyzer rules for teams that want repeatable query review in the IDE and CI. The
...
| --- | --- | --- |
| Query shape and translation | Local methods, unstable ordering, non-translatable overloads, sorts that `Distinct` discards, and query shapes that can fall out of SQL translation. | [EF Core client-side evaluation analyzer](/LinqContraband/ef-core-client-side-evaluation-analyzer/), [LC001: local method](/LinqContraband/LC001_LocalMethod.html), [LC050: OrderBy before Distinct](/LinqContraband/LC050_OrderByBeforeDistinct.html), [EF Core pagination OrderBy analyzer](/LinqContraband/ef-core-pagination-orderby-analyzer/) |
| Materialization and projection | Early `ToList`, whole-entity fetches, unbounded result sets, nested collection materializers, scalar reads that should project in SQL, and `Min`/`Max`/`Average` calls that throw when no row matches. | [EF Core projection analyzer](/LinqContraband/ef-core-projection-analyzer/), [LC017: whole entity projection](/LinqContraband/LC017_WholeEntityProjection.html), [LC057: Min, Max or Average on an empty query](/LinqContraband/LC057_EmptyQueryAggregate.html), [EF Core premature materialization analyzer](/LinqContraband/ef-core-premature-materialization-analyzer/) |
| Loading and includes | Missing includes, cartesian explosion, excessive eager loading, deep include chains, includes that a projection ignores, and untagged complex queries. | [LC045: missing include](/LinqContraband/LC045_MissingInclude.html), [LC049: Include ignored by a projection](/LinqContraband/LC049_IncludeIgnoredByProjection.html), [LC006: cartesian explosion](/LinqContraband/LC006_CartesianExplosion.html), [EF Core Include analyzer](/LinqContraband/ef-core-include-analyzer/) |
| Execution and async | Database work inside loops, synchronous or overlapping same-context EF Core calls, repeated saves, missing cancellation tokens, async-stream buffering, and `TransactionScope`s that do not flow across `await`. | [LC046: concurrent DbContext operations](/LinqContraband/LC046_ConcurrentDbContextOperations.html), [LC058: TransactionScope without async flow](/LinqContraband/LC058_TransactionScopeWithoutAsyncFlow.html), [EF Core async query analyzer](/LinqContraband/ef-core-async-query-analyzer/), [EF Core CancellationToken analyzer](/LinqContraband/ef-core-cancellation-token-analyzer/) |
| Tracking and context lifetime | Missing `AsNoTracking`, no-tracking writes, mixed tracking modes, repeated `SaveChanges`, lost updates without a concurrency token, DbContext lifetime mistakes, and disposing the connection the context owns. | [LC030: DbContext lifetime](/LinqContraband/LC030_DbContextInSingleton.html), [LC048: lost update risk](/LinqContraband/LC048_LostUpdateRisk.html), [LC059: disposed context connection](/LinqContraband/LC059_DisposedContextConnection.html), [LC036: context captured across threads](/LinqContraband/LC036_DbContextCapturedAcrossThreads.html), [EF Core DbContext lifetime analyzer](/LinqContraband/ef-core-dbcontext-lifetime-analyzer/) |
| Bulk operations and modeling | Set-based write opportunities, unbounded bulk updates or deletes, tracked-delete pipeline bypasses, missing keys, missing explicit foreign keys, seed data or column defaults that change on every run, migrations that EF Core 9 rejects inside a transaction, and `OnModelCreating` overrides that skip the base configuration. | [LC047: ExecuteDelete bypasses tracked delete](/LinqContraband/LC047_ExecuteDeleteBypassesTrackedDelete.html), [LC054: Migrate inside a transaction](/LinqContraband/LC054_MigrateInsideTransaction.html), [LC055: missing base.OnModelCreating](/LinqContraband/LC055_MissingBaseOnModelCreating.html), [LC032: ExecuteUpdate](/LinqContraband/LC032_ExecuteUpdateForBulkUpdates.html), [LC035: missing Where before bulk execute](/LinqContraband/LC035_MissingWhereBeforeExecuteDeleteUpdate.html), [EF Core ExecuteUpdate analyzer](/LinqContraband/ef-core-executeupdate-analyzer/) |
| Raw SQL and security | Interpolated raw SQL, constructed SQL strings, unsafe command SQL, LINQ composed over stored procedure calls, and query-filter bypasses. | [LC018: interpolated raw SQL](/LinqContraband/LC018_AvoidFromSqlRawWithInterpolation.html), [LC034: interpolated command SQL](/LinqContraband/LC034_AvoidExecuteSqlRawWithInterpolation.html), [LC056: LINQ over a stored procedure](/LinqContraband/LC056_StoredProcedureComposed.html), [LC021: IgnoreQueryFilters](/LinqContraband/LC021_AvoidIgnoreQueryFilters.html) |
