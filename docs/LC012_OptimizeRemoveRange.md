...
- Calls whose entity and context are covered by LC047 tracked-delete pipeline evidence: a source-visible `SaveChanges` / interceptor conversion of `EntityState.Deleted`, or Fluent `OnDelete(ClientCascade|ClientSetNull)` on that principal. `ExecuteDelete` would skip that pipeline. When the `RemoveRange` receiver does not resolve to a context (a `DbSet<T>` parameter), LC012 still stays quiet if any source `DbContext` in the compilation covers that entity. `HasQueryFilter` alone does not suppress LC012.
