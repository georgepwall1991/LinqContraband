...
- Fluent API keys in `OnModelCreating`: `modelBuilder.Entity<TEntity>().HasKey(...)`, `modelBuilder.Entity<TEntity>(b => b.HasKey(...))`, scoped local builder variables, and chained builder calls such as `entity.ToTable("Products").HasKey(...)`. Entity types declared in a referenced project resolve like the context's own.
- Configuration helpers: source methods that `OnModelCreating` passes its `ModelBuilder` or an `EntityTypeBuilder<TEntity>` to, such as a static `Product.OnModelCreating(modelBuilder)` or a `modelBuilder.ConfigureCatalog()` extension, followed up to four calls deep. A helper that is never called from `OnModelCreating` does not count.
- Builder methods passed as method groups: `modelBuilder.Entity<Package>(BuildPackageEntity)`, where `BuildPackageEntity(EntityTypeBuilder<Package> package)` calls `package.HasKey(...)`, as BaGet does. `this.Build...` and `SomeClass.Build...` method groups count the same way.
- C# 14 extension blocks: a helper declared as `extension(ModelBuilder modelBuilder) { public void ConfigureStore(...) { ... } }` or `extension(EntityTypeBuilder<Tag> tag) { ... }` is followed like a classic `this ModelBuilder` extension method, as in Duende IdentityServer's `ConfigurePersistedGrantContext`. The key shape does not matter: `HasKey(x => new { x.UserCode })` counts like any other `HasKey`.
