...
The inserted `Where`/`Select` are `System.Linq` operators. `FirstAsync`/`SingleAsync` bind through the EF Core using alone, so when `System.Linq` is not already in scope (no file, namespace or global using) the fixer adds `using System.Linq;`.

The fixer attaches to the terminal materializer in chains such as `users.Where(x => x.IsActive).First()` and inserts the projection after the existing query steps. It withholds the code action when the terminal predicate is not an inline lambda, such as `users.First(activePredicate)`, because leaving a hoisted `Expression<Func<TEntity, bool>>` or `Func<TEntity, bool>` on the scalar materializer would produce non-compiling code.
