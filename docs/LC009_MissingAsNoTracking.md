...
One query gets one report. In `db.Orders.ToList().Where(o => o.Total > 1).ToList()` or `(await db.Orders.ToListAsync()).Where(...).ToList()` the EF query runs at the inner `ToList()`/`ToListAsync()`, so that is where LC009 reports. The outer `ToList()` is LINQ to Objects over a list that is already loaded and is not reported again. `AsEnumerable()` is the other way round: it only switches to LINQ to Objects, and the query runs when a later call enumerates it. In `db.Orders.AsEnumerable().Where(o => o.Total > 1).ToList()` LC009 reports the outer `ToList()`, and reports `AsEnumerable()` itself only when no later materializer reports the query, as in a `foreach` over `db.Orders.AsEnumerable().Where(...)` or `db.Orders.AsEnumerable().Select(o => o.Total).ToList()`. The rule still follows the entities through that outer call: when its result is changed (for example in a `foreach` over it), the query is on a write path and stays quiet, and when it is handed on (or returned, where returned entities report), the report comes without a fix.
...
- A helper reshapes the query into something that is not an entity, such as AutoMapper's `ProjectTo<OrderDto>()` or an extension method returning `IQueryable<int>` of IDs. EF Core does not track DTOs or scalars. A helper that still returns entities keeps the report: the root entity, a type derived from it (`OfType<PriorityOrder>()`), a base class or interface it implements (`Cast<IAuditable>()`), a type the root entity has as a navigation, an entity the context exposes as a `DbSet`, groupings of those, and anonymous types with a member that may hold an entity. A Join or GroupJoin result selector that projects only scalars, such as `(c, p) => new { c.Id, PagesRead = p.Count() }`, loads no entity and stays quiet.
- The enclosing method returns `IQueryable<T>` (deferred execution — the caller owns the tracking decision).
- The method **returns the materialized entities** to its caller: `return db.Users.ToList();`, `=> db.Users.FirstOrDefaultAsync(...)`, a result local or a `foreach` variable returned or yielded, a LINQ-to-Objects copy that still holds them (`return users.Where(...).ToList();`), or any of these as an element of a returned tuple literal, nested tuples included (`return (verifyResult, credential, key);`). A tuple that carries only values read from the entity (`return (order.Status, order.Id);`) does not return it and still reports, and a tuple stored in a local before it is returned is not followed. The analyzer cannot see the caller, and in repository-style code the caller usually changes and saves what it gets (on Kavita, 7 of 8 sampled callers of reported repository getters did), so calling the method read-only is a guess. This applies only when the query runs on a context the method does not create: a context in a local (`using var db = new AppDbContext();`) dies with the method, so its returned entities still report. Returns from a lambda or local function are not returns to the caller and still report. Set `dotnet_code_quality.LC009.report_returned_entities = true` to report returned entities as before (see [Configuration](#configuration)).
...
### Configuration
Returned entities (see above) are quiet by default. To report them, for example in a codebase where callers only read what repository methods return, add this to `.editorconfig` or `.globalconfig`:
...
Any other value, or no value, keeps them quiet. Reports on returned entities never carry the fix, because a caller may still save them.
...
- returned or yielded (`return db.Users.ToList();`, `return user;`), where those report at all,
