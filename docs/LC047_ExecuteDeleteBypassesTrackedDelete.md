...
Untyped `Entries()` with no cast covers the whole context. `Entries<T>()` or a cast of `.Entity` to `T` / an interface covers that type and proven derived types or implementers. Same-type private helpers are followed to depth 4. Constructed generic contexts such as `AppDbContext<int>` match conversion stored on `AppDbContext<TTenant>`. A registered interceptor may be a derived type, or a base-typed local assigned from `new TInterceptor()`. DI registration is proven only from an inline options lambda; method-group helpers, lambdas that only call another helper, non-generic `AddDbContext(Type, ...)`, lookalike extension methods, and interceptors with no source body stay quiet.
