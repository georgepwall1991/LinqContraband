...
- Path-insensitive Deleted reads paired with conversion on a sibling `if` / `switch` arm, after `if (Deleted) continue`, in an uncalled local function, on a non-`EntityEntry` `State` property, on a `switch` whose value is not `entry.State`, or in an unproven exception/loop/goto shape. Full CFG modeling is a non-goal of this proof.
