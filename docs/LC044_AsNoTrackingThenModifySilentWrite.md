...
- A pre-mutation tracking operation that can throw into a handler which still reaches the mutation is not treated as safe; a terminating handler is safe because the untracked path cannot reach the write or save.
