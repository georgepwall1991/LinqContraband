...
A method applied to an aggregate's result, such as `Convert.ToBoolean(g.Min(c => Convert.ToInt32(c.ReadOnly)))` or `Math.Round(g.Average(c => c.Amount), 2)`, works on a scalar rather than the group's elements, so LC024 does not report it. Whether that outer method translates is LC001's concern.
