...
| Low — opportunistic hygiene | Candidate queue | LC016 expression-bodied fixer no-op is test-confirmed and fixed. All remaining source-reasoned findings stay in the candidate queue until independently test-confirmed or rejected. |
...
Current verification (2026-07-04, stacked LC023+LC027+LC017+LC035+LC016 fix branches after the 5.6.38 release):
...
- Full local net10.0 suite passes 1263 tests on the stacked LC023+LC027+LC017+LC035+LC016 fix branch; `RuleCatalogDocGenerator --check`, `dotnet build LinqContraband.sln --no-restore --configuration Release`, and `SampleDiagnosticsVerifier` across net8.0/net9.0/net10.0 all passed there.
