...
> Rows below are the **2026-07-04 current-state audit** — the 2026-06-04 rerun deltas, 2026-06-10 full re-verification, LC045 releases, 2026-07-02 adversarial fixes, 2026-07-03 LC037 hardening, and the 2026-07-04/05 LC002 `Last*`, LC021 named-filter, LC012 later-save/query-source, LC023 self-referential predicate, LC027 Fluent key-type, LC017 projection-member, LC035 filtered-path, LC016 expression-bodied, LC015 downstream-sort pagination, LC045 queryable-source, LC044 prior-reattach, LC007 deferred `AsEnumerable()` execution and nested-foreach source execution, LC036 Parallel/delegate callback capture, LC010 scoped-receiver fixer and fresh-context-per-iteration guard, LC025 identity-resolution no-tracking and multi-origin fixer safety, LC024 GroupBy result-selector, LC031 `ToLookup()` materializer, LC040 `DbContext.Set<TEntity>()` tracked-source, LC006 reference-prefixed sibling, LC018 SQL identifier-position fixer guards, LC001 nested-lambda/fixer-safety guards, and LC030 static DbContext storage detection are folded into the rows. Every rule was independently re-checked against current source, tests, docs, samples, and the candidate queue; subsequent hardening has raised the local net10.0 suite to **1316 tests**.
...
| LC010 | SaveChanges inside loop | Change Tracking & Context Lifetime | Warning | 4 | 4 | 4 | 4 | 4 | 5 | Low | Mature loop analyzer covering direct, `foreach`, `await foreach`, `do`, local-function-called-from-loop, retry-loop guardrails, duplicate diagnostics, and fresh per-iteration contexts that should stay quiet when the saved `DbContext` local is created inside the loop body. Aliases to shared contexts, contexts declared in `for` initializers, and pre-save reassignments still report. The conservative do-while-only fixer keeps FixAll/CRLF coverage and refusals for unsafe loop/control-flow shapes, nested loops, multiple saves, and non-terminal saves. LC010 has 36 focused tests plus 3 cross-analyzer checks. Keep Low priority. |
...
| LC030 | DbContext lifetime mismatch | Change Tracking & Context Lifetime | Info | 4 | 4 | 4 | 4 | 5 | 4 | Low | Strong manual-only lifetime rule: long-lived types proven via interface/base-class, middleware, hosted-service, singleton DI registration, direct singleton DbContext registration, and optional `long_lived_types` config. Static `DbContext` fields/properties on proven long-lived types are now reported while static storage on unproven types remains quiet. Current suite has 25 analyzer tests, no fixer, and an 88-line doc explaining why no single safe fix exists. Severity keeps it out of the urgent stack. |
...
| --- | --- | --- |
| LC009 | FP | `MaterializedEntityIsMutated` only recognises writes whose target property instance is the materialized entity local itself. A nested entity-member mutation such as `user.Profile.DisplayName = name` can still look read-only and receive an `AsNoTracking()` advisory even when a later helper or caller saves the write. |
| LC011 | FP | `ApplyConfigurationsFromAssembly` is only honoured for literal `typeof(X).Assembly` arguments from the current compilation. Mainstream current-assembly shapes such as `Assembly.GetExecutingAssembly()` or a local/member holding that assembly may miss visible `IEntityTypeConfiguration<TEntity>` key configuration and report LC011 incorrectly. |
| LC013 | FP | `TryGetQueryChainReceiver` treats deferred-typed extension methods as transparent query-chain links and recurses to their first argument. A custom extension that materializes before returning `IQueryable<T>` (for example `query.ToList().AsQueryable()`) may be treated as if it still executes against a disposed context. |
| LC017 | unsafe-fix | The original direct/null-conditional/indexed LC017 projection-member lead is fixed, but a narrower source-reasoned fixer risk remains: analyzer usage can recognise interface/base/conversion property access while the fixer collects only supported entity-local/null-conditional/indexed property syntax. A cast/interface access such as `((IHasName)e).Name` should likely be diagnostic-only. |
| LC030 | FP | `AnalyzeProperty` flags any property whose type derives from `DbContext` on a proven long-lived type, with no distinction between stored/auto-properties and computed factory/scope properties that return a fresh context per access. Confirm with a computed getter/factory scenario before changing behaviour. |
| LC039 | FP | `AreMutuallyExclusiveBranches` recognises `if`/`else`, switch sections, and try/catch, but not switch-expression arms. A shape such as `state switch { 1 => db.SaveChanges(), 2 => db.SaveChanges(), _ => 0 }` may report repeated saves even though only one arm executes. |
| LC041 | missing-fix | The analyzer already reports chained non-key materializers such as `users.Where(x => x.IsActive).First()` followed by one property read, but the fixer may select the inner receiver invocation from the diagnostic span and fail to attach a safe projection fix. Confirm whether this is still true with a chained materializer fixer regression before changing behaviour. |
| LC041 | unsafe-fix | The fixer can emit non-compiling code when the terminal materializer predicate is a hoisted `Expression<Func<TEntity, bool>>` or `Func<TEntity, bool>` instead of an inline lambda, because the predicate is kept on the projected scalar terminal (for example `users.Select(x => x.Name).First(active)`). Expected safe outcome: no fix unless the predicate can be moved before `Select`. |
...
| Low — opportunistic hygiene | Candidate queue | LC016 expression-bodied fixer no-op, LC015 downstream-sort suppression, LC045 widened-enumerable fixer safety, LC044 prior-reattach suppression, LC007 deferred `AsEnumerable()` execution detection, LC007 nested-foreach source execution detection, LC006 reference-prefixed sibling detection, LC018 SQL identifier-position fixer safety, LC036 Parallel/delegate callback capture detection, LC010 scoped-receiver fixer safety and fresh-context-per-iteration suppression, LC025 identity-resolution no-tracking detection and multi-origin fixer safety, LC024 GroupBy result-selector detection, LC031 `ToLookup()` materializer detection, LC040 `DbContext.Set<TEntity>()` tracked-source detection, LC001 nested-lambda/fixer-safety guards, and LC030 static DbContext storage detection are test-confirmed and fixed. All remaining source-reasoned findings stay in the candidate queue until independently test-confirmed or rejected; the broad LC017 projection-member lead is fixed, but a narrower cast/interface/base-conversion fixer-safety lead remains queued. |
...
| LC041 | Hoisted predicate handling | **Deferred.** Hoisted predicates remain unresolved in two places: key-predicate analyzer suppression may still miss them, and the fixer may need to withhold or restructure scalar projection fixes when the terminal predicate is not inline. The old null-guarded single-property read residual is fixed by the 2026-06-26 pass. |
...
Current verification (2026-07-05, stacked hardening branches through LC030 static DbContext storage detection after the 5.6.38 release):
...
- Focused red/green regression confirmed that LC010 offered a non-compiling move-after-loop fixer when the saved context was declared inside the `do` loop body, then passed after the fixer withheld moves whose receiver declaration would go out of scope. The LC010 net10.0 slice passes 36 tests.
...
- Full local net10.0 suite passes 1316 tests on the stacked LC030 static DbContext storage branch. `RuleCatalogDocGenerator --check`, `dotnet build LinqContraband.sln --no-restore --configuration Release`, and `SampleDiagnosticsVerifier` across net8.0/net9.0/net10.0 all pass on this branch.
