...
Reviewed: 2026-07-02 (multi-agent adversarial review — 45 parallel per-rule reviewers + skeptic verification + 7 worktree-isolated net10.0 test-confirmations; **7 defects test-confirmed**, see the 2026-07-02 section; prior: repo hygiene pass 2026-06-26, full per-rule re-verification 2026-06-10, overnight rerun 2026-06-04, deep rescan 2026-05-29)
...
| LC003 | Prefer Any() over Count() existence checks | Materialization & Projection | Warning | 3 | 4 | 2 | 4 | 4 | 3 | High | **2026-07-02: shipped unsafe fixer test-confirmed (FS 4→2, Priority Low→High).** The analyzer correctly fires on `Count() == <constant 0>` (its `IsZero` reads the folded `ConstantValue`), but the fixer's `!`-negation branch keys on a bare `0` literal token (`IsZeroLiteral`, `ValueText=="0"` — `AnyOverCountFixer.cs:148`), so `Count() == Empty` where `const int Empty = 0` rewrites to `Any()` instead of `!Any()` — a **silent logical inversion** (compiles; the empty-check becomes a non-empty-check). The earlier "robust on const-folded forms" note described the *analyzer*, not this fixer path — every `== 0` fixer test uses a bare `0` literal, missing the const-reference shape. Analyzer scalar-context coverage (boolean assignments, return expressions, async `LongCountAsync`→`AnyAsync`; 29 tests) is otherwise sound. |
...
| LC008 | Synchronous EF method in async context | Execution & Async | Warning | 4 | 4 | 2 | 4 | 4 | 4 | High | **2026-07-02: shipped unsafe fixer test-confirmed (FS 3→2, Priority Low→High).** The fixer replaces the sync invocation with `await <call>` in place but never parenthesizes when the invocation is the receiver of a further `.`/`[]`/`()` (`SyncBlockerFixer.cs:96-126`). `db.Users.ToList().Count` → `await db.Users.ToListAsync().Count`, which binds as `await (….Count)` → **CS1061 build break**; hits the very common `.ToList().Count`, `.ToList()[i]`, `.First().Name`, `.Single().Id`, `.ToArray().Length` shapes. The correct rewrite needs `(await db.Users.ToListAsync()).Count`. Analyzer detection, the expression-tree-lambda guard, and the static-local-function sync boundary (closed 2026-06-10) are unaffected; docs/sample coverage of the async counterpart families stays strong. |
...
| LC011 | Entity missing primary key | Schema & Modeling | Warning | 4 | 5 | 4 | 5 | 5 | 4 | Medium | Reference-quality across FP/T/DS: namespace-validated attributes, context-specific applied configurations, current-assembly config scanning, inferred owned types, scoped/chained Fluent API, duplicate-safe fixes, 46-test suite, 80-line doc. **2026-07-02: documented crash residual (Priority Low→Medium).** `TryResolveLocalBuilder`↔`TryResolveEntityTypeFromBuilderExpression` (and `TryResolveLocalConfiguration`) have no cycle/visited guard, so a self-referential builder local in `OnModelCreating` (e.g. `var entity = entity.HasKey("Id");`) recurses to a **StackOverflow that aborts the analysis host** during IDE-live analysis; the position guard never breaks because the declaration's `SpanStart` always precedes the self-reference. Test-confirmed on net10.0 (test-host abort). It requires *non-compiling* source, so it never bites a clean build — headline scores hold — but a depth/visited guard is warranted. Otherwise remaining risk is exotic dynamic model construction (not source-detectable). |
...
| LC014 | Avoid string case conversion in queries | Query Shape & Translation | Warning | 3 | 4 | 4 | 4 | 4 | 4 | Medium | Hardened around EF-backed proof, local aliases, and `Join`/`GroupJoin` key-selector ownership; the argument walk follows only content-carrying arguments; manual-only stance is correct. **2026-07-02: async-terminal FN test-confirmed (A 4→3, Priority Low→Medium).** `IsTargetQueryableMethod` hard-requires `ContainingType.Name=="Queryable"` in `System.Linq` (`AvoidStringCaseConversionAnalyzer.cs:122-127`), so EF's async predicate terminals on `EntityFrameworkQueryableExtensions` (`AnyAsync`/`AllAsync`/`CountAsync`/`FirstOrDefaultAsync`/`SingleAsync`/…) never fire: `db.Users.Any(u => u.Email.ToLower()==x)` warns but `db.Users.AnyAsync(u => u.Email.ToLower()==x)` is **silent**. A sync/async asymmetry on the dominant modern EF query style; sibling rules LC008/LC015 already special-case these async extensions. Test-confirmed on net10.0 (sync case flags, async case does not). |
...
| LC026 | Missing CancellationToken in async call | Execution & Async | Info | 3 | 3 | 2 | 4 | 4 | 3 | High | **2026-07-02: shipped unsafe fixer test-confirmed (FS 4→2, Priority Low→High).** The analyzer reports on the full outer invocation, so `diagnosticSpan.Start` points at the receiver; the fixer then does `FindToken(start).Parent.AncestorsAndSelf().OfType<InvocationExpressionSyntax>().FirstOrDefault()` (`MissingCancellationTokenFixer.cs:39`), which returns the **innermost** invocation. On the dominant chained shape `query.Where(...).ToListAsync()` (and `context.Set<T>().ToListAsync()`) it appends the token to `Where(...)` → `query.Where(u => u.Id > 0, cancellationToken).ToListAsync()` — a **build break** (no such `Where` overload) and LC026 still persists. Every existing fixer test uses a plain-identifier/member-access receiver, the only shapes where the innermost-invocation heuristic accidentally lands correctly. Token discovery (fields/properties, `ct` preference; 21 tests) is unaffected. Test-confirmed on net10.0. |
...
| LC032 | ExecuteUpdate for bulk scalar updates | Bulk Operations & Set-Based Writes | Info | 4 | 4 | 2 | 4 | 4 | 3 | High | Ships an async-aware fixer (loop+`SaveChanges` → `ExecuteUpdate`, warning comment, token preservation, duplicate-write collapse) with 38 tests (24 fixer) and a 113-line doc. **2026-07-02: shipped unsafe fixer test-confirmed (FS 4→2, Priority Low→High).** `AllowedQuerySteps` whitelists `Skip`/`Take`/`Distinct` and the fixer transplants the chain verbatim as the `ExecuteUpdate` receiver (`ExecuteUpdateForBulkUpdatesQueryAnalysis.cs:19`). A tracked `foreach` over `db.Users.…Take(100)` works today; the fix rewrites it to `db.Users.…Take(100).ExecuteUpdate(…)`, which EF cannot translate (LIMIT/OFFSET/DISTINCT in a set-based UPDATE) and **throws `InvalidOperationException` at runtime** — the auto-fix turns working code into a runtime exception. Same for `.Skip`/`.Distinct`. The prior "declines are documented and reasonable" note missed this operator gap. Test-confirmed on net10.0 (fix transplants `.Take(100)`). |
...
| LC037 | Constructed raw SQL strings | Raw SQL & Security | Warning | 3 | 4 | 4 | 4 | 4 | 5 | High | Strong manual security rule (3-file analyzer: concatenation, `string.Format`/`Concat`, `StringBuilder`, aliased-local resolution; 26 tests); 5.5.5 added `SqlQueryRaw<T>` as a construction sink with no LC018 double-report. **2026-07-02: compiler-killing crash test-confirmed (A 4→3, Priority Low→High).** A self-referential reassignment `sql = sql + id;` before `ExecuteSqlRaw(sql)` drives `TryResolveLocalValue`→`IsConstructedRawSql`→`IsNonConstant`→`IsConcatWithNonConstant` into unbounded recursion → **StackOverflow that aborts the analysis host** (uncatchable by Roslyn's AD0001 handler). Root cause: the write-position guard at `RawSqlStringConstructionLocalResolution.cs:44` uses `assignment.Syntax.SpanStart` (the LHS `sql`, which precedes the RHS reference), so it fails to exclude the assignment that *contains* the reference, and there is no visited-set/depth guard. The repro is **valid, compiling, common code**. Same crash class as the LC005 (5.5.x) and LC045 (5.6.1 hot-fix) StackOverflows — needs a visited-set guard urgently. Test-confirmed on net10.0 (test-host abort). |
...
| --- | --- | --- | --- | --- |
| LC037 | **crash** (High) | `sql = sql + id;` before `ExecuteSqlRaw(sql)` → unbounded recursion → **StackOverflow aborts the analysis host** on valid compiling code (no visited/depth guard; write-position guard uses the LHS span). | `RawSqlStringConstructionLocalResolution.cs:44` | A 4→3, Pri→High |
| LC003 | **unsafe-fix** (High) | `Count() == Empty` (`const int Empty=0`) → `Any()` instead of `!Any()` — **silent logical inversion** (fixer negates only on a bare `0` literal token). | `AnyOverCountFixer.cs:148` | FS 4→2, Pri→High |
| LC008 | **unsafe-fix** (High) | `db.Users.ToList().Count` → `await db.Users.ToListAsync().Count` (no parens) → binds as `await (….Count)` → **CS1061 build break**. | `SyncBlockerFixer.cs:96-126` | FS 3→2, Pri→High |
| LC026 | **unsafe-fix** (High) | `query.Where(...).ToListAsync()` → token appended to the innermost `Where(...)` not the diagnosed `ToListAsync()` → **build break**, diagnostic persists. | `MissingCancellationTokenFixer.cs:39` | FS 4→2, Pri→High |
| LC032 | **unsafe-fix** (High) | fixer transplants a `Skip`/`Take`/`Distinct` source chain verbatim into the `ExecuteUpdate` receiver → EF **throws `InvalidOperationException` at runtime** (working code → runtime exception). | `ExecuteUpdateForBulkUpdatesQueryAnalysis.cs:19` | FS 4→2, Pri→High |
| LC014 | **FN** (High) | EF async predicate terminals (`AnyAsync`/`CountAsync`/`FirstOrDefaultAsync`/…) on `EntityFrameworkQueryableExtensions` never fire; the sync equivalents do — sync/async asymmetry on the dominant EF style. | `AvoidStringCaseConversionAnalyzer.cs:122-127` | A 4→3, Pri→Medium |
| LC011 | **crash** (low) | self-referential builder local in `OnModelCreating` (`var entity = entity.HasKey("Id");`) → infinite `TryResolveLocalBuilder`↔`TryResolveEntityTypeFromBuilderExpression` recursion → **StackOverflow** (IDE-live on non-compiling source only). | `EntityMissingPrimaryKeyConfigurationScan.cs` | Pri→Medium (scores hold) |

### Metadata-drift corrections

- **Doc-category mismatches (genuine doc bugs).** `docs/LC005_MultipleOrderBy.md` states `Category: Correctness` but the shipped `DiagnosticDescriptor` category is `Performance`; `docs/LC036_DbContextCapturedAcrossThreads.md` states `Category: Reliability` but the descriptor is `Safety`. The doc text should match the descriptor.
- **Test-count drift.** LC017 = **37** methods (row said 38); LC023 = **24** (row said 26, and the 2026-06-10 rerun table says 16 — both are wrong); LC025 = **29** (row said 31); LC044 = **35** (row and 2026-06-13 pass said 30 — the suite *grew*, an undercount). LC003/LC005/LC011/LC030/LC032/LC035/LC036 test counts verified correct.
- **Doc line-count drift is uniform +5.** Every per-rule doc line count cited in the scorecard is stale by exactly five lines (e.g. LC006 94→99, LC007 82→87, LC011 75→80, LC015 112→117, LC017 171→176, LC030 81→86, LC032 108→113, LC036 41→46, LC044 76→81) — a systematic shift from a shared trailer added in the 2026-06-26 metadata pass. Benign (boilerplate, not substance), so the DS anchors hold; the LC011/LC032 numbers are corrected inline above where their rows were already being edited.
...
| LC037 | FN | A StringBuilder built with separate statement-based Append calls (rather than a fluent chain) is not detected, producing 0 diagnostics where 1 is expected… |
...
| Priority | Rules | Work |
| --- | --- | --- |
| High — **2026-07-02 test-confirmed**, fix in order | **LC037**, LC003, LC008, LC026, LC032 | **LC037**: add a visited-set/depth guard to the raw-SQL local resolver — a `sql = sql + id;` self-reference StackOverflows the compiler/IDE on valid code (top of the queue; same class as the LC005/LC045 crashes). **LC003**: negate on any constant-zero, not just a bare `0` literal (currently inverts the check). **LC008**: parenthesize `(await …)` when the awaited call is the receiver of a further `.`/`[]`/`()` (currently CS1061). **LC026**: target the *diagnosed* invocation, not the innermost, when inserting the token (currently build-break on `.Where(...).ToListAsync()`). **LC032**: decline the fix (or bail) when the source chain contains `Skip`/`Take`/`Distinct` (currently rewrites to a runtime-throwing `ExecuteUpdate`). All five have a red repro test above. |
| Medium — **2026-07-02 test-confirmed** | LC011, LC014 | **LC011**: add a cycle/visited guard to the builder-local resolver (StackOverflow on a self-referential builder local — IDE-live only, needs non-compiling source). **LC014**: extend `IsTargetQueryableMethod` to EF's `EntityFrameworkQueryableExtensions` async predicate terminals (`AnyAsync`/`FirstOrDefaultAsync`/`CountAsync`/…), closing the sync/async asymmetry — mirror the LC008/LC015 async handling. |
| Low — opportunistic, Tier-1-importance hygiene first | None | LC045 parenthesized conditional path reporting, LC018 `SqlQueryRaw<T>` fixer residual, LC005 local sort reset, LC027 relationship-builder local false positive, LC043 nested-capture fixer guard, LC022 modern EF guidance, LC041 null-conditional scalar-read FN, LC029 identity-boundary guidance, LC031 docs/test depth, LC040 docs/test depth, LC016 docs/sample depth, LC008 docs/sample depth, LC002 docs/sample depth, LC004 docs depth, LC039 doc expansion, LC028 test depth, LC019 docs/test depth, LC038 docs/test depth, LC035 docs/test depth, LC026 docs/test depth, LC003 docs/test depth, and LC021 suppression/test depth are addressed. **2026-07-02 candidate queue**: ~35 additional source-reasoned FP/FN/unsafe-fix findings surfaced by the multi-agent review but **not yet independently test-confirmed** are tabulated in the 2026-07-02 section — treat them as leads for the next hardening pass, not verified defects, and test-confirm each before promoting. Two doc-category mismatches (LC005, LC036) and per-rule test/line-count drift are also corrected there. |
