...
| LC010 | SaveChanges inside loop | Change Tracking & Context Lifetime | Warning | 4 | 4 | 4 | 4 | 4 | 5 | Low | Mature loop analyzer covering direct, `foreach`, `await foreach`, `do`, local-function-called-from-loop, retry-loop guardrails, and duplicate diagnostics; conservative do-while-only fixer with FixAll/CRLF coverage and refusals for unsafe loop/control-flow shapes, nested loops, multiple saves, non-terminal saves, and scoped receivers that would go out of scope after a move. There are 31 dedicated LC010 tests plus 3 cross-analyzer checks. Keep Low priority; the fresh DbContext-per-iteration FP lead remains unconfirmed. |
...
Current verification (2026-07-05, stacked LC023+LC027+LC017+LC035+LC016+LC015+LC045+LC044+LC007+LC036+LC010+LC025+LC024+LC031 fix branches after the 5.6.38 release):
...
- Focused red/green regression confirmed that LC010 offered a non-compiling move-after-loop fixer when the saved context was declared inside the `do` loop body, then passed after the fixer withheld moves whose receiver declaration would go out of scope. The LC010 net10.0 slice passes 32 tests.
- Focused red/green regression confirmed that LC025 missed entities materialized from `AsNoTrackingWithIdentityResolution()` before `Update(...)`, then passed after the analyzer and fixer treated that EF Core method as a no-tracking directive alongside `AsNoTracking()`. The LC025 net10.0 slice passes 31 tests.
...
- Full local net10.0 suite passes 1295 tests on the stacked LC023+LC027+LC017+LC035+LC016+LC015+LC045+LC044+LC007+LC036+LC010+LC025+LC024+LC031 fix branch; `RuleCatalogDocGenerator --check`, `dotnet build LinqContraband.sln --no-restore --configuration Release`, and `SampleDiagnosticsVerifier` across net8.0/net9.0/net10.0 all pass on the LC031 branch.
