...
| Medium — current-state leads | LC017, LC035 | Confirm each with a red test before repair: LC017 analyzer/fixer projection-member mismatch; LC035 definitely assigned/all-filtered conditional receiver false positives. |
...
- Full local net10.0 suite passes 1238 tests on the stacked LC023+LC027 fix branch; `RuleCatalogDocGenerator --check`, `dotnet build LinqContraband.sln --no-restore --configuration Release`, and `SampleDiagnosticsVerifier` across net8.0/net9.0/net10.0 all passed there.
