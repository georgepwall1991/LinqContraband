...
Reviewed: 2026-07-04 (45-rule current-state audit — one read-only reviewer per rule, folded into scorecard, shortlist, and candidate queue; prior: 2026-07-03 LC037 StringBuilder statement-flow fix; 2026-07-02 multi-agent adversarial review with **7 defects test-confirmed**; repo hygiene pass 2026-06-26, full per-rule re-verification 2026-06-10, overnight rerun 2026-06-04, deep rescan 2026-05-29)
...
| LC002 | Premature query continuation after materialization | Materialization & Projection | Warning | 4 | 4 | 2 | 3 | 4 | 4 | High | Strong analyzer with one live fixer-safety gap. LC002 has 45 tests, including 14 fixer tests, and the set/keyed/grouped materializer fixes remain covered. However, the move-before-materialization fixer is still offered for terminal `Last`/`LastOrDefault` after inline `ToList`/`ToArray`/`AsEnumerable`; moving those onto `IQueryable` can change runtime behaviour, especially unordered EF queries. Keep A/FP/DS/Imp at 4, but demote FS/T and mark High until `Last*` fixes are suppressed or guarded with regression coverage. |
...
| LC006 | Multiple collection Includes | Loading & Includes | Warning | 4 | 4 | 3 | 4 | 4 | 4 | Low | High-impact sibling-collection rule; `LocalAssignmentCache` follows single-assignment locals, covering `AsSplitQuery()` across locals and sibling `Include`s split across locals. Include-path parsing uses shared `IncludePathParser`. The 99-line doc covers split-query trade-offs and LC028/LC038 boundaries. FS stays 3: `AsSplitQuery()` changes query behaviour and must be user-owned. Current coverage is 23 tests, including 6 fixer tests. Multi-reassignment chains remain an intentional non-goal. |
| LC007 | Database execution inside loop | Execution & Async | Warning | 4 | 4 | 4 | 4 | 5 | 5 | Low | Strong analyzer for proven EF execution inside loops: `Find`, explicit loads, materializers, aggregates, navigation `Query()`, `DbContext.Set<T>()`, single-assignment query locals, and EF set-based executors. Tests cover 22 LC007 cases: 17 analyzer and 5 fixer. The fixer remains deliberately narrow, rewriting only unconditional strongly typed explicit-loading in `foreach`/`await foreach` loops to `Include` and removing the per-item load. Docs/sample/catalog are current; the doc is now 87 lines. |
...
| LC010 | SaveChanges inside loop | Change Tracking & Context Lifetime | Warning | 4 | 4 | 4 | 4 | 4 | 5 | Low | Mature loop analyzer covering direct, `foreach`, `await foreach`, `do`, local-function-called-from-loop, retry-loop guardrails, and duplicate diagnostics; conservative do-while-only fixer with FixAll/CRLF coverage and refusals for unsafe loop/control-flow shapes. There are 30 dedicated LC010 tests plus 3 cross-analyzer checks. Keep Low priority, but source-reasoned candidate leads for fresh DbContext-per-iteration FPs and fixer relocation/data-flow risks need red-test confirmation before rescoring. |
...
| LC012 | Use ExecuteDelete instead of RemoveRange | Bulk Operations & Set-Based Writes | Warning | 4 | 4 | 3 | 3 | 4 | 4 | Medium | Conservative analyzer remains healthy, including EF `ExecuteDelete` availability, query-shaped single-argument gates, and same-context/reachability `SaveChanges` suppression. However, the fixer is not at analyzer parity: it still withholds fixes for any later `SaveChanges` and directly rewrites the query argument to `ExecuteDelete` without proving that the query source matches the `RemoveRange` receiver context. Keep Medium until fixer parity and cross-context-source safety are regression-tested; 30 focused tests pass today (21 analyzer, 9 fixer). |
...
| LC015 | Missing OrderBy before pagination | Query Shape & Translation | Warning | 4 | 4 | 4 | 4 | 4 | 5 | Low | High-value reliability rule with strong EF-backed source/key detection for unordered `Skip`/`Take`/`Chunk`/`ElementAt*`/`Last*` and misplaced `OrderBy` after pagination. Fixer registers only when a single source-detectable key is available, and declines no-key, `[Keyless]`, multi-`[Key]`, and EF Core class-level `[PrimaryKey(...)]` composite entities. Remaining documented residual: Fluent-only composite keys can still receive a partial-key fix because the source model is invisible. Coverage: 29 LC015 tests, including 11 fixer tests. |
| LC016 | Avoid DateTime.Now/UtcNow in queries | Query Shape & Translation | Warning | 4 | 4 | 3 | 3 | 4 | 2 | Low | Clock detection across `DateTime`/`DateTimeOffset` `Now`/`UtcNow`, `IQueryable` lambdas, per-lambda dedup, and unique-name statement-bodied extraction are covered (20 LC016 tests, 5 fixer/FixAll). Docs/README/sample frame deterministic clock boundaries, injected-clock testability, `UtcNow` preference, provider server-clock alternatives, and the local-extraction contract. Expression-bodied members can get a diagnostic/code action but the fixer currently no-ops because it requires an enclosing statement, so Tests drops to 3 until that edge has a regression or fix. |
| LC017 | Whole entity projection | Materialization & Projection | Info | 4 | 4 | 2 | 4 | 5 | 3 | Medium | Strong 37-test heuristic with a 176-line doc. Analyzer remains A/FP 4: it conservatively flags large EF materializations with 1-2 locally observed property uses and skips escaping usage. FS is weaker than prior rows implied: the fixer builds anonymous-type projections from a narrower foreach/direct-member scan than the analyzer's usage analysis, so mixed supported access shapes can leave post-fix code referencing omitted properties. Add a red fixer test before promoting this from candidate to confirmed; a DTO-helper or shared analyzer/fixer usage collector is the obvious upgrade. |
...
| LC021 | IgnoreQueryFilters usage | Raw SQL & Security | Warning | 4 | 4 | 3 | 3 | 3 | 4 | Medium | LC021 remains a useful EF-only security review rule, with 18 net10.0 tests covering parameterless extension/static diagnostics, suppressions, generated-code exclusion, and the parameterless remover fixer. However, the named-filter `IgnoreQueryFilters(filterKeys)` overload is not covered: the analyzer reports it, but fixer argument handling is unsafe for extension syntax and docs do not describe the named-filter boundary. Keep Medium until named-overload analyzer/fixer tests and docs are added. |
...
| LC023 | Prefer Find/FindAsync for primary key lookups | Materialization & Projection | Info | 4 | 4 | 3 | 3 | 4 | 3 | Medium | Careful key/async detection with shipped query-filter gating: visible `HasQueryFilter` entities are suppressed, including configuration-class, base-type, inherited-key, and non-generic `Entity(typeof(X))` forms. 24 LC023 tests pass, including 5 fixer/FixAll cases. Remaining follow-up: confirm and fix the self-referential predicate unsafe fixer lead, where `x.Id == x.Other` can lift `x.Other` outside the lambda into `Find(x.Other)`. Until covered, FS/T stay 3 and priority Medium. |
...
| LC027 | Missing explicit foreign key property | Schema & Modeling | Info | 4 | 4 | 3 | 3 | 4 | 3 | Medium | Strong explicit-FK analyzer with good relationship-configuration coverage, including direct Fluent chains, configuration classes, string shadow FKs, single-assignment relationship-builder locals, split `WithOne`/`WithMany` continuations, shadowing, and reassignment ambiguity. Coverage is 27 tests, including 4 fixer/fix-all paths. Fix strategy drops to 3: the fixer infers key type only from `[Key]`, `Id`, or `{Type}Id`, so fluent-only non-conventional primary keys can receive the wrong FK type. Add a red fixer test before re-promoting. |
...
| LC030 | DbContext lifetime mismatch | Change Tracking & Context Lifetime | Info | 4 | 4 | 4 | 4 | 5 | 4 | Low | Strong manual-only lifetime rule: long-lived types proven via interface/base-class, middleware, hosted-service, singleton DI registration, direct singleton DbContext registration, and optional `long_lived_types` config. Current suite has 22 analyzer tests, no fixer, and an 86-line doc explaining why no single safe fix exists. Severity keeps it out of the urgent stack. |
...
| LC035 | Missing Where before bulk execute | Bulk Operations & Set-Based Writes | Info | 4 | 3 | 4 | 3 | 3 | 4 | Medium | High-impact safety smell with no fixer by design. There are 22 analyzer tests. The 5.5.9 and 5.6.14 reassignment fixes are still locked, but current source-reasoned FP leads remain for definitely assigned locals without an unconditional base assignment and all-filtered ternary/switch-expression receivers. Demote FP/T/DS until those shapes are test-confirmed and fixed or explicitly rejected. |
...
| LC039 | Repeated SaveChanges on same context | Change Tracking & Context Lifetime | Info | 4 | 4 | 4 | 4 | 4 | 4 | Low | Useful reliability smell guarded for transaction boundaries (`using`/`await using` declarations included), mutually exclusive if/else branches, switch sections, and try-vs-catch saves; 21 analyzer tests, no fixer. Docs cover batching guidance, explicit transaction boundaries, branch/try/finally behaviour, separate contexts, executable-root scoping, EF-only boundary recognition, and manual-only rationale. Remaining analyzer gaps: exception-handler and nested-loop control flow. |
| LC040 | Mixed tracking and no-tracking modes | Change Tracking & Context Lifetime | Info | 3 | 4 | 4 | 4 | 4 | 3 | Low | Branch resolution covers if/else, switch, and ternary arms, with later shared materialization compared against reachable earlier modes; ordinary nested block cases are covered. Try/catch path modelling and conditional/loop/try local reassignment remain deliberately conservative because tracking may depend on whether a throw or reassignment actually occurred. Coverage locks EF-only tracking markers, transparent EF options, transactions, aliases, reassignments, and separate contexts; LC040 has 18 analyzer tests and remains manual-only. |
...
| LC044 | AsNoTracking entity mutated then SaveChanges | Change Tracking & Context Lifetime | Warning | 4 | 4 | 4 | 4 | 5 | 5 | Low | Reference-quality manual rule with same-context chain proof, single-assignment and reachability gates, last-tracking-directive handling, compound/increment mutations, earlier-save and after-mutation re-attach suppression. Manual-only remains correct: removing `AsNoTracking` vs explicit attach/update is intent-dependent. 35 analyzer tests cover direct/async materializers, foreach, same/different context, re-attach after mutation, ambiguous reassignments, and nested-block reachability. Candidate only: re-attach before the mutation is still source-reasoned but unverified; confirm with a red test before rescoring. |
...
| LC012 | **FS 4→3, T 4→3, Priority Low→Medium** | Analyzer parity is strong, but fixer parity is not: later `SaveChanges` still suppresses fixes too broadly, and cross-context query-source safety is not proven. |
| LC016 | **T 4→3** | Expression-bodied members can receive a code action that no-ops because the fixer requires an enclosing statement. |
...
| LC021 | **T 4→3, DS 4→3, Priority Low→Medium** | Named-filter `IgnoreQueryFilters(filterKeys)` is not covered in tests or docs, and the current fixer argument selection is unsafe for extension syntax with arguments. |
...
| Priority | Rules | Work |
| --- | --- | --- |
| High — current-state lead | LC002 | Test-confirm the terminal `Last`/`LastOrDefault` after inline materializer fixer hazard, then either suppress the code action for `Last*` or add a semantics-preserving guard with regression coverage. |
| Medium — current-state leads | LC012, LC017, LC021, LC023, LC027, LC035 | Confirm each with a red test before repair: LC012 fixer parity/cross-context query-source safety; LC017 analyzer/fixer projection-member mismatch; LC021 named-filter overload docs/tests/fixer safety; LC023 self-referential predicate fixer; LC027 fluent-only non-conventional key type inference; LC035 definitely assigned/all-filtered conditional receiver false positives. |
| Low — opportunistic hygiene | LC016 and candidate queue | LC016 expression-bodied fixer no-op is low-impact and should be regression-tested when touching clock fixes. All remaining source-reasoned findings stay in the candidate queue until independently test-confirmed or rejected. |

Rejected/deferred-by-design items (LC004 nested-local-function, LC036 method-group, LC040 try/catch + `Select`, LC044 untaken-branch re-attach, LC020 Ordinal flagging, LC015 `TakeLast`/`SkipLast`, LC031 `TakeLast`, and the old LC021 claim that selective filter keys are harmless false positives) stay closed — do not re-chase without new evidence. The current LC021 Medium item is different: named-overload fixer/docs/test coverage. See the 2026-06-04 Rerun tables below for full rationale.
