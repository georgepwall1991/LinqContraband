...
| LC008 | Synchronous EF method in async context | Execution & Async | Warning | 4 | 4 | 3 | 4 | 3 | 4 | Low | Solid sync/async pair detection with an expression-tree-lambda guard; fixer is limited to simple 1:1 rewrites (`ToList`→`ToListAsync`) and the doc has no EF API inventory or "no async equivalent" guidance (DS stays `3`). **The deferred orphaned-warning item is closed (2026-06-10)**: a `static` local function is now a hard sync boundary in the async-context walk — it cannot capture the async context and `await` is illegal inside it, so the unfixable warning no longer fires; capturing local functions/lambdas and `async static` local functions still flag. |
...
| Low — opportunistic, Tier-1-importance hygiene first | None | LC002 docs/sample depth, LC004 docs depth, LC039 doc expansion, LC028 test depth, LC019 docs/test depth, LC038 docs/test depth, LC035 docs/test depth, LC026 docs/test depth, LC003 docs/test depth, and LC021 suppression/test depth are addressed. Everything else is currently acceptable, explicitly deferred, or appropriately harsh-scored. |
