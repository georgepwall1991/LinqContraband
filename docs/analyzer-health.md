...
| LC044 | AsNoTracking entity mutated then SaveChanges | Change Tracking & Context Lifetime | Warning | 4 | 4 | 4 | 4 | 5 | 5 | Low | Reference-quality manual rule with same-context chain proof, single-assignment and reachability gates, last-tracking-directive handling, direct and nested property/field receiver detection with source-visible `[NotMapped]` exclusion, compound/increment mutations, and per-mutation member-path-aware re-attach/detach/earlier-save proof. Root or matching owning/nested re-attach paths suppress only their covered mutations when guaranteed on the mutation-to-save path; complementary `if`/`else` and normal/catch paths can establish that proof collectively, while optional, sibling, branch-transfer, caught-exception, and later unsafe paths do not, including inside foreach loops. Manual-only remains correct: removing `AsNoTracking` vs explicit attach/update is intent-dependent. 84 analyzer tests cover direct/async materializers, nested property and field member writes, explicit non-mapped graph state, foreach, same/different context, root/nested/sibling re-attach paths before/after mutation, safe-first/unsafe-sibling ordering, optional, same-branch, complementary-branch, optional-nested complementary, caught-throw-skipped complementary, fully covered try/catch, caught-throw-skipped catch reattach, harmless finally, and finally-clear invalidation control flow, precise break/continue/goto targets, caught compatible exceptions plus post-try, constant-false-filter, non-matching, exclusive, return, and goto-past-save safety, unreachable earlier saves, conditional and guarded pre-attach including loop guards, stale attach after explicit detach/tracker clear before save that must still report, ambiguous reassignments, and nested-block reachability. |
...
| Rule | Change | Why |
| --- | --- | --- |
| LC044 | No score change | Red regressions confirmed that nested property/field writes were missed and that initial suppression could let a sibling reattach, an optional branch/loop reattach, or a caught transfer hide an unsafe mutation. LC044 now records root-to-leaf graph paths per mutation and proves reattach, detach, and earlier-save coverage on each mutation-to-save path, including precise branch targets, caught exception types/filters, post-try operations, and handler exits. Review added collective proof for complementary `if`/`else` branches and normal/catch paths that each reattach, with optional-outer-branch, caught-throw-before-branch-reattach, nested caught-throw-before-handler-reattach, and finally-clear negatives so the collective proof cannot over-suppress; harmless finally blocks preserve valid coverage. The focused net10.0 LC044 slice passes 84 tests and the full net10.0 suite passes 2,012. |
...
- Focused red/green regression confirmed that LC044 missed `user.Address.City = value` after `user` was materialized through `AsNoTracking()`, then passed after mutation recording followed nested property receivers back to the queried local. Codex review then exposed both a configured/backing-field receiver gap and an explicit `[NotMapped]` false-positive boundary; red/green guardrails now follow property/field chains while stopping at source-visible non-mapped members. GitHub Codex review subsequently confirmed that `Update(user.Address)` and `Entry(user.Address).State = Modified` still reported; member-path-aware re-attach proof now suppresses matching root/owner/nested paths while a sibling re-attach remains diagnostic. Further Codex passes found that LC044 evaluated only the first mutation, allowing a safely reattached `Home` write to hide a later untracked `Work` write; optional post-mutation/foreach reattaches plus a lexically earlier unreachable save could suppress diagnostics; loop-local `break`/`continue` transfers could bypass an otherwise matching reattach; and caught throws could bypass proof while branch targets that still reached reattach were over-rejected. Suppression now runs independently per mutation path, requires reattach or earlier-save proof on every mutation-to-save path, distinguishes transfers that skip proof from those targeting it, and models compatible caught exceptions only when the reattach is inside the exited try and a non-constant-false handler can reach the save. It reports the earliest remaining unsafe mutation while preserving same-branch updates, post-try reattaches, non-matching/exclusive catches, and handlers that return or jump past saving. Direct, compound, and increment property mutations continue to use the same receiver-root path. The LC044 net10.0 slice passes 77 tests and the full net10.0 suite passes 2,005 tests.
