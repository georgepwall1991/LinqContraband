...
Reviewed: 2026-07-22 (full 45-rule parallel current-state audit at HEAD `a5360c2` / 5.6.43 — six domain batches re-checked analyzer source, focused tests, docs, samples, and catalog claims; **0 score changes**, note-only hygiene for test-count and LC045 collection-provenance drift; net10.0 suite **1,853** passed; candidate queue remains empty. Prior 2026-07-10 LC045 synchronous foreach/extraction; 2026-07-09 LC045 origin-aware control flow; 2026-07-08 LC010 local-delegate/method-group loop invocation detection; 2026-07-07 LC045 `DbContext.Set<TEntity>()` root detection; 2026-07-05 multi-rule guard passes; 2026-07-04 45-rule audit; 2026-07-03 LC037 StringBuilder statement-flow; 2026-07-02 multi-agent adversarial review with **7 defects test-confirmed**; repo hygiene 2026-06-26; full re-verification 2026-06-10; overnight rerun 2026-06-04; deep rescan 2026-05-29)
...
- Base audited commit: a5360c28eb9c17f6bd22ceec32a6e3c4cf78c4c8
...
| LC045 | Missing Include — navigation on materialized entity | Loading & Includes | Warning | 4 | 3 | 4 | 4 | 4 | 5 | Low | **New in 5.6.0; origin-aware flow 2026-07-09; sync foreach / element extraction 2026-07-10; collection provenance 2026-07 (HEAD `a5360c2` / #294).** Detects the canonical EF Core read-side bug: navigation access on a materialized (or direct-foreach) DbSet-rooted entity without matching `Include` — N+1 with lazy proxies, or null/empty data when no other loading mechanism or relationship fix-up applies. The usage scan caches a Roslyn control-flow graph per executable root and tracks materialized collections, extracted entities, aliases, **collection/navigation prefixes**, and binding generations by origin. Nested collection iteration composes prefixes so `item.Product` under `order.Items` reports maximally as `Items.Product`; collection navigation replacement invalidates nested provenance while still-proven earlier reads report. Reads proven before a later escape, capture, return, external store, result reassignment, or entity repoint still report; only subsequent uncertain reads stay quiet. Navigation writes satisfy later reads only for the same entity origin and only when every path reaching the read contains the write, including loop back-edges. Conditional-access, maximal-path, `DbContext.Set<TEntity>()`, and queryable-source fixer coverage remain intact (**212 focused tests** at HEAD). A stays 4 and FP stays `3`: `AutoInclude()` remains invisible and null-guarded access deliberately fires (documented). Remaining watch item: C# 14 null-conditional assignment for newer-Roslyn consumers. |
...
The 2026-07-22 full parallel re-audit (six domain batches over LC001–LC045 against live analyzer source, focused tests, docs, samples, and catalog metadata) found **no new source-reasoned High/Medium defect leads**. Prior July hardening closed the previous queue entries. Scorecard demotions based on unverified leads remain planning-risk demotions only; construct a red test and confirm each before treating anything as a shipped defect. (This is exactly how several 2026-06-04 probe claims were later *rejected* on verification.)

| Rule | Class | Lead (unverified) |
| --- | --- | --- |
| None | — | No source-reasoned current-state candidates remain after the 2026-07-22 audit. Documented residuals (e.g. LC045 `AutoInclude()` invisibility, LC015 Fluent-only composite-key partial fix) stay design-priced in scores, not queue items. |
...
| Priority | Rules | Work |
| --- | --- | --- |
| High — current-state lead | None | No High leads after the 2026-07-22 audit. Prior High items (LC002 `Last*` fixer safety, 2026-07-02 unsafe fixers/crashes) are test-confirmed and fixed. |
| Medium — current-state leads | None | No Medium leads. Prior Medium items (LC017/LC035 guard passes and related July hardening) are fixed. |
| Low — opportunistic hygiene | None | Candidate queue is empty. July hardening (LC045 origin-aware flow / collection provenance, LC010 delegate-loop detection, LC001/LC006/LC007/LC009/LC011/LC013/LC015/LC016/LC018/LC024/LC025/LC030/LC031/LC036/LC039/LC040/LC041/LC044, etc.) is folded into scorecard notes. Opportunistic watch items only: LC045 `AutoInclude()` if model metadata becomes source-visible; C# 14 null-conditional assignment on newer Roslyn consumers. |
...
Base audited commit: master at `a5360c28eb9c17f6bd22ceec32a6e3c4cf78c4c8` (2026-07-22 health-doc refresh; includes LC045 collection-provenance fix #294 on top of the 5.6.43 release line). Since the 2026-06-04 baseline (5.5.13): descriptor hygiene, repo/CI hardening, `IncludePathParser` shared by LC006/LC045, **LC045 shipped in 5.6.0** with the **5.6.1** chained-`?.` StackOverflow hot-fix, and the July 2026 adversarial + guard hardening through 5.6.43.
...
### 2026-07-22 current-state verification (this audit)

- Six parallel domain-batch reviewers covered **all 45 rules** (Query Shape, Materialization, Loading/Includes, Change Tracking, Execution/Async, Raw SQL + Schema + Bulk). Evidence: per-rule `LC0xx.md` + batch JSON under the session health-audit scratch, plus machine-readable `all-rules-summary.json` (one entry per rule: focused test count, fixer yes/no, doc/sample paths, scores, note_delta, lead).
- **Score changes: 0.** Confirmed scorecard metric columns match live health. **Note refreshes only:** LC001 (39 methods), LC002 (46 methods), LC010 (136 focused), LC014 (27 tests), LC032 (40 tests), LC033 (11 analyzer files), LC037 (multi-partial wording), LC045 (collection provenance + **212** focused tests).
- Candidate queue: **empty.** Planning Shortlist High/Medium: **none.**
- Full local net10.0 suite: **1,853** passed, 0 failed, 0 skipped (`dotnet test LinqContraband.sln --framework net10.0` at HEAD `a5360c2`).
- No analyzer/fixer production C# was changed in this audit — health doc + scratch evidence only.
- Optional catalog/sample tooling not required for scorecard truth; CI remains multi-TFM authority for net8/net9 on this Mac.

Historical verification notes retained below (2026-07-10 LC045 foreach pass and earlier July regressions). Those slice-level test counts (e.g. LC045 “221”) are pass-log claims; the scorecard authoritative focused-folder inventory for LC045 is **212** at this HEAD.

Current verification history (2026-07-10, LC045 synchronous foreach and exact extraction after the 5.6.43 release):
...
- Full local net10.0 suite passes 1,853 tests after the LC045 synchronous-foreach and extraction pass (re-confirmed 2026-07-22 at HEAD `a5360c2` with the same **1,853** total). `RuleCatalogDocGenerator --check` and `SampleDiagnosticsVerifier` for net10.0 pass locally; CI covers net8.0/net9.0 on Ubuntu.
