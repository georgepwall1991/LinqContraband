...
| LC045 | Missing Include — navigation on materialized entity | Loading & Includes | Warning | 4 | 3 | 4 | 4 | 4 | 5 | Low | **New in 5.6.0; origin-aware flow hardened 2026-07-09.** Detects the canonical EF Core read-side bug: navigation access on a materialized entity without `Include` — N+1 with lazy proxies, or null/empty data when no other loading mechanism or relationship fix-up applies. The usage scan now caches a Roslyn control-flow graph per executable root and tracks materialized collections, extracted entities, aliases, navigation prefixes, and binding generations by origin. Reads proven before a later escape, capture, return, external store, result reassignment, or entity repoint still report; only subsequent uncertain reads stay quiet. Navigation writes satisfy later reads only for the same entity origin and only when every path reaching the read contains the write, including loop back-edges. Existing conditional-access, maximal-path, `DbContext.Set<TEntity>()`, and queryable-source fixer coverage remains intact (179 tests). A stays 4 and FP stays `3`: `AutoInclude()` remains invisible and null-guarded access deliberately fires (documented). Remaining watch item: C# 14 null-conditional assignment for newer-Roslyn consumers. |
...
Current verification (2026-07-09, LC045 origin-aware control flow after the 5.6.43 release):
...
- Full local net10.0 suite passes 1811 tests on the LC045 origin-aware control-flow branch. `RuleCatalogDocGenerator --check` and `SampleDiagnosticsVerifier` for net10.0 pass locally; CI covers net8.0/net9.0 on Ubuntu.
