...
| Low — opportunistic, Tier-1-importance hygiene first | None | LC045 parenthesized conditional path reporting, LC018 `SqlQueryRaw<T>` fixer residual, LC005 local sort reset, LC027 relationship-builder local false positive, LC043 nested-capture fixer guard, LC022 modern EF guidance, LC041 null-conditional scalar-read FN, LC029 identity-boundary guidance, LC008 docs/sample depth, LC002 docs/sample depth, LC004 docs depth, LC039 doc expansion, LC028 test depth, LC019 docs/test depth, LC038 docs/test depth, LC035 docs/test depth, LC026 docs/test depth, LC003 docs/test depth, and LC021 suppression/test depth are addressed. Promote new low-priority work only on fresh concrete evidence. |
...
Current local verification (2026-06-26, release-bound LC029 identity Select boundary pass):
...
- `dotnet test tests/LinqContraband.Tests/LinqContraband.Tests.csproj --framework net10.0 --filter "FullyQualifiedName~LC029_RedundantIdentitySelect" --verbosity minimal` passed with **16 LC029 tests**.
- `dotnet test LinqContraband.sln --no-build --framework net10.0 --verbosity minimal` passed with **1152 tests**.
