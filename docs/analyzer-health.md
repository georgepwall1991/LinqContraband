...
| LC007 | Database execution inside loop | Execution & Async | Warning | 4 | 4 | 4 | 4 | 5 | 5 | Low | Strong analyzer for proven EF execution inside loops: `Find`, explicit loads, materializers and aggregates after deferred `AsEnumerable()` boundaries, navigation `Query()`, `DbContext.Set<T>()`, single-assignment query locals, and EF set-based executors. Tests cover 24 LC007 cases: 19 analyzer and 5 fixer. The fixer remains deliberately narrow, rewriting only unconditional strongly typed explicit-loading in `foreach`/`await foreach` loops to `Include` and removing the per-item load. Docs/sample/catalog are current; the doc is now 89 lines. |
...
| LC036 | DbContext captured by thread work item | Execution & Async | Warning | 4 | 4 | 4 | 4 | 5 | 5 | Low | High-value thread-safety rule covering lambda, anonymous-method, callback, async-lambda, member capture, factory/scope-safe, materialized-value, `Parallel.For`/`ForEach`/`Invoke`, and local-function shapes (20 tests). The method-group FN claim was rejected — arbitrary method-group inspection is a documented non-goal. Compact 48-line doc remains a DS=5 anchor (violation, safer shape, intent, explicit non-goals). |
...
| Rule | Change | Why |
| --- | --- | --- |
| LC036 | No score change | Test-confirmed the current-state lead: `DbContext` captures in `Parallel.For(...)` and `Parallel.Invoke(...)` callbacks stayed silent even though `Parallel.ForEach(...)` already reported the same unsafe cross-thread capture. LC036 now treats `Parallel.For`, `Parallel.ForEach`, and `Parallel.Invoke` consistently while preserving the safe shape where the callback creates its own context. LC036 now has 20 tests. |
...
| Low — opportunistic hygiene | Candidate queue | LC016 expression-bodied fixer no-op, LC015 downstream-sort suppression, LC045 widened-enumerable fixer safety, LC044 prior-reattach suppression, LC007 deferred `AsEnumerable()` execution detection, and LC036 Parallel API capture detection are test-confirmed and fixed. All remaining source-reasoned findings stay in the candidate queue until independently test-confirmed or rejected. |
...
Current verification (2026-07-04, stacked LC023+LC027+LC017+LC035+LC016+LC015+LC045+LC044+LC007+LC036 fix branches after the 5.6.38 release):
...
- Focused red/green regressions confirmed that LC036 missed `DbContext` captures inside `Parallel.For(...)` and `Parallel.Invoke(...)`, then passed after the analyzer recognised those `System.Threading.Tasks.Parallel` APIs alongside `Parallel.ForEach(...)`. A callback-local context guard confirms `Parallel.For(...)` remains quiet when the work creates its own context. The LC036 net10.0 slice passes 20 tests.
- Full local net10.0 suite passes 1288 tests on the stacked LC023+LC027+LC017+LC035+LC016+LC015+LC045+LC044+LC007+LC036 fix branch; `RuleCatalogDocGenerator --check`, `dotnet build LinqContraband.sln --no-restore --configuration Release`, and `SampleDiagnosticsVerifier` across net8.0/net9.0/net10.0 all pass on the LC036 branch.
