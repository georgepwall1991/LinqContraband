...
| LC046 | Concurrent EF Core operations on the same DbContext | Execution & Async | Warning | 3 | 4 | 4 | 4 | 4 | 5 | Medium | **New in 5.7.0.** Reports the second proven overlapping async EF Core operation on one context, with the first operation as an additional location, and covers query materializers/aggregates, find, load, save, set-based writes, relational commands, aliases, `DbContext.Set<TEntity>()`, and exact `Task.WhenAll(Enumerable.Select(...))` fan-out. Forty-one focused net10.0 tests lock sequential and conditional awaits, complementary completion branches, separate/per-item contexts, branch exclusivity, stable versus reassigned/escaped state, computed/repository boundaries, exact-symbol lookalikes, and LC036 ownership. Analyzer maturity stays at 3 and priority Medium until indirect helper-returned tasks, escaped task collections, and loop-built task lists have evidence-backed coverage. |
...
| --- | --- | --- |
| LC046 | **New row: A3 / FP4 / FS4 / T4 / DS4 / Imp5, Medium** | Red/green development established the catalog contract and direct same-context `Task.WhenAll` failure before implementation. The analyzer now detects proven overlapping queries, saves, finds, loads, set-based commands, relational raw commands, direct task locals, stable aliases, and exact selector fan-out. Conservative negatives cover sequential awaits, distinct or per-item contexts, branch joins, reassignments, escapes, computed properties, repository queries, query construction, custom lookalikes, and LC036-owned thread APIs. Conditional completion no longer suppresses the still-active path, while complementary guaranteed completion remains quiet. The focused net10.0 slice passes 41 tests; catalog/architecture coverage passes 244 tests; the full local net10.0 suite passes 2,299 tests. |
...
| --- | --- | --- |
| LC046 | FN maturity watch | Indirect helpers returning EF tasks, escaped task collections, and loop-built task lists remain deliberately outside the first release until concrete source patterns can be reproduced without weakening context-origin precision. |
...
| Medium — current-state leads | LC046 | Mature the new rule only from test-confirmed real shapes: indirect helper-returned tasks, escaped task collections, or loop-built task lists. |
...
- LC046 red/green verification passes 41 focused tests, the catalog/architecture filter passes 244 tests, and the full local net10.0 suite passes 2,299 tests. `RuleCatalogDocGenerator --check` and `SampleDiagnosticsVerifier` for net10.0 pass with 46 catalog entries; CI covers net8.0/net9.0 on Ubuntu.
