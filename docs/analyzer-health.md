...
| Low — opportunistic hygiene | Candidate queue | LC016 expression-bodied fixer no-op, LC015 downstream-sort suppression, LC045 widened-enumerable fixer safety, LC044 prior-reattach suppression, LC007 deferred `AsEnumerable()` execution detection, LC036 Parallel/delegate callback capture detection, LC010 scoped-receiver fixer safety, LC025 identity-resolution no-tracking detection, LC024 GroupBy result-selector detection, LC031 `ToLookup()` materializer detection, and LC040 `DbContext.Set<TEntity>()` tracked-source detection are test-confirmed and fixed. All remaining source-reasoned findings stay in the candidate queue until independently test-confirmed or rejected. |
