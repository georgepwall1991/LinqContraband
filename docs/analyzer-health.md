...
| LC037 | Constructed raw SQL strings | Raw SQL & Security | Warning | 4 | 4 | 4 | 4 | 4 | 5 | Low | Strong manual security rule (3-file analyzer: concatenation, `string.Format`/`Concat`, `StringBuilder`, aliased-local resolution; 27 tests); 5.5.5 added `SqlQueryRaw<T>` as a construction sink with no LC018 double-report. **2026-07-02 recursion guard shipped:** the valid `sql = sql + id; ExecuteSqlRaw(sql)` self-reference now reports normally instead of driving `TryResolveLocalValue` into an unbounded StackOverflow. Local resolution only considers writes whose whole operation ends before the reference being resolved, and recursive local/string-builder chasing is capped, preserving prior alias/conditional-write behaviour while preventing compiler/IDE aborts. Candidate StringBuilder statement-flow FN remains unverified and should be test-confirmed before promotion. |
...
| Priority | Rules | Work |
| --- | --- | --- |
| High — **2026-07-02 test-confirmed**, fix in order | **LC003**, LC008, LC026, LC032 | **LC003**: negate on any constant-zero, not just a bare `0` literal (currently inverts the check). **LC008**: parenthesize `(await …)` when the awaited call is the receiver of a further `.`/`[]`/`()` (currently CS1061). **LC026**: target the *diagnosed* invocation, not the innermost, when inserting the token (currently build-break on `.Where(...).ToListAsync()`). **LC032**: decline the fix (or bail) when the source chain contains `Skip`/`Take`/`Distinct` (currently rewrites to a runtime-throwing `ExecuteUpdate`). LC037's `sql = sql + id` StackOverflow has been fixed and regression-locked. |
