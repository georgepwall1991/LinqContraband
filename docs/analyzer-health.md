...
| Priority | Rules | Work |
| --- | --- | --- |
| High | LC006, LC015 | **LC006**: document the `AsSplitQuery` tradeoff (extra roundtrips, lost index usage, when Cartesian is legitimate for nested paths) and tighten complex-include-chain flow handling. **LC015**: either deepen the no-fix rationale and expose a clear stable-key prompt, or implement a fixer for the unambiguous "single keyed entity over IQueryable<T>" shape. |
| Medium | LC002, LC009, LC012, LC018, LC024, LC035 | LC002 needs provider-variance and intentional-client-boundary docs. LC009's fixer (FS=2) needs negative/intentional-tracking coverage. LC012 needs unit-of-work timing docs and richer fixer tests. LC018's constant-only FP gap closed in 5.4.5; cross-provider raw-SQL API variants outside `FromSqlRaw` remain. LC024 needs an enumerated safe-aggregate guide. LC035 needs richer filtered-local/reassignment cases. |
