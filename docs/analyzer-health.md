...
| LC006 | Multiple collection Includes | Loading & Includes | Warning | 3 | 4 | 3 | 4 | 2 | 4 | **High** | High-impact rule but the `AsSplitQuery()` fixer trades one risk for another (separate roundtrips, index loss) and the 29-line doc never explains the tradeoff or when Cartesian is legitimate; complex include-chain flow handling is limited. |
...
| LC015 | Missing OrderBy before pagination | Query Shape & Translation | Warning | 4 | 4 | 3 | 4 | 4 | 5 | **High** | High-value reliability rule with strong alias-following; FS stays at `3` because the fixer is intentionally advisory (no key inference) — for a `Warning` of this importance the lack of an actionable fix is the primary user gap. |
...
| LC018 | FromSqlRaw with interpolated strings | Raw SQL & Security | Warning | 4 | 3 | 4 | 4 | 4 | 5 | **High** | Security-critical rule with broad call-shape coverage; FP drops to `3` because constant-only interpolation guards are not exhaustively tested and the doc still under-covers cross-provider raw-SQL API variants and the LC037 boundary. |
...
| Priority | Rules | Work |
| --- | --- | --- |
| High | LC006, LC015, LC018 | **LC006**: document the `AsSplitQuery` tradeoff (extra roundtrips, lost index usage, when Cartesian is legitimate for nested paths) and tighten complex-include-chain flow handling. **LC015**: either deepen the no-fix rationale and expose a clear stable-key prompt, or implement a fixer for the unambiguous "single keyed entity over IQueryable<T>" shape. **LC018**: lock in constant-only interpolation FP coverage, document the LC037 boundary, and expand cross-provider raw-SQL API coverage. |
| Medium | LC002, LC009, LC012, LC024, LC035 | LC002 needs provider-variance and intentional-client-boundary docs. LC009's fixer (FS=2) needs negative/intentional-tracking coverage. LC012 needs unit-of-work timing docs and richer fixer tests. LC024 needs an enumerated safe-aggregate guide. LC035 needs richer filtered-local/reassignment cases. |
...
- `dotnet test tests/LinqContraband.Tests/LinqContraband.Tests.csproj --framework net10.0 --no-restore --filter FullyQualifiedName~LC018_AvoidFromSqlRawWithInterpolation` passed with 24 tests.
...
- `dotnet test LinqContraband.sln --framework net10.0 --no-restore` passed with 800 tests.
