...
| LC023 | Prefer Find/FindAsync for primary key lookups | Materialization & Projection | Info | 4 | 4 | 3 | 3 | 3 | 3 | Medium | Careful key/Async detection with a shipped fixer — but the deferred Round-2 correctness concern is unresolved: `Find` checks the change tracker first, and a tracker hit bypasses global query filters, so on a `HasQueryFilter` entity the `FirstOrDefault(pk)` → `Find` rewrite can return an already-tracked soft-deleted / other-tenant row the original query would have filtered out (the database-query path does apply filters). FS and T drop to `3` until the fixer (or the diagnostic) is gated on `HasQueryFilter`; 16 tests include no query-filter negatives and no dedicated fixer-edge file. |
...
| LC045 | Missing Include — navigation on materialized entity | Loading & Includes | Warning | 3 | 3 | 4 | 4 | 4 | 5 | Medium | **New in 5.6.0.** Detects the canonical EF Core read-side bug: navigation access on a materialized entity without `Include` — N+1 with lazy proxies, silent null without. Deep flow analysis (shape-preserving-operator chain proof to a DbSet, Include-path parsing via shared `IncludePathParser`, nav-write satisfaction, entity-escape and repointed-local guards, `?.`/indexed/constructor shapes) and a safe Include/ThenInclude-inserting fixer with keyword escaping. 58 tests incl. post-crash regressions; 87-line doc with 8 documented FP disciplines. **A/FP capped at `3`**: 5.6.0 shipped an uncatchable StackOverflowException on chained `?.` that killed csc (hot-fixed in 5.6.1), four review rounds were needed pre-ship, and `AutoInclude()` is invisible while null-guarded access deliberately fires — the conditional-access surface needs an adversarial pass before this scores like a mature rule. |
...
| Medium — next batch, in order | LC045, LC023, LC012, LC025, LC009, LC008 | **LC045** (Imp 5, A/FP 3): run a dedicated adversarial `analyzer-fp-fn-hunter` pass over the conditional-access surface — chained/nested `?.` with indexers (`order?.Items?[0].Product?.Name`), mixed `!`/cast/`?.` paths, nested scopes — plus negative-confirm the out-of-scope shapes; the 5.6.0 csc crash says this flow analysis is the highest-risk code in the package. **LC023** (only live shipped-fix hazard): confirm the narrowed hazard against EF Core 9 — `Find` returns an already-tracked filtered-out instance (tracker hit bypasses the filter) while its database path applies filters — then gate the fixer (or the diagnostic) on `HasQueryFilter`, or document the tracked-instance divergence. **LC012**: same-instance + reachability analysis for cross-context / mutually-exclusive-branch `SaveChanges`. **LC025**: port LC044's `HasMultipleAssignments` ambiguity guard for conditionally-reassigned locals. **LC009**: cross-method mutation heuristic / identity-resolution fixer variant. **LC008**: the unfixable orphaned warning on a sync EF call in a `static` local function. |
