...
> Rows below are the **2026-07-04 current-state audit** — the 2026-06-04 rerun deltas, 2026-06-10 full re-verification, LC045 releases, 2026-07-02 adversarial fixes, 2026-07-03 LC037 hardening, and the 2026-07-04/05 LC002 `Last*`, LC021 named-filter, LC012 later-save/query-source, LC023 self-referential predicate, LC027 Fluent key-type, LC017 projection-member, LC035 filtered-path, LC016 expression-bodied, LC015 downstream-sort pagination, LC045 queryable-source, LC044 prior-reattach, LC007 deferred `AsEnumerable()` execution and nested-foreach source execution, LC036 Parallel/delegate callback capture, LC010 scoped-receiver fixer and fresh-context-per-iteration guard, LC025 identity-resolution no-tracking and multi-origin fixer safety, LC024 GroupBy result-selector, LC031 `ToLookup()` materializer, LC040 `DbContext.Set<TEntity>()` tracked-source, LC006 reference-prefixed sibling, LC018 SQL identifier-position fixer guards, LC001 nested-lambda/fixer-safety guards, LC030 static DbContext storage detection and computed-property guard, LC013 custom extension boundaries, LC039 switch-expression branch guards, and LC011 current-assembly configuration scanning are folded into the rows. Every rule was independently re-checked against current source, tests, docs, samples, and the candidate queue; subsequent hardening has raised the local net10.0 suite to **1352 tests**.
...
| LC030 | DbContext lifetime mismatch | Change Tracking & Context Lifetime | Info | 4 | 4 | 4 | 4 | 5 | 4 | Low | Strong manual-only lifetime rule: long-lived types proven via interface/base-class, middleware, hosted-service, singleton DI registration, direct singleton DbContext registration, and optional `long_lived_types` config. Static `DbContext` fields/properties on proven long-lived types are reported while static storage on unproven types remains quiet. Computed getter properties that directly create a fresh context through `IDbContextFactory<TContext>.CreateDbContext()` or `new TContext()` stay quiet, while auto-properties, initialized get-only properties, and root-service-provider lookups still report. Current suite has 29 analyzer tests, no fixer, and an updated doc explaining why no single safe fix exists. Severity keeps it out of the urgent stack. |
...
| --- | --- | --- |
| LC009 | FP | Still current on source review, but narrower than the closed 2026-06-10 helper-save issue: `MaterializedEntityIsMutated` recognises writes whose property instance is the materialized entity local itself, but not nested entity-member writes such as `user.Profile.DisplayName = name`, where the write target's instance is another property reference. Add a red regression before teaching LC009 to treat nested member-state writes as write paths. |
| LC017 | unsafe-fix | The broad direct/null-conditional/indexed projection-member fixer lead is fixed, but a narrower unconfirmed fixer-safety lead remains: when a diagnostic combines a fixer-supported access such as `e.Id` with a cast/interface/base-conversion access such as `((IHasName)e).Name` or `((BaseEntity)e).Name`, the analyzer can count both properties while the fixer may project only the supported direct member and leave downstream code depending on the original entity shape. Add a red fixer test that expects no code fix for mixed supported plus conversion-based member access before changing implementation. Direct inherited base-property access without a cast is already supported. |
| LC041 | missing-fix | Still current on source review: `users.Where(x => x.IsActive).First()` followed by one direct property read reports LC041, but `RegisterCodeFixesAsync` can resolve the diagnostic start token to the inner `Where(...)` invocation before `IsSafeFixMaterializer`, so no safe projection fix is attached. Add a red code-fix regression expecting `users.Where(...).Select(x => x.Name).First()` before changing behavior. |
| LC041 | unsafe-fix | Source-confirmed but not yet red-test-confirmed: when the terminal materializer predicate is hoisted as `Expression<Func<TEntity, bool>>` or `Func<TEntity, bool>`, the fixer does not recognise it as a movable predicate and leaves it on the scalar terminal after inserting `Select`, for example `users.Select(x => x.Name).First(active)`, which does not compile because `active` still targets `TEntity`. Add a red fixer test and then either move the hoisted predicate into `Where` before `Select` when type-safe, or withhold the code fix for non-inline terminal predicates. |
...
| Low — opportunistic hygiene | Candidate queue | LC016 expression-bodied fixer no-op, LC015 downstream-sort suppression, LC045 widened-enumerable fixer safety, LC044 prior-reattach suppression, LC007 deferred `AsEnumerable()` execution detection, LC007 nested-foreach source execution detection, LC006 reference-prefixed sibling detection, LC018 SQL identifier-position fixer safety, LC036 Parallel/delegate callback capture detection, LC010 scoped-receiver fixer safety and fresh-context-per-iteration suppression, LC025 identity-resolution no-tracking detection and multi-origin fixer safety, LC024 GroupBy result-selector detection, LC031 `ToLookup()` materializer detection, LC040 `DbContext.Set<TEntity>()` tracked-source detection, LC001 nested-lambda/fixer-safety guards, LC030 static DbContext storage detection and computed-property guard, LC013 custom extension boundaries, LC039 switch-expression branch guards, and LC011 current-assembly configuration scanning are test-confirmed and fixed. All remaining source-reasoned findings stay in the candidate queue until independently test-confirmed or rejected; the broad LC017 projection-member lead is fixed, but a narrower cast/interface/base-conversion fixer-safety lead remains queued. |
...
| LC009 | Tracked mutation persisted via a helper / cross-method SaveChanges (FP) | **Fixed 2026-06-10.** Property mutations of the materialized entity now mark the body as a write path for direct result locals, foreach variables over the result, inline materializer property writes, compound assignments, and increment/decrement operations; the remaining returned-untouched-and-mutated-by-caller case stays documented as invisible and severity remains Info. A narrower nested-member mutation residual is tracked in the current candidate queue. |
...
Current verification (2026-07-05, stacked hardening branches through LC030 computed `DbContext` property guard after the 5.6.38 release):
...
- Full local net10.0 suite passes 1352 tests on the stacked LC030 computed `DbContext` property guard branch. `RuleCatalogDocGenerator --check`, `dotnet build LinqContraband.sln --no-restore --configuration Release`, and `SampleDiagnosticsVerifier` across net8.0/net9.0/net10.0 all pass on this branch.
