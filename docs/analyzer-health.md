...
| Priority | Rules | Work |
| --- | --- | --- |
| High | LC043 | Fresh review found an unsafe fixer edge: a buffered local captured inside a nested lambda/local executable root can be missed by the single-use check, allowing the fixer to remove the local and leave the capture broken. Next pass should add the nested-capture regression and count nested executable-root references as additional uses, plus token/buffer-argument guard coverage if still scoped. |
...
| Low — opportunistic, Tier-1-importance hygiene first | LC022 | LC045 parenthesized conditional path reporting, LC018 `SqlQueryRaw<T>` fixer residual, LC005 local sort reset, LC027 relationship-builder local false positive, LC008 docs/sample depth, LC002 docs/sample depth, LC004 docs depth, LC039 doc expansion, LC028 test depth, LC019 docs/test depth, LC038 docs/test depth, LC035 docs/test depth, LC026 docs/test depth, LC003 docs/test depth, and LC021 suppression/test depth are addressed. LC022 has minor sample/health wording drift around modern EF correlated collection translation, but it is too marginal to outrank the LC043 unsafe-fix item. |
...
Current local verification (2026-06-26, release-bound LC027 relationship-builder local pass):
...
- `dotnet test tests/LinqContraband.Tests/LinqContraband.Tests.csproj --framework net10.0 --filter "FullyQualifiedName~LC027_MissingExplicitForeignKey" --verbosity minimal` passed with **27 LC027 tests**.
- `dotnet test LinqContraband.sln --no-build --framework net10.0 --verbosity minimal` passed with **1132 tests**.
