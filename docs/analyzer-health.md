...
| Medium — current-state leads | LC035 | Confirm with red tests before repair: LC035 definitely assigned/all-filtered conditional receiver false positives. |
...
Current verification (2026-07-04, stacked LC023+LC027+LC017 fix branches after the 5.6.38 release):
...
- Full local net10.0 suite passes 1241 tests on the stacked LC023+LC027+LC017 fix branch; `RuleCatalogDocGenerator --check`, `dotnet build LinqContraband.sln --no-restore --configuration Release`, and `SampleDiagnosticsVerifier` across net8.0/net9.0/net10.0 all passed there.
