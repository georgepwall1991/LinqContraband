...
| LC034 | ExecuteSqlRaw with interpolated strings | Raw SQL & Security | Warning | 4 | 4 | 4 | 4 | 4 | 5 | Low | Strong sync/async raw-SQL detection (60 tests) with named/direct unsafe SQL, lookalike negatives, supported core-library-scalar positives, and custom/generic/enum/framework-lookalike type, adjacent-hole, command/multi-statement shape, quote/escaped-quote, quoted-identifier apostrophe, doubled-delimiter identifier, structural-position, and mixed sync/async FixAll coverage. Docs now spell out LC018/LC034/LC037 ownership, direct-vs-hidden construction, quoted-interpolation limits, and parameterized `ExecuteSqlRaw` alternatives. |
...
| Rule | Change | Why |
| --- | --- | --- |
| LC034 | No score change | A red fixer regression confirmed that direct structural interpolation such as `ExecuteSqlRaw($"DELETE FROM {tableName}")` received an `ExecuteSql(...)` rewrite even though database parameters cannot represent SQL identifiers. LC034 now limits automatic fixes to proven core-library scalar parameters in unambiguous single-statement `UPDATE`/`DELETE`/`INSERT` value positions. Review then closed unsafe boundaries around ambiguous reference and custom/generic/enum/framework-lookalike value types, adjacent holes, provider-specific and trailing multi-statement commands, provider-style backslash-escaped quotes, and bracketed, double-quoted, or backtick-delimited identifiers including apostrophes and doubled delimiter escapes. Mixed sync/async FixAll now shares one action identity. Ambiguous shapes remain diagnostic-only; proven scalar DML rewrites retain dedicated positive coverage. The focused net10.0 LC034 slice passes 60 tests. |
...
Current verification (2026-07-16, LC034 structural SQL fixer guard after the 5.6.45 release):

- Focused red/green regression confirmed that `ExecuteSqlRaw($"DELETE FROM {tableName}")` received an `ExecuteSql(...)` fix that would parameterize a table identifier as a value and fail at runtime, then passed after LC034 adopted a likely-value-position guard. Codex review then narrowed automatic fixes to proven core-library scalar parameters in unambiguous single-statement `UPDATE`/`DELETE`/`INSERT` value positions and added guards for adjacent holes, custom/generic/enum/framework-lookalike value types, provider-specific and trailing multi-statement commands, provider-style backslash-escaped quotes, and bracketed, double-quoted, or backtick-delimited identifiers including apostrophes and doubled delimiter escapes. Mixed sync/async FixAll now shares one action identity. Ambiguous shapes remain diagnostic-only while proven scalar DML rewrites retain dedicated positive coverage. The LC034 net10.0 slice passes 60 tests and the full net10.0 suite passes 1,963 tests.
