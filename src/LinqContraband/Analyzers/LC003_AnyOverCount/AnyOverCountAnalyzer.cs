...
namespace LinqContraband
{
    ...
    public class AnyOverCountAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static readonly LocalizableString MessageFormat = "Use Any() instead of Count() > 0 for efficient existence checking on IQueryable";
        private static readonly LocalizableString Description = "Checking if Count() is greater than 0 on an IQueryable can be expensive as it may iterate the entire result set. Any() is optimized to return as soon as a match is found.";
        ...
        private void AnalyzeBinaryOperator(OperationAnalysisContext context)
        {
            ...
            if (binaryOp.OperatorKind == BinaryOperatorKind.GreaterThan)
            {
                ...
                constantValue = binaryOp.RightOperand.ConstantValue.HasValue ? binaryOp.RightOperand.ConstantValue.Value : null;
            }
            else // LessThan
            {
                // 0 < Count()
                constantValue = binaryOp.LeftOperand.ConstantValue.HasValue ? binaryOp.LeftOperand.ConstantValue.Value : null;
            ...
            while (countInvocation is IConversionOperation conversion)
            {
