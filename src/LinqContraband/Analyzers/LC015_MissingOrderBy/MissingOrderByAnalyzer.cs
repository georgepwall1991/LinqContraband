...
namespace LinqContraband.Analyzers.LC015_MissingOrderBy
{
    ...
    public class MissingOrderByAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static readonly LocalizableString MessageFormat = "The method '{0}' is called on an unordered IQueryable. Call 'OrderBy' or 'OrderByDescending' first to ensure deterministic results.";
        private static readonly LocalizableString Description = "Pagination and Last operations on unordered IQueryables are non-deterministic.";
        ...
        private bool HasOrderByUpstream(IOperation operation)
        {
            ...
            while (current != null)
            {
                if (current is IInvocationOperation inv)
                {
                    ...
                    if (OrderingMethods.Contains(method.Name) && method.ReturnType.IsIQueryable())
                    {
                        return true;
                    }
        ...
        private bool IsOrderedQueryable(ITypeSymbol type)
        {
             ...
             {
                 if (i.Name == "IOrderedQueryable" && i.ContainingNamespace?.ToString() == "System.Linq") return true;
             }
             return false;
        }
    }
