...
namespace LinqContraband.Sample.Samples.LC015_MissingOrderBy
{
    ...
    /// <remarks>
    /// <para>
    /// <strong>The Crime:</strong> Calling <c>Skip</c>, <c>Last</c>, or <c>Chunk</c> on an <c>IQueryable</c> without explicitly sorting it first.
    /// </para>
    /// <para>
    /// <strong>Why it's bad:</strong> Without an explicit <c>OrderBy</c>, the database does not guarantee the order of results.
    /// This makes pagination non-deterministic (items might appear on multiple pages or be skipped entirely) and <c>Last()</c> calls unpredictable.
    /// </para>
    /// <para>
    /// <strong>The Fix:</strong> Call <c>OrderBy</c> or <c>OrderByDescending</c> before applying pagination or retrieving the last element.
    /// </para>
    /// </remarks>
